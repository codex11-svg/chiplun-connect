...
export default function App() {
  ...
  // Check if a specialist is busy at the exact user-selected time
  const getBusyStaff = (date, time) => {
    if (!selectedStore || !date || !time) return [];
    return allBookings
      ...
      {showConfirmModal && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-[300] flex items-center justify-center p-6 animate-in zoom-in-95">
           <div className="bg-white w-full rounded-[3rem] p-8 shadow-2xl">
              <div className="w-16 h-16 bg-emerald-100 text-emerald-600 rounded-full flex items-center justify-center mx-auto mb-4"><Lucide.CheckCircle size={32}/></div>
              <h3 className="text-xl font-black text-center mb-2">Confirm Booking</h3>
              <div className="bg-gray-50 p-5 rounded-2xl space-y-3 mb-6">
                 <div className="flex justify-between text-[10px] font-black uppercase"><span>Expert:</span><span className="text-emerald-700">{bookingMeta.resourceName || 'First Available'}</span></div>
                 <div className="flex justify-between text-[10px] font-black uppercase"><span>Time:</span><span>{bookingMeta.time}</span></div>
                 <div className="flex justify-between text-[10px] font-black uppercase pt-2 border-t border-gray-200"><span>Payable:</span><span className="text-emerald-600">₹{cart.reduce((a, b) => a + Number(b.price), 0)}</span></div>
              </div>
              <button disabled={isProcessing} onClick={handleFinalConfirm} className="w-full bg-emerald-600 text-white py-5 rounded-2xl font-black uppercase tracking-widest text-xs shadow-lg disabled:opacity-50">
      ...
      {deleteTarget && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-[300] flex items-center justify-center p-6">
           <div className="bg-white w-full rounded-[3rem] p-8 shadow-2xl border-2 border-rose-100">
              ...
              <p className="text-center text-gray-400 text-xs mt-3 leading-relaxed">This will erase <span className="font-bold text-gray-900">{deleteTarget.name}</span> and all its credentials forever.</p>
              <div className="flex flex-col gap-3 mt-8">
                 <button onClick={handleFullStoreDeletion} className="w-full bg-rose-500 text-white py-4 rounded-2xl font-black uppercase text-xs">Destroy Store</button>
      ...
      <header className="bg-emerald-600 text-white p-6 pb-12 rounded-b-[4rem] shadow-lg sticky top-0 z-50">
        <div className="flex justify-between items-center mb-6">
          <div onClick={() => setView('home')} className="cursor-pointer active:scale-95 transition-all">
            ...
            <p className="text-[9px] font-bold opacity-70 uppercase tracking-widest">Enterprise Pro • V14</p>
          </div>
        ...
        {view === 'home' && (
          <div className="relative">
            ...
            <input type="text" placeholder="Search businesses..." className="w-full bg-white/10 border border-white/20 rounded-2xl py-3.5 pl-12 pr-4 text-white placeholder-emerald-200 outline-none" />
          </div>
        )}
      </header>
        ...
        {view === 'store_detail' && selectedStore && (
           <div className="pt-4 space-y-6 animate-in slide-in-from-right-4 pb-10">
             ...
             <section className="bg-white p-6 rounded-[3rem] shadow-sm border border-gray-100">
               ...
               <h3 className="text-[10px] font-black text-gray-400 uppercase mb-4 font-black">Services Available</h3>
             ...
             {cart.length > 0 && (
               <div className="bg-white p-6 rounded-[3rem] shadow-sm border border-gray-100 animate-in slide-in-from-bottom-4">
                 <h3 className="text-[10px] font-black text-gray-400 uppercase mb-6 text-center tracking-widest">Select Schedule</h3>
                 <div className="space-y-4">
                   <div className="flex gap-2">
                      ...
                      {/* TIME SELECTION - UPDATED TO MANUAL PICKER */}
                      <div className="w-32 relative">
                        <input 
                          type="time" 
                          onChange={e => setBookingMeta({...bookingMeta, time: e.target.value})} 
                          className="w-full bg-gray-50 p-4 rounded-2xl font-bold text-sm outline-none border border-gray-100 shadow-inner" 
                        />
                      </div>
                   </div>

                   {/* SPECIALIST SELECTION - FIXED */}
                   <div className="space-y-1.5">
                      <p className="text-[9px] font-black text-gray-400 uppercase ml-1">Assign Expert</p>
                      <select 
                   ...
                   <input placeholder="WhatsApp Number" type="tel" value={bookingMeta.custPhone} onChange={e => setBookingMeta({...bookingMeta, custPhone: e.target.value})} className="w-full bg-gray-50 p-4 rounded-2xl font-bold text-sm outline-none border border-gray-100 shadow-inner uppercase" />
        ...
        {view === 'dashboard' && isSessionVerified && profile.role === 'vendor' && (
          <div className="pt-4 space-y-6 animate-in slide-in-from-bottom-8 pb-10">
            ...
            <section className="bg-white p-7 rounded-[3rem] shadow-sm border border-gray-100 space-y-5 animate-in fade-in">
               ...
               <div className="space-y-4">
                  <div className="w-full h-32 bg-gray-50 rounded-2xl overflow-hidden border border-gray-100 shadow-inner">
                     <img src={editForm.image} className="w-full h-full object-cover" onError={(e) => e.target.src="https://via.placeholder.com/400x200?text=No+Logo"} />
                  </div>
                  <input placeholder="Logo Image URL" value={editForm.image} onChange={e => setEditForm({...editForm, image: e.target.value})} className="w-full bg-gray-50 p-4 rounded-xl font-bold text-xs outline-none uppercase" />
                  ...
                  <button onClick={async () => { await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'stores', profile.businessId), editForm); alert("Updated Successfully!"); }} className="w-full bg-emerald-600 text-white py-3 rounded-xl font-black uppercase text-[10px]">Sync Profile</button>
               </div>
            </section>
            ...
            <button onClick={() => updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'stores', profile.businessId), { isLive: !myStore?.isLive })} className={`w-full py-5 rounded-[2.5rem] font-black uppercase shadow-2xl active:scale-95 transition-all ${myStore?.isLive ? 'bg-rose-500 text-white' : 'bg-emerald-600 text-white'}`}>
               {myStore?.isLive ? 'Go Offline' : 'Publish Live'}
            </button>
          </div>
        )}

        {/* ... Admin Hub Login Logic (Stable PIN 112607) ... */}
        {view === 'admin_auth' && (
           <div className="pt-20 text-center">
              <Lucide.Lock className="mx-auto text-emerald-600 mb-6" size={48} />
              <h2 className="text-3xl font-black mb-10 tracking-tighter text-emerald-900 uppercase">Admin Core</h2>
              ...
              <button onClick={() => { if(adminPinInput===ADMIN_PIN) setView('admin_panel'); else { alert("DENIED"); setAdminPinInput(''); } }} className="w-full bg-emerald-600 text-white py-5 rounded-[2rem] font-black shadow-2xl uppercase">Unlock</button>
           </div>
        )}
        ...
        {view === 'merchant_login' && (
           <div className="pt-20 space-y-6 animate-in zoom-in-95 duration-500 px-4">
              <div className="text-center mb-10">
                 ...
                 <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest text-center">Verify Secure ID for MH-08 Hub</p>
              </div>
             ...
             <div className="bg-emerald-600 text-white p-8 rounded-[3.5rem] shadow-2xl space-y-6 relative overflow-hidden text-center">
                ...
                <div className="grid grid-cols-2 gap-4 border-t border-white/10 pt-6">
                   ...
                   <div className="text-center">
                      <p className="text-[8px] font-black uppercase opacity-60">Wait Time</p><p className="text-2xl font-black">+{queueDisplay.delay}m</p>
                   </div>
                </div>
                <div className="bg-white/10 p-2 rounded-xl text-[8px] font-black uppercase">Service scheduled for: {bookingMeta.time}</div>
             </div>
              ...
              {searchedBooking && (
                <div className="bg-emerald-600 text-white p-8 rounded-[3.5rem] shadow-2xl animate-in zoom-in-95">
                   <div className="flex justify-between items-start mb-6">
                      <div><p className="text-[10px] font-black uppercase opacity-60">Confirmed Partner</p><h3 className="text-xl font-black italic uppercase">{searchedBooking.storeName}</h3></div>
