...
const appId = "chiplun-pro-v22-stable"; 
...
export default function App() {
  ...
  // --- Targeted Fix: Track Token Engine ---
  const handleTrackToken = () => {
    if (!trackInput) return;
    const cleanToken = trackInput.trim().toUpperCase();
    // Scan all live bookings for the specific displayId
    const found = allBookings.find(b => b.displayId?.toUpperCase() === cleanToken);
    
    if (found) {
      setSearchedBooking(found);
    } else {
      alert("Token ID not found. Please verify the ID and ensure the store has confirmed it.");
    }
  };
  ...
  const handleFinalConfirm = async () => {
    ...
    const payload = { 
      ...
      queuePos, estWait: delay, storeId: selectedStore.id, storeName: selectedStore.name, timestamp: Date.now() 
    };
  ...
  if (loading) return <div className="h-screen flex items-center justify-center bg-emerald-600 text-white font-black animate-pulse uppercase tracking-widest text-xs">ChiplunConnect Pro V22</div>;
  ...
  return (
    <div className="max-w-md mx-auto bg-gray-50 min-h-screen flex flex-col shadow-2xl relative font-sans text-gray-900 overflow-x-hidden">
      ...
      {deleteTarget && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-md z-[600] flex items-center justify-center p-6 animate-in fade-in">
           <div className="bg-white w-full rounded-[3rem] p-10 shadow-2xl border-4 border-rose-500">
              ...
              <p className="text-center text-gray-400 text-xs mt-3 mb-10 leading-relaxed font-medium">Permanently destroy <span className="text-rose-600 font-bold underline">"{deleteTarget.name}"</span> and all login access.</p>
      ...
      <header className="bg-emerald-600 text-white p-6 pb-12 rounded-b-[4rem] shadow-lg sticky top-0 z-50">
        ...
        {view === 'home' && (
          <div className="relative animate-in slide-in-from-top-2">
            ...
            <input type="text" placeholder="Search verified services..." className="w-full bg-white/10 border border-white/20 rounded-2xl py-3.5 pl-12 pr-4 text-white placeholder-emerald-200 outline-none shadow-inner" />
          </div>
        )}
      </header>
      ...
      <main className="flex-1 -mt-6 px-4 pb-32 overflow-y-auto z-10">
        ...
        {view === 'home' && (
          <div className="space-y-8 pt-2">
             ...
             <section className="animate-in slide-in-from-bottom-4">
               <h2 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-4 ml-1 italic font-black">Verified in Chiplun</h2>
               <div className="space-y-4">
                 {stores.filter(s => s.isLive).map(store => (
                   <div key={store.id} onClick={() => { setSelectedStore(store); setView('detail'); setCart([]); }} className="bg-white p-3 rounded-[2.5rem] flex gap-4 items-center shadow-sm border border-gray-100 hover:border-emerald-200 active:scale-[0.98] transition-all">
             ...
             {cart.length > 0 && (
               <div className="bg-white p-6 rounded-[3rem] shadow-sm border border-gray-100 animate-in slide-in-from-bottom-4">
                 ...
                 <div className="space-y-4">
                   ...
                   <button disabled={!bookingMeta.date || !bookingMeta.time} onClick={() => setShowConfirmModal(true)} className="w-full bg-emerald-600 text-white py-5 rounded-3xl font-black shadow-xl uppercase active:scale-95 transition-all tracking-widest">Review & Secure</button>
                 </div>
              </div>
            )}
           </div>
              ...
              {searchedBooking ? (
                <div className="bg-emerald-600 text-white p-8 rounded-[3.5rem] shadow-2xl animate-in zoom-in-95 relative overflow-hidden">
                   ...
                   <div className="flex justify-between items-start mb-8 relative z-10">
                      <div><p className="text-[10px] font-black uppercase opacity-60">Verified Order</p><h3 className="text-xl font-black italic uppercase tracking-tight">{searchedBooking.storeName}</h3></div>
        ...
        {view === 'account' && (
           <div className="pt-4 space-y-6 px-4 pb-20">
              {!isSessionVerified ? (
                ...
                <div className="space-y-6 animate-in slide-in-from-bottom-8">
                   ...
                   {/* TARGETED UPDATE: Merchant Ledger with Service Name */}
                      ...
                      <div className="space-y-3 max-h-72 overflow-y-auto">
                        {merchantBookings.length > 0 ? merchantBookings.sort((a,b) => b.timestamp - a.timestamp).map((b, i) => (
                           <div key={i} className="bg-white/10 p-4 rounded-2xl border border-white/5 space-y-1">
                              ...
                              {/* ADDED SERVICE NAME AS REQUESTED */}
                              <p className="text-[9px] font-black text-emerald-100 uppercase bg-emerald-700/50 px-2 py-0.5 rounded inline-block mb-1">
                                Service: {b.services?.map(s => s.name).join(', ')}
                              </p>
                              <p className="text-[9px] opacity-70 uppercase font-bold tracking-tighter block">{b.date} • {b.time} • {b.resourceName || 'Stylist Assigned'}</p>
                        ...
                        )) : <p className="text-center py-10 text-[9px] opacity-60 font-bold uppercase tracking-widest italic">Zero active bookings</p>}
                      </div>
                   </section>
                   ...
                   <section className="bg-white p-7 rounded-[3rem] shadow-sm border border-gray-100 space-y-5 animate-in fade-in">
                      ...
                      <input placeholder="Logo Link" value={editForm.image} onChange={e => setEditForm({...editForm, image: e.target.value})} className="w-full bg-gray-50 p-4 rounded-xl font-bold text-xs outline-none shadow-inner uppercase" />
        ...
        {/* ... (Admin and Confirmation Views kept solid from V21) ... */}
        {view === 'admin' && (
          ...
          <div className="pt-4 space-y-6 pb-20 px-4">
            ...
            {stores.map(s => (
                <div key={s.id} className="bg-white p-5 rounded-[2.5rem] border border-gray-100 flex items-center gap-4 animate-in fade-in shadow-sm">
                   ...
                   <div className="flex-1 font-bold text-sm uppercase">{s.name} <span className="block text-[8px] opacity-40 italic">{s.category} • ID: {s.merchantId}</span></div>
