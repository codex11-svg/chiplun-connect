...
export default function App() {
  ...
  const mData = useMemo(() => {
    ...
    return {
      store: s || { resources: [], services: [], category: 'salon' },
  ...
  const getResourceStats = (sId, rId, totalCap) => {
    ...
    return { count: active.length, left: (Number(totalCap) || 0) - taken };
  };
  ...
  const handleBookingConfirm = async () => {
    ...
    try {
      ...
      const price = Number(activeCart?.price || 0) * (activeStore.category === 'travel' ? (bookForm.seats || 1) : 1);
      const payload = { ...bookForm, displayId: id, storeId: activeStore.id, storeName: activeStore.name, serviceName: activeCart.name, totalPrice: price, status: 'pending', timestamp: Date.now(), payment: 'Cash' };
  ...
  const handleVLogin = async () => {
    ...
    try {
      ...
      if (snap.exists() && snap.data().password === vLogin.pass) {
        ...
        setView('merchant');
      } else notify("Invalid ID or Key", "error");
    } catch (e) { notify("Login Error", "error"); }
    setIsProcessing(false);
  };

  if (loading) return <div className="h-screen flex items-center justify-center bg-emerald-600 text-white font-black animate-pulse uppercase tracking-[0.2em]">CHIPLUN CONNECT</div>;
      ...
      <main className="flex-1 -mt-6 px-4 pb-32 z-10">
        ...
        {view === 'home' && (
          <div className="space-y-8 pt-2">
            <div className="grid grid-cols-4 gap-3 animate-in fade-in">
               ...
               <button onClick={() => setSearch('clinic')} className="flex flex-col items-center gap-2"><div className="p-4 bg-white rounded-2xl border border-slate-100 shadow-sm"><Lucide.Stethoscope size={20} className="text-emerald-500"/></div><span className="text-[9px] font-black uppercase text-slate-400">Clinic</span></button>
               <button onClick={() => setSearch('repair')} className="flex flex-col items-center gap-2"><div className="p-4 bg-white rounded-2xl border border-slate-100 shadow-sm"><Lucide.Wrench size={20} className="text-amber-500"/></div><span className="text-[9px] font-black uppercase text-slate-400">Repair</span></button>
            </div>
            <section className="space-y-4">
              <h2 className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic px-1">Live Now</h2>
              <div className="space-y-4">
                {marketplace.map(store => (
                  <div key={store.id} onClick={() => { setActiveStore(store); setView('detail'); setActiveCart(null); setBookForm({ date:'', time:'', phone:'', resId:'', seats:1 }); }} className="bg-white p-3 rounded-[2.5rem] flex gap-4 items-center shadow-sm border border-slate-100 group active:scale-[0.98] transition-all">
                ...
                {marketplace.length === 0 && <p className="text-center py-20 text-slate-300 font-black uppercase text-[10px] tracking-widest opacity-50">Empty Marketplace</p>}
              </div>
            </section>
          </div>
        )}

        {/* VIEW: MERCHANT CONSOLE (STRICT CATEGORY SEPARATION) */}
        {view === 'merchant' && mData && (
          <div className="pt-6 space-y-6 animate-in slide-in-from-bottom-8 px-1">
            ...
            <div className={`p-6 rounded-[2.5rem] flex items-center justify-between shadow-lg transition-all ${mData.store.isLive ? 'bg-emerald-600 text-white' : 'bg-slate-900 text-white'}`}>
              <div><p className="text-[9px] font-black uppercase opacity-70 mb-1">Public Availability</p><p className="text-xl font-black uppercase italic tracking-tighter">{mData.store.isLive ? 'Shop is Open' : 'Shop is Closed'}</p></div>
            ...
            {mTab === 'ledger' && (
               <section className="space-y-4 pb-20">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-white p-6 rounded-[2rem] border border-slate-100 text-center shadow-sm"><p className="text-[8px] font-black text-slate-400 mb-1 tracking-widest uppercase">Revenue</p><p className="text-2xl font-black text-emerald-600 tracking-tighter italic">₹{mData.rev}</p></div>
                    <div className="bg-white p-6 rounded-[2rem] border border-slate-100 text-center shadow-sm"><p className="text-[8px] font-black text-slate-400 mb-1 tracking-widest uppercase">Pending</p><p className="text-2xl font-black text-blue-600 tracking-tighter italic">{mData.queue.length}</p></div>
                  </div>
                  {mData.queue.map((b, i) => (
                    <div key={i} className="bg-white p-5 rounded-[2rem] border-l-8 border-emerald-500 shadow-sm flex justify-between items-center mx-1 animate-in slide-in-from-left-4">
                      <div className="flex-1 pr-4">
                        ...
                        <p className="text-[8px] text-blue-600 font-black uppercase mt-1 tracking-widest">Paid via {b.payment}</p>
                      </div>
                      <div className="flex gap-2">
                        <button onClick={() => window.open(`tel:${b.phone}`)} className="p-3 bg-slate-50 text-slate-400 rounded-xl transition-all active:scale-90"><Lucide.Phone size={18}/></button>
            ...
            {mTab === 'assets' && (
              <div className="bg-white p-8 rounded-[3rem] border border-slate-100 space-y-6 mx-1">
                <button onClick={() => {
                   const n = prompt(mData.store.category === 'salon' ? "Staff Name:" : "Route Description (e.g. Khed-Mumbai 9AM):");
                   const c = mData.store.category === 'travel' ? prompt("Max Capacity (Seats):") : 1;
                ...
                }} className="w-full py-4 border-2 border-dashed border-slate-200 rounded-2xl font-black uppercase text-[10px] text-slate-400 hover:border-emerald-300 hover:text-emerald-500">+ Add {mData.store.category === 'salon' ? 'Professional' : 'Vehicle Slot'}</button>
                <div className="space-y-3">
                   {mData.store.resources?.map((r, i) => (
                     <div key={i} className="flex justify-between items-center p-4 bg-slate-50 rounded-2xl border border-slate-100 shadow-inner">
                        <div><p className="font-black text-xs uppercase italic leading-none">{r.name}</p>{mData.store.category === 'travel' && <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mt-1">Max Capacity: {r.capacity} Seats</p>}</div>
            ...
            {mTab === 'prices' && (
               <div className="bg-white p-8 rounded-[3rem] border border-slate-100 space-y-6 mx-1">
                  ...
                  <button onClick={() => {
                     const n = prompt("Item Label (e.g. Haircut / Mumbai Express):");
                     const p = prompt("Final Fixed Price (₹):");
                     if (n && p) updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'stores', profile.businessId), { services: arrayUnion({ name: n, price: Number(p) }) });
                  }} className="w-full py-4 border-2 border-dashed border-slate-200 rounded-2xl font-black uppercase text-[10px] text-slate-400 hover:border-emerald-300">+ Add Price Point</button>
                  <div className="space-y-3">
        ...
        {view === 'business' && profile.role !== 'vendor' && (
          <div className="pt-6 space-y-6 animate-in slide-in-from-bottom-8 px-1">
            ...
            <div className="flex bg-slate-200 p-1.5 rounded-[1.8rem] shadow-inner border border-slate-300 relative mx-1">
               ...
               <button onClick={() => setHubView('register')} className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest relative z-10 transition-colors ${hubView === 'register' ? 'text-white' : 'text-slate-500'}`}>Join Us</button>
            ...
            {hubView === 'register' ? (
              <div className="bg-white p-8 rounded-[3rem] shadow-xl border border-slate-100 space-y-4 mx-1">
                ...
                <button onClick={() => { setIsProcessing(true); addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'requests'), { ...regForm, status: 'pending', timestamp: Date.now() }).then(() => { notify("Sent for review!"); setView('home'); setIsProcessing(false); }) }} className="w-full bg-emerald-600 text-white py-5 rounded-[1.5rem] font-black uppercase text-[10px] tracking-widest active:scale-[0.97] transition-all shadow-xl">Apply for Verification</button>
                <button onClick={() => setView('admin')} className="w-full text-[8px] font-black text-slate-300 uppercase tracking-widest pt-4">System Master Console</button>
              </div>
            ) : (
          ...
          <div className="pt-10 space-y-6 animate-in fade-in px-2">
             ...
             {!adminAuth ? (
               <div className="bg-white p-8 rounded-[3rem] shadow-2xl border border-rose-100 space-y-4 text-center">
                 ...
                 <button onClick={() => { if (adminPin === ADMIN_PIN) setAdminAuth(true); else notify("Denied", "error"); }} className="w-full bg-rose-600 text-white py-5 rounded-2xl font-black shadow-xl uppercase active:scale-95 transition-all tracking-widest">Verify Identity</button>
               </div>
             ) : (
        ...
        {view === 'detail' && activeStore && (
          <div className="pt-4 space-y-6 animate-in slide-in-from-right-4">
            ...
            <div className="relative mx-1">
              <img src={activeStore.image} className="w-full h-56 rounded-[2.5rem] object-cover shadow-xl shadow-emerald-900/10" />
            ...
            {activeCart && (
              <div className="bg-white p-6 rounded-[3rem] shadow-sm border border-slate-100 animate-in slide-in-from-bottom-6 space-y-5 mx-1">
                  ...
                  <div className="space-y-2">
                    <div className="space-y-2">
                      {activeStore.resources?.map(r => {
                        const { count, left } = getResourceStats(activeStore.id, r.id, r.capacity);
                        ...
                        return (
                          <div key={r.id} onClick={() => !isFull && setBookForm({...bookForm, resId: r.id})} className={`p-4 rounded-2xl border-2 transition-all cursor-pointer flex justify-between items-center ${bookForm.resId === r.id ? 'border-emerald-600 bg-emerald-50' : 'border-slate-50 opacity-60'} ${isFull ? 'opacity-30 cursor-not-allowed' : ''}`}>
                            ...
                            <span className={`text-[8px] font-black uppercase tracking-tighter ${isFull ? 'text-rose-500' : 'text-emerald-600'} leading-none`}>
                              {activeStore.category === 'salon' ? `Waitlist: ${count}` : isFull ? 'Sold Out' : `${left} Seats Left`}
                            </span>
                          </div>
                        );
                      })}
                  ...
                  <button disabled={!bookForm.date || !bookForm.time || !bookForm.phone || !bookForm.resId} onClick={() => setShowPayment(true)} className="w-full bg-emerald-600 text-white py-5 rounded-2xl font-black uppercase text-[11px] tracking-[0.2em] active:scale-[0.97] transition-all shadow-xl shadow-emerald-200 disabled:opacity-40">Proceed to Confirmation</button>
              </div>
            )}
          </div>
        )}
        ...
        {view === 'track' && receipt && (
          <div className="pt-10 space-y-8 animate-in slide-in-from-bottom-8">
            <h2 className="text-xl font-black text-emerald-900 uppercase italic tracking-tighter text-center leading-none">Booking Confirmed</h2>
            <div className="bg-white p-8 rounded-[3.5rem] shadow-2xl border-t-8 border-emerald-500 text-center space-y-6 mx-1">
                ...
                <div className="bg-slate-50 p-5 rounded-3xl text-[9px] font-black uppercase text-left space-y-2 border border-slate-100 shadow-inner">
                  ...
                  <div className="flex justify-between pt-2 border-t border-slate-200 text-xs font-black leading-none italic tracking-widest"><span>Amount:</span><span className="text-emerald-600 text-lg tracking-tighter">₹{receipt.totalPrice}</span></div>
                </div>
        ...
        <div className="fixed inset-0 bg-emerald-600 z-[1001] flex items-center justify-center p-6 animate-in zoom-in-95 duration-200 text-white">
           <div className="text-center space-y-8">
              ...
              <div className="space-y-2">
                 ...
                 <p className="text-sm font-bold uppercase opacity-70 tracking-widest">Confirm to issue Token ID</p>
              </div>
