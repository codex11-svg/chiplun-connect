...
export default function App() {
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, (u) => {
      log("Auth", u ? `UID: ${u.uid}` : "Guest Mode");
      setUser(u);
    });
    return () => unsub();
  }, []);
  ...
  useEffect(() => {
    ...
    const unsubPublic = [
      onSnapshot(collection(db, ...root, 'public', 'data', 'stores'), (s) => setStores(s.docs.map(d => ({ id: d.id, ...d.data() }))), e => log("Err", e.message)),
      onSnapshot(collection(db, ...root, 'public', 'data', 'bookings'), (s) => setBookings(s.docs.map(d => ({ id: d.id, ...d.data() }))), e => log("Err", e.message))
    ];

    let unsubAdmin = [];
    if (isAdmin) {
      unsubAdmin.push(onSnapshot(collection(db, ...root, 'public', 'data', 'requests'), (s) => setRequests(s.docs.map(d => ({ id: d.id, ...d.data() })))));
    }

    const timer = setTimeout(() => setLoading(false), 1000);
    return () => { [...unsubPublic, ...unsubAdmin].forEach(f => f && f()); clearTimeout(timer); };
  }, [isAdmin]);
  ...
  const handleAdminLogin = async () => {
    if (!adminEmail || !adminPass) return notify("Enter details", "error");
    setIsProcessing(true);
    try {
      const res = await signInWithEmailAndPassword(auth, adminEmail, adminPass);
      if (res.user.uid !== MASTER_ADMIN_UID) {
        await signOut(auth);
        notify("Access Denied: Master ID Mismatch", "error");
      } else {
        notify("Vault Decrypted");
        setView('admin');
      }
  ...
  const handleBookingExecution = async () => {
    ...
    try {
      ...
      let fTime = bookForm.time;
      if (selStore.category === 'travel') fTime = selStore.resources?.find(r => r.id === bookForm.resId)?.time || "9:00 AM";
      const payload = { ...bookForm, time: fTime, displayId: id, storeId: selStore.id, storeName: selStore.name, serviceName: selService.name, totalPrice: total, status: 'pending', timestamp: Date.now(), payment: 'Cash' };
      await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'bookings'), payload);
      setActiveReceipt(payload);
      setShowConfirm(false); setShowPay(false); setView('track');
    } catch (e) { notify("Sync Error", "error"); }
    setIsProcessing(false);
  };

  const handleVLogin = async () => {
    if (!vLogin.id || !vLogin.pass) return notify("Missing Creds", "error");
    setIsProcessing(true);
    try {
  ...
  return (
    <div className="max-w-md mx-auto bg-slate-50 min-h-screen flex flex-col shadow-2xl relative font-sans text-slate-900 overflow-x-hidden selection:bg-emerald-100">
      ...
      <header className="bg-emerald-600 text-white p-6 pb-12 rounded-b-[3.5rem] shadow-lg sticky top-0 z-50">
        <div className="flex justify-between items-center mb-6">
          <div onClick={() => setView('home')} className="cursor-pointer active:scale-95 transition-all">
            ...
            <p className="text-[10px] font-bold opacity-80 uppercase tracking-widest mt-1">V50 Pro Supreme</p>
          </div>
          <div className="flex items-center gap-2">
            <button 
              onClick={() => setView('admin')} 
              className={`w-10 h-10 rounded-2xl flex items-center justify-center border transition-all ${view === 'admin' ? 'bg-white text-emerald-600 shadow-inner' : 'bg-white/10 text-white border-white/20'}`}
            >
              <Lucide.Shield size={18} />
            </button>
            <button 
               onClick={() => {
                 if (profile.role === 'vendor' && profile.businessId) setView('merchant');
                 else setView('business');
               }} 
               className={`w-10 h-10 rounded-2xl flex items-center justify-center border transition-all ${view === 'merchant' || view === 'business' ? 'bg-white text-emerald-600 shadow-inner' : 'bg-white/10 text-white border-white/20'}`}
            >
        ...
        {view === 'track' && (
           <div className="pt-6 space-y-6 animate-in slide-in-from-bottom-8 px-1">
              ...
              <div className="bg-white p-6 rounded-[2.5rem] shadow-xl border border-slate-100 space-y-4">
                 ...
                 <button onClick={() => { if(!trackId) notify("Enter Token", "error"); }} className="w-full py-5 bg-emerald-600 text-white rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-lg active:scale-95">Check Status</button>
              </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="bg-white p-2 rounded-[2.5rem] border border-slate-100 shadow-lg relative overflow-hidden h-44 group">
                 ...
                 <label className="absolute inset-0 flex flex-col items-center justify-center cursor-pointer">
                    ...
                    <span className="text-[8px] font-black uppercase mt-1 text-slate-800 px-2 text-center">Update Photo</span>
          ...
          <div className="pt-10 space-y-6 animate-in fade-in px-2">
             ...
             {!isAdmin ? (
               <div className="bg-white p-8 rounded-[3rem] shadow-2xl border border-rose-100 space-y-4 text-center">
                 ...
                 <button onClick={handleAdminLogin} disabled={isProcessing} className="w-full bg-rose-600 text-white py-5 rounded-2xl font-black shadow-xl uppercase active:scale-95 transition-all tracking-[0.2em]">Authorize Login</button>
               </div>
             ) : (
               <div className="space-y-6 pb-20 px-1 animate-in slide-in-from-bottom-8">
                 <div className="flex justify-between items-center bg-rose-50 p-4 rounded-2xl border border-rose-100">
                    ...
                    <button onClick={() => { signOut(auth); setView('home'); }} className="text-[8px] font-black uppercase bg-white border border-rose-200 px-4 py-2 rounded-xl text-rose-600">Sign Out</button>
                 </div>
          ...
          <div className="pt-4 space-y-6 animate-in slide-in-from-right-4 px-1 pb-32">
            ...
            {selService && (
              <div className="bg-white p-6 rounded-[3rem] shadow-sm border border-slate-100 animate-in slide-in-from-bottom-6 space-y-5 mx-1">
                  ...
                  <div className="space-y-2">
                    <label className="text-[8px] font-black text-slate-400 uppercase ml-2 tracking-widest">Select Professional / Trip</label>
                    <div className="space-y-2">
                      {(selStore.resources || []).map(r => {
                        const active = bookings.filter(b => b.storeId === selStore.id && b.resId === r.id && b.status === 'pending');
                        const taken = active.reduce((sum, b) => sum + (Number(b.seats) || 1), 0);
                        const left = (Number(r.capacity) || 0) - taken;
                        ...
                        return (
                          <div key={r.id} onClick={() => !isFull && setBookForm({...bookForm, resId: r.id})} className={`p-4 rounded-2xl border-2 transition-all cursor-pointer flex justify-between items-center ${bookForm.resId === r.id ? 'border-emerald-600 bg-emerald-50' : 'border-slate-50 opacity-60'} ${isFull ? 'opacity-30 cursor-not-allowed grayscale' : ''}`}>
                            ...
                            <span className={`text-[8px] font-black uppercase tracking-tighter ${isFull ? 'text-rose-500' : 'text-emerald-600'}`}>{selStore.category === 'salon' ? `Wait Rank: ${active.length + 1}` : isFull ? 'FULL' : `${left} S Left`}</span>
                          </div>
                        );
                      })}
                    </div>
                  ...
                  <button disabled={!bookForm.date || (!bookForm.time && selStore.category === 'salon') || !bookForm.phone || !bookForm.resId || !bookForm.name} onClick={() => setShowPay(true)} className="w-full bg-emerald-600 text-white py-5 rounded-2xl font-black uppercase text-[11px] tracking-[0.2em] active:scale-[0.97] transition-all shadow-xl">Get Token</button>
              </div>
            )}
          </div>
        )}
