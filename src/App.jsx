...
const appId = "chiplun-pro-final-v17"; 
...
export default function App() {
  ...
  if (loading) return <div className="h-screen flex items-center justify-center bg-emerald-600 text-white font-black animate-pulse text-lg tracking-widest uppercase">ChiplunConnect Pro V17</div>;
    ...
    <div className="max-w-md mx-auto bg-gray-50 min-h-screen flex flex-col shadow-2xl relative font-sans text-gray-900 overflow-x-hidden">
      ...
      {showConfirmModal && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-[500] flex items-center justify-center p-6 animate-in zoom-in-95">
           <div className="bg-white w-full rounded-[3.5rem] p-10 shadow-2xl">
              ...
              <h3 className="text-2xl font-black text-center mb-6 uppercase italic">Verify Booking</h3>
              <div className="bg-gray-50 p-6 rounded-3xl space-y-3 mb-8 text-xs font-black uppercase border border-gray-100 shadow-inner">
              ...
              <button disabled={isProcessing} onClick={handleFinalConfirm} className="w-full bg-emerald-600 text-white py-5 rounded-[2.5rem] font-black uppercase text-sm shadow-xl active:scale-95 transition-all disabled:opacity-50">
      ...
      {deleteTarget && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-md z-[500] flex items-center justify-center p-6">
           <div className="bg-white w-full rounded-[3rem] p-8 shadow-2xl border-4 border-rose-500">
              <Lucide.Trash2 size={48} className="text-rose-500 mx-auto mb-4 animate-bounce" />
              <h3 className="text-2xl font-black text-center uppercase tracking-tighter">Destroy Record?</h3>
              <p className="text-center text-gray-400 text-xs mt-3 mb-8 font-medium">This will wipe <span className="text-rose-600 font-bold underline">"{deleteTarget.name}"</span> and all login access forever.</p>
      ...
      <header className="bg-emerald-600 text-white p-6 pb-12 rounded-b-[4rem] shadow-lg sticky top-0 z-50">
        <div className="flex justify-between items-center mb-6">
          <div onClick={() => setView('home')} className="cursor-pointer active:scale-95 transition-all">
            ...
            <p className="text-[9px] font-bold opacity-70 uppercase tracking-widest">Enterprise Edition • MH-08</p>
          </div>
        ...
        {view === 'home' && (
          <div className="relative animate-in slide-in-from-top-2">
            ...
            <input type="text" placeholder="Search Chiplun marketplace..." className="w-full bg-white/10 border border-white/20 rounded-2xl py-3.5 pl-12 pr-4 text-white placeholder-emerald-200 outline-none focus:bg-white/20" />
          </div>
        )}
      </header>
      ...
      <main className="flex-1 -mt-6 px-4 pb-32 overflow-y-auto z-10">
        ...
        {/* VIEW: STORE DETAIL (Strict Category Fields) */}
             ...
             <section className="bg-white p-6 rounded-[3rem] shadow-sm border border-gray-100">
               ...
               {selectedStore.category === 'travel' && (
                  <div className="space-y-4 mb-8 p-6 bg-blue-50/50 rounded-[2.5rem] border border-blue-100 animate-in fade-in">
                    <p className="text-[10px] font-black text-blue-600 uppercase tracking-widest text-center">Travel Logistics</p>
                    <div className="grid grid-cols-2 gap-3">
                       <input placeholder="Source" onChange={e => setBookingMeta({...bookingMeta, source: e.target.value})} className="bg-white p-3 rounded-2xl text-xs font-bold outline-none border border-blue-100 uppercase" />
                       <input placeholder="Destination" onChange={e => setBookingMeta({...bookingMeta, destination: e.target.value})} className="bg-white p-3 rounded-2xl text-xs font-bold outline-none border border-blue-100 uppercase" />
                    </div>
                    <select onChange={e => setBookingMeta({...bookingMeta, tripType: e.target.value})} className="w-full bg-white p-3 rounded-2xl text-xs font-bold border border-blue-100 uppercase"><option>One-Way</option><option>Round-Trip</option></select>
                  </div>
               )}

               {selectedStore.category === 'clinic' && (
                  <div className="space-y-4 mb-8 p-6 bg-emerald-50/50 rounded-[2.5rem] border border-emerald-100">
                    <p className="text-[10px] font-black text-emerald-600 uppercase tracking-widest text-center">Medical Intake</p>
                    <div className="flex gap-2">
                       <input placeholder="Patient Age" type="number" onChange={e => setBookingMeta({...bookingMeta, patientAge: e.target.value})} className="flex-1 bg-white p-3 rounded-2xl text-xs font-bold outline-none border border-emerald-100" />
                       <select onChange={e => setBookingMeta({...bookingMeta, consultType: e.target.value})} className="flex-1 bg-white p-3 rounded-2xl text-xs font-bold border border-emerald-100"><option>General</option><option>Specialist</option><option>Emergency</option></select>
                    </div>
                  </div>
               )}
        ...
        {/* VIEW: TRACK TOKEN (Database Logic Fix) */}
        {view === 'bookings' && (
              ...
              {searchedBooking ? (
                <div className="bg-emerald-600 text-white p-8 rounded-[3.5rem] shadow-2xl animate-in zoom-in-95 relative overflow-hidden">
                   ...
                   <div className="flex justify-between items-start mb-8 relative z-10">
                      <div><p className="text-[10px] font-black uppercase opacity-60">Verified Partner</p><h3 className="text-xl font-black italic uppercase tracking-tight">{searchedBooking.storeName}</h3></div>
                   ...
                   <button onClick={() => setSearchedBooking(null)} className="w-full mt-10 py-3 text-[10px] font-black uppercase bg-white/10 rounded-2xl active:scale-95">New Search</button>
                </div>
              ) : (
        ...
        {/* VIEW: MERCHANT HUB (Fixed Sessions) */}
        {view === 'dashboard' && (
              ...
              {!isSessionVerified ? (
                 <div className="animate-in zoom-in-95 duration-500">
                    <div className="text-center mb-10">
                       ...
                       <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">Authorize Secure Session</p>
                    </div>
                    <div className="bg-white p-8 rounded-[3.5rem] shadow-2xl space-y-4 border border-gray-100">
                       ...
                       <button onClick={handleVendorLogin} className="w-full bg-emerald-600 text-white py-5 rounded-[2rem] font-black shadow-xl uppercase active:scale-95 transition-all">Authorize Access</button>
                    </div>
                 </div>
              ) : (
                <div className="space-y-6 animate-in slide-in-from-bottom-8">
                   ...
                   <section className="bg-white p-7 rounded-[3rem] shadow-sm border border-gray-100">
                      <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-6 font-black">Manage Menu</h3>
                      ...
                      <div className="space-y-3 pt-6 border-t border-gray-100">
                        ...
                        }} className="w-full bg-emerald-600 text-white py-3 rounded-xl font-black uppercase text-[10px]">Publish to Menu</button>
                      </div>
                   </section>
        ...
        {view === 'admin_auth' && (
           <div className="pt-20 text-center animate-in zoom-in-95">
              ...
              <button onClick={() => { if(adminPinInput===ADMIN_PIN) setView('admin_panel'); else { alert("DENIED"); setAdminPinInput(''); } }} className="w-full bg-emerald-600 text-white py-5 rounded-[2rem] font-black shadow-2xl active:scale-95 uppercase">Access Panel</button>
           </div>
        )}
        ...
        {view === 'confirmation' && (
           <div className="text-center pt-10 animate-in zoom-in-90 duration-700 pb-10 px-4">
             ...
             <h2 className="text-4xl font-black text-emerald-900 mb-6 uppercase tracking-tighter">Reserved</h2>
             ...
             <button onClick={() => setView('home')} className="w-full bg-emerald-700 text-white py-5 rounded-[2.5rem] font-black shadow-2xl active:scale-95 uppercase tracking-widest mt-10">Return Home</button>
           </div>
        )}
      ...
      <nav className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-white/90 backdrop-blur-3xl border-t border-gray-100 px-8 py-5 pb-9 flex justify-between items-center z-50 rounded-t-[3.5rem] shadow-2xl">
        ...
        <button onClick={() => { setView('bookings'); setSearchedBooking(null); }} className={`flex flex-col items-center gap-1.5 transition-all ${view === 'bookings' ? 'text-emerald-600 scale-110' : 'text-gray-300'}`}><Lucide.Search size={22} /><span className="text-[9px] font-black uppercase tracking-widest">Track</span></button>
        <button onClick={() => setView('dashboard')} className={`flex flex-col items-center gap-1.5 transition-all ${['merchant_login', 'dashboard'].includes(view) ? 'text-emerald-600 scale-110' : 'text-gray-300'}`}><Lucide.UserCircle size={22} /><span className="text-[9px] font-black uppercase">Account</span></button>
      </nav>
