...
const appId = "chiplun-pro-v24-stable"; 
...
export default function App() {
  ...
  // --- Schedule Logic ---
  const isTimeInLunch = (timeStr, startStr, endStr) => {
    ...
    return target >= toMins(startStr) && target < toMins(endStr);
  };
  ...
  const handleFinalConfirm = async () => {
    ...
    try {
      ...
      setQueueInfo({ pos, delay: (pos - 1) * 30 });
  ...
  if (loading) return <div className="h-screen flex items-center justify-center bg-emerald-600 text-white font-black animate-pulse">CHIPLUN CONNECT PRO V24</div>;
  ...
  return (
    <div className="max-w-md mx-auto bg-gray-50 min-h-screen flex flex-col shadow-2xl relative font-sans text-gray-900 overflow-x-hidden">
      ...
      {showConfirmModal && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-[500] flex items-center justify-center p-6 animate-in zoom-in-95">
           <div className="bg-white w-full rounded-[3.5rem] p-10 shadow-2xl border-t-8 border-emerald-500">
              ...
              <h3 className="text-xl font-black text-center mb-6 uppercase tracking-tighter italic">Review Appointment</h3>
              ...
              <button disabled={isProcessing} onClick={handleFinalConfirm} className="w-full bg-emerald-600 text-white py-5 rounded-[2.5rem] font-black uppercase text-xs shadow-xl active:scale-95 transition-all">
                {isProcessing ? 'Verifying...' : 'Book My Slot'}
              </button>
              <button onClick={() => setShowConfirmModal(false)} className="w-full py-4 text-gray-400 font-black text-[10px] uppercase text-center w-full">Cancel</button>
           </div>
        </div>
      )}
      ...
      {deleteTarget && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-md z-[600] flex items-center justify-center p-6">
           <div className="bg-white w-full rounded-[3rem] p-10 shadow-2xl text-center border-4 border-rose-500">
              ...
              <h3 className="text-xl font-black text-center uppercase tracking-tighter">Destroy Business?</h3>
              <p className="text-gray-400 text-xs mt-3 mb-10 leading-relaxed font-medium">This will wipe <span className="text-rose-600 font-bold underline">"{deleteTarget.name}"</span> and all login access forever.</p>
      ...
      <header className="bg-emerald-600 text-white p-6 pb-12 rounded-b-[4rem] shadow-lg sticky top-0 z-50">
        <div className="flex justify-between items-center mb-6">
          <div onClick={() => setView('home')} className="cursor-pointer active:scale-95 transition-all">
            ...
            <p className="text-[9px] font-bold opacity-70 uppercase tracking-widest italic tracking-tighter">Enterprise Suite MH-08</p>
          </div>
          <button onClick={() => setView('admin')} className="w-10 h-10 bg-white/10 rounded-2xl flex items-center justify-center border border-white/5 active:bg-white/30 transition-all shadow-inner"><Lucide.ShieldCheck size={20}/></button>
        </div>
        {view === 'home' && (
      ...
      <main className="flex-1 -mt-6 px-4 pb-32 overflow-y-auto z-10">
        ...
        {view === 'home' && (
          <div className="space-y-8 pt-2">
             ...
             <section className="animate-in slide-in-from-bottom-4">
               <h2 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-4 ml-1 italic font-black">Live in Chiplun</h2>
               <div className="space-y-4">
                 {stores.map(store => (
                   <div key={store.id} onClick={() => { setSelectedStore(store); setView('detail'); setCart([]); }} className="bg-white p-3 rounded-[2.5rem] flex gap-4 items-center shadow-sm border border-gray-100 active:scale-[0.98] transition-all group">
                     ...
                     <div className="flex-1">
                       <div className="flex items-center gap-1.5 mb-1">
                          <span className={`text-[7px] font-black uppercase px-2 py-0.5 rounded-full inline-block ${store.category === 'salon' ? 'bg-rose-50 text-rose-600' : 'bg-emerald-50 text-emerald-600'}`}>{store.category}</span>
                          {store.isLive ? <span className="text-[7px] font-black uppercase bg-emerald-500 text-white px-2 py-0.5 rounded-full">Open</span> : <span className="text-[7px] font-black uppercase bg-rose-500 text-white px-2 py-0.5 rounded-full">Offline</span>}
                       </div>
        ...
        {view === 'detail' && selectedStore && (
           <div className="pt-4 space-y-6 animate-in slide-in-from-right-4 pb-10">
             ...
             <section className="bg-white p-6 rounded-[3rem] shadow-sm border border-gray-100">
               ...
               <p className="text-gray-400 text-xs mb-8 italic font-medium leading-none"><Lucide.MapPin size={10} className="inline mr-1"/> {selectedStore.address}</p>
             ...
             {cart.length > 0 && (
               <div className="bg-white p-6 rounded-[3rem] shadow-sm border border-gray-100 animate-in slide-in-from-bottom-4">
                 ...
                 <div className="space-y-4">
                   {!selectedStore.isLive && <div className="p-4 bg-rose-50 border border-rose-100 text-rose-600 rounded-2xl text-center font-black text-[10px] uppercase">This Store is currently taking a break.</div>}
                   ...
                   <input placeholder="WhatsApp Number" type="tel" value={bookingMeta.custPhone} onChange={e => setBookingMeta({...bookingMeta, custPhone: e.target.value})} className="w-full bg-gray-50 p-4 rounded-2xl font-bold text-sm outline-none border border-gray-100 uppercase" />
                   
                   <button 
                     disabled={!selectedStore.isLive || !bookingMeta.date || !bookingMeta.time}
                     onClick={() => {
                        if(isTimeInLunch(bookingMeta.time, selectedStore.lunchStart, selectedStore.lunchEnd)) {
                           alert(`Shop is on Lunch Break between ${selectedStore.lunchStart} and ${selectedStore.lunchEnd}. Please pick a different time!`);
                        } else {
                   ...
                   >
                     {selectedStore.isLive ? 'Review & Book' : 'Shop is Offline'}
                   </button>
                 </div>
              </div>
            )}
        ...
        {/* VIEW: MERCHANT HUB (Account Portal) */}
        {view === 'account' && (
           <div className="pt-4 space-y-6 px-4 pb-10 animate-in slide-in-from-bottom-4">
              {!isSessionVerified ? (
                 <div className="animate-in zoom-in-95 pt-20">
                    ...
                    <div className="bg-white p-8 rounded-[3.5rem] shadow-2xl space-y-4 border border-gray-100">
                       ...
                       <button onClick={handleVendorLogin} className="w-full bg-emerald-600 text-white py-5 rounded-[2rem] font-black shadow-xl uppercase active:scale-95 transition-all tracking-widest">Authorize Session</button>
                    </div>
                 </div>
              ) : (
                <div className="space-y-6 animate-in slide-in-from-bottom-8">
                   ...
                   <section className="bg-emerald-600 text-white p-7 rounded-[3rem] shadow-xl space-y-4 border border-black/5">
                      <h3 className="text-[10px] font-black uppercase tracking-widest flex items-center opacity-80 font-black italic"><Lucide.Calendar size={14} className="mr-2"/> Incoming Queue</h3>
                      <div className="space-y-3 max-h-72 overflow-y-auto pr-1">
                        {merchantBookings.length > 0 ? merchantBookings.sort((a,b) => b.timestamp - a.timestamp).map((b, i) => (
                           <div key={i} className="bg-white/10 p-4 rounded-2xl border border-white/5 space-y-1 animate-in fade-in">
                              ...
                              <p className="text-[8px] font-black text-emerald-100 uppercase tracking-tighter opacity-80">Service: {b.services?.map(s => s.name).join(', ')}</p>
                              <p className="text-[9px] opacity-70 uppercase font-bold tracking-widest block pt-1">{b.date} • {b.time} • {b.resourceName || 'Assigned'}</p>
                              <div className="pt-2 flex justify-between items-center text-[10px] font-black uppercase border-t border-white/5">
                   ...
                   {/* MERCHANT CONTROLS: LIVE/OFFLINE & LUNCH HOUR */}
                   <section className="bg-white p-7 rounded-[3rem] shadow-sm border border-gray-100 space-y-6">
                      <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest flex items-center font-black italic"><Lucide.Clock size={14} className="mr-2 text-emerald-600"/> Operations Control</h3>
                      
                      <div className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl border">
                        <div><p className="font-black text-xs uppercase">Store Visibility</p><p className="text-[8px] font-bold text-gray-400 uppercase">Users can only book when Live</p></div>
                        <button 
                           onClick={() => updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'stores', profile.businessId), { isLive: !myStore?.isLive })}
                           className={`px-6 py-2 rounded-xl text-[9px] font-black uppercase shadow-lg transition-all ${myStore?.isLive ? 'bg-emerald-500 text-white' : 'bg-rose-500 text-white'}`}
                        >
                           {myStore?.isLive ? 'LIVE' : 'OFFLINE'}
                        </button>
                      </div>

                      <div className="p-4 bg-gray-50 rounded-2xl border space-y-4">
                        <p className="font-black text-xs uppercase text-center mb-2">Set Daily Lunch Break</p>
                        <div className="grid grid-cols-2 gap-4">
                           <div className="space-y-1"><p className="text-[8px] font-black uppercase text-gray-400 ml-1">Break Start</p><input type="time" value={editForm.lunchStart} onChange={e => setEditForm({...editForm, lunchStart: e.target.value})} className="w-full bg-white p-3 rounded-xl text-xs font-black border" /></div>
                           <div className="space-y-1"><p className="text-[8px] font-black uppercase text-gray-400 ml-1">Break End</p><input type="time" value={editForm.lunchEnd} onChange={e => setEditForm({...editForm, lunchEnd: e.target.value})} className="w-full bg-white p-3 rounded-xl text-xs font-black border" /></div>
                        </div>
                        <button onClick={async () => {
                           await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'stores', profile.businessId), { lunchStart: editForm.lunchStart, lunchEnd: editForm.lunchEnd });
                           alert("Schedule Updated!");
                        }} className="w-full bg-emerald-600 text-white py-3 rounded-xl font-black uppercase text-[10px] shadow-lg">Save Break Schedule</button>
                      </div>
                   </section>

                   <section className="bg-white p-7 rounded-[3rem] shadow-sm border border-gray-100 space-y-6 animate-in fade-in">
                      <div className="flex justify-between items-center"><h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest flex items-center font-black italic"><Lucide.Image size={14} className="mr-2 text-emerald-600"/> Branding</h3>{editForm.image && <img src={editForm.image} className="w-10 h-10 rounded-xl object-cover" />}</div>
                      <input placeholder="Image URL" value={editForm.image} onChange={e => setEditForm({...editForm, image: e.target.value})} className="w-full bg-gray-50 p-4 rounded-xl font-bold text-xs outline-none shadow-inner uppercase border" />
                      <input placeholder="Display Name" value={editForm.name} onChange={e => setEditForm({...editForm, name: e.target.value})} className="w-full bg-gray-50 p-4 rounded-xl font-bold text-xs outline-none shadow-inner uppercase border" />
                      <button onClick={async () => { await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'stores', profile.businessId), { image: editForm.image, name: editForm.name }); alert("Appearance Synced!"); }} className="w-full bg-emerald-600 text-white py-3 rounded-xl font-black uppercase text-[10px] shadow-lg">Save Appearance</button>
                   </section>
                </div>
              )}
           </div>
        ...
        {/* ... (Business Portal, Track, Admin kept solid) ... */}
        {view === 'onboarding' && (
           <div className="pt-4 space-y-6 animate-in slide-in-from-bottom-4 px-4 pb-20">
              <div className="text-center mb-10">
                 ...
                 <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest text-center">MH-08 Verification Intake</p>
              </div>
              <div className="bg-white p-7 rounded-[3.5rem] shadow-xl space-y-4 border border-gray-100 text-left">
                 <input placeholder="Business Title" value={regForm.bizName} onChange={e => setRegForm({...regForm, bizName: e.target.value})} className="w-full bg-gray-50 p-4 rounded-xl font-bold text-sm outline-none border border-transparent focus:border-emerald-200 uppercase" />
                 <input placeholder="Area / Landmark" value={regForm.addr} onChange={e => setRegForm({...regForm, addr: e.target.value})} className="w-full bg-gray-50 p-4 rounded-xl font-bold text-sm outline-none border border-transparent focus:border-emerald-200 uppercase" />
        ...
        {view === 'track' && (
           <div className="pt-4 space-y-6 px-4 animate-in slide-in-from-bottom-4 pb-20 text-center">
              ...
              {searchedBooking && (
                <div className="bg-emerald-600 text-white p-8 rounded-[3.5rem] shadow-2xl animate-in zoom-in-95 relative overflow-hidden text-left">
                   <div className="absolute top-0 right-0 p-4 opacity-10"><Lucide.CheckCircle size={80}/></div>
                   <h3 className="text-xl font-black italic uppercase tracking-tight">{searchedBooking.storeName}</h3>
                   <span className="bg-white/20 px-3 py-1 rounded-xl text-[10px] font-black tracking-widest inline-block mt-2 mb-8">{searchedBooking.displayId}</span>
                   <div className="space-y-4 text-sm font-bold border-t border-white/10 pt-6">
                      <p className="opacity-60 text-[8px] uppercase">Queue Pos: #{searchedBooking.queuePos || '1'}</p>
                      <p className="opacity-60 text-[8px] uppercase">Wait: +{searchedBooking.estWait}m</p>
                      <p className="bg-white/10 p-4 rounded-3xl">Scheduled: {searchedBooking.date} • {searchedBooking.time}</p>
                   </div>
        ...
        {view === 'admin' && (
           <div className="pt-20 text-center animate-in zoom-in-95 px-4">
              ...
              <button onClick={() => { if(adminPinInput===ADMIN_PIN) setView('admin_panel'); else setAdminPinInput(''); }} className="w-full bg-emerald-600 text-white py-5 rounded-[2rem] font-black shadow-2xl active:scale-95 uppercase">Unlock</button>
           </div>
        )}
          ...
          <div className="pt-4 space-y-6 pb-20 px-4">
            ...
            {stores.map(s => (
                <div key={s.id} className="bg-white p-5 rounded-[2.5rem] border border-gray-100 flex items-center gap-4 animate-in fade-in shadow-sm">
                   <div className="w-12 h-12 bg-emerald-50 rounded-2xl flex items-center justify-center font-black text-emerald-600 uppercase text-xl">{s.name[0]}</div>
           ...
           <div className="text-center pt-10 animate-in zoom-in-90 duration-700 pb-10 px-4">
             ...
             <div className="bg-emerald-600 text-white p-8 rounded-[3.5rem] shadow-2xl space-y-6 relative overflow-hidden text-center shadow-emerald-200">
                ...
                <div className="grid grid-cols-2 gap-4 border-t border-white/10 pt-6 font-black uppercase">
                   <div className="text-center border-r border-white/10"><p className="text-[8px] opacity-60">Position</p><p className="text-2xl">#{queueInfo.pos}</p></div>
                   <div className="text-center"><p className="text-[8px] opacity-60">Estimated</p><p className="text-2xl">+{queueInfo.delay}m</p></div>
                </div>
             </div>
      ...
      <nav className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-white/95 backdrop-blur-3xl border-t border-gray-100 px-8 py-5 pb-9 flex justify-between items-center z-[100] rounded-t-[3.5rem] shadow-2xl">
        <button onClick={() => setView('home')} className={`flex flex-col items-center gap-1.5 transition-all ${view === 'home' ? 'text-emerald-600 scale-110' : 'text-gray-300'}`}><Lucide.Home size={22} /><span className="text-[9px] font-black uppercase tracking-widest italic">Explore</span></button>
        <button onClick={() => setView('onboarding')} className={`flex flex-col items-center gap-1.5 transition-all ${view === 'onboarding' ? 'text-emerald-600 scale-110' : 'text-gray-300'}`}><Lucide.Building2 size={22} /><span className="text-[9px] font-black uppercase tracking-widest italic">Business</span></button>
        <button onClick={() => { setView('track'); setSearchedBooking(null); }} className={`flex flex-col items-center gap-1.5 transition-all ${view === 'track' ? 'text-emerald-600 scale-110' : 'text-gray-300'}`}><Lucide.Calendar size={22} /><span className="text-[9px] font-black uppercase tracking-widest italic">Track</span></button>
        <button onClick={() => setView('account')} className={`flex flex-col items-center gap-1.5 transition-all ${['account', 'merchant_hub'].includes(view) ? 'text-emerald-600 scale-110' : 'text-gray-300'}`}><Lucide.UserCircle size={22} /><span className="text-[9px] font-black uppercase tracking-widest italic">Account</span></button>
      </nav>
