...
const appId = typeof __app_id !== 'undefined' ? __app_id : 'chiplun-supreme-v50-lite'; 
...
export default function App() {
  ...
  const handleBooking = async () => {
    ...
    try {
      ...
      setTracked(payload);
      setView('track');
    } catch (e) { toast("Booking Failed", "error"); }
  ...
  return (
    <div className="max-w-md mx-auto bg-slate-50 min-h-screen flex flex-col shadow-2xl relative font-sans text-slate-900 selection:bg-emerald-100 overflow-x-hidden">
      ...
      <header className="bg-emerald-600 text-white p-6 pb-12 rounded-b-[3.5rem] shadow-lg sticky top-0 z-50">
        <div className="flex justify-between items-center mb-6">
          <div onClick={() => setView('home')} className="cursor-pointer active:scale-95 transition-all">
            ...
            <p className="text-[10px] font-bold opacity-80 uppercase tracking-widest mt-1">Production V50 Lite</p>
          </div>
          <button 
        ...
        {view === 'home' && (
          <div className="relative animate-in slide-in-from-top-4 duration-500">
            ...
            <input type="text" placeholder="Search salons or travels..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-white/10 border border-white/20 rounded-2xl py-4 pl-12 pr-4 text-white placeholder-emerald-200 outline-none focus:bg-white/20" />
          </div>
        )}
      </header>
      ...
      <main className="flex-1 -mt-6 px-4 pb-32 z-10">
            ...
            <div className="grid grid-cols-4 gap-3 animate-in fade-in duration-700">
               {CATEGORIES.map(c => (
                 <button key={c.id} onClick={() => setSearchQuery(c.id === searchQuery ? '' : c.id)} className={`flex flex-col items-center gap-2 p-1 transition-all ${searchQuery === c.id ? 'scale-110 opacity-100' : 'opacity-60'}`}>
                   <div className={`p-4 rounded-2xl border shadow-sm ${searchQuery === c.id ? 'bg-emerald-600 text-white' : 'bg-white'}`}>{c.i}</div>
                   <span className="text-[9px] font-black uppercase text-slate-500">{c.n}</span>
                 </button>
               ))}
            </div>
            <section className="space-y-4">
              <h2 className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic px-1">Active Now</h2>
                ...
                {liveStores.map(store => (
                  <div key={store.id} onClick={() => { setSelectedStore(store); setView('detail'); setCart(null); }} className="bg-white p-3 rounded-[2.5rem] flex gap-4 items-center shadow-sm border border-slate-100 active:scale-[0.98] transition-all">
                    ...
                    <div className="flex-1">
                      ...
                      <h3 className="font-bold text-slate-800 text-sm leading-tight mt-1">{store.name}</h3>
        ...
        {/* VIEW: MERCHANT LITE (FIXED PRICING LOGIC) */}
        {view === 'merchant' && mData && (
          <div className="pt-6 space-y-6 animate-in slide-in-from-bottom-8">
            ...
            <div className={`p-6 rounded-[2.5rem] flex items-center justify-between shadow-lg transition-all ${mData.store.isLive ? 'bg-emerald-600 text-white' : 'bg-slate-900 text-white'}`}>
              <div><p className="text-[9px] font-black uppercase opacity-70">Visibility</p><p className="text-xl font-black uppercase italic tracking-tighter">{mData.store.isLive ? 'Shop Online' : 'Shop Offline'}</p></div>
            ...
            <div className="flex bg-white p-1 rounded-2xl border border-slate-100 shadow-sm">
                {['ledger', 'assets', 'menu'].map(t => (
                    <button key={t} onClick={() => setMerchantTab(t)} className={`flex-1 py-3 px-2 rounded-xl text-[9px] font-black uppercase tracking-widest ${merchantTab === t ? 'bg-emerald-50 text-emerald-600' : 'text-slate-400'}`}>{t}</button>
                ))}
            </div>
            ...
            {merchantTab === 'ledger' && (
               <section className="space-y-4 pb-20">
                  <div className="grid grid-cols-2 gap-4 px-1">
                    ...
                    <div className="bg-white p-6 rounded-[2rem] border border-slate-100 text-center"><p className="text-[8px] font-black text-slate-400 mb-1">PENDING</p><p className="text-2xl font-black text-blue-600">{mData.pending.length}</p></div>
                  </div>
            ...
            {merchantTab === 'assets' && (
              <div className="bg-white p-8 rounded-[3rem] border border-slate-100 space-y-6 mx-1 animate-in fade-in">
                <button onClick={() => {
                   ...
                   const c = mData.store.category === 'travel' ? prompt("Total Seats:") : 1;
                ...
                }} className="w-full py-4 border-2 border-dashed border-slate-200 rounded-2xl font-black uppercase text-[10px] text-slate-400 hover:border-emerald-500">+ Add {mData.store.category === 'salon' ? 'Expert' : 'Route'}</button>
                <div className="space-y-3">
                   {mData.store.resources?.map((r, i) => (
                     <div key={i} className="flex justify-between items-center p-4 bg-slate-50 rounded-2xl border border-slate-100">
                        <div><p className="font-black text-xs uppercase italic">{r.name}</p>{mData.store.category === 'travel' && <p className="text-[8px] font-black text-slate-400 uppercase mt-1">Capacity: {r.capacity} Seats</p>}</div>
            ...
            {merchantTab === 'menu' && (
               <div className="bg-white p-8 rounded-[3rem] border border-slate-100 space-y-6 mx-1 animate-in fade-in">
                  ...
                  <button onClick={() => {
                     const n = prompt("Service (e.g. Khed to Mumbai):");
                  ...
                  }} className="w-full py-4 border-2 border-dashed border-slate-200 rounded-2xl font-black uppercase text-[10px] text-slate-400 hover:border-emerald-300">+ Add Service Item</button>
                  <div className="space-y-3">
          ...
          <div className="pt-6 space-y-6 animate-in slide-in-from-bottom-8 px-1">
            ...
            {bizSubView === 'register' ? (
              ...
              <div className="bg-white p-8 rounded-[3rem] shadow-xl border border-slate-100 space-y-4 text-center">
                ...
                <button onClick={handleVLogin} className="w-full bg-emerald-600 text-white py-5 rounded-[1.5rem] font-black uppercase text-[10px] tracking-widest shadow-xl active:scale-95 transition-all">Unlock Dashboard</button>
              </div>
            )}
          </div>
        )}
          ...
          <div className="pt-10 space-y-6 animate-in fade-in px-2">
             ...
             {!adminAuth ? (
               <div className="bg-white p-8 rounded-[3rem] shadow-2xl border border-rose-100 space-y-4 text-center">
                 ...
                 <button onClick={() => { if (adminPin === ADMIN_PIN) setAdminAuth(true); else toast("Denied", "error"); }} className="w-full bg-rose-600 text-white py-5 rounded-2xl font-black shadow-xl uppercase active:scale-95 transition-all tracking-widest">Verify Admin Access</button>
               </div>
             ) : (
                 ...
                 {adminTab === 'requests' ? requests.map(r => (
                    <div key={r.id} className="bg-white p-6 rounded-[2.5rem] border border-slate-100 space-y-4 shadow-sm animate-in slide-in-from-bottom-4">
                        ...
                        <div className="flex gap-2">
                          ...
                          <button onClick={() => adminApprove(r)} className="flex-[2] py-3 bg-emerald-600 text-white rounded-2xl font-black text-[9px] uppercase shadow-lg active:scale-95">Manual Approve</button>
                        </div>
                    </div>
                 )) : stores.map(s => (
          ...
          <div className="pt-4 space-y-6 animate-in slide-in-from-right-4">
            ...
            <section className="bg-white p-6 rounded-[3rem] border border-slate-100 shadow-sm space-y-4 mx-1">
              ...
              <div className="space-y-2">
                ...
                )) : <p className="text-center text-[9px] text-slate-300 font-black uppercase py-4">No pricing listed yet</p>}
              </div>
            </section>
            ...
            {cart && (
              <div className="bg-white p-6 rounded-[3rem] shadow-sm border border-slate-100 animate-in slide-in-from-bottom-6 space-y-5 mx-1">
                  <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-center italic leading-none">2. Assign Professional/Slot</h3>
                        ...
                        return (
                          <div key={r.id} onClick={() => !isFull && setBookForm({...bookForm, resId: r.id})} className={`p-4 rounded-2xl border-2 transition-all cursor-pointer flex justify-between items-center ${bookForm.resId === r.id ? 'border-emerald-600 bg-emerald-50' : 'border-slate-50 opacity-60'} ${isFull ? 'opacity-30 cursor-not-allowed' : ''}`}>
                            ...
                            <span className={`text-[8px] font-black uppercase tracking-tighter ${isFull ? 'text-rose-500' : 'text-emerald-600'} leading-none`}>
                              {selectedStore.category === 'salon' ? `Queue: ${count} (~${count * 20}m)` : isFull ? 'Sold Out' : `${left} Seats Left`}
                            </span>
                          </div>
                        );
                      })}
                  ...
                  {selectedStore.category === 'travel' && bookForm.resId && (
                    <div className="space-y-1">
                       <label className="text-[8px] font-black text-slate-400 uppercase ml-2 tracking-widest">Number of Passengers</label>
                  ...
                  <div className="flex gap-2">
                    <input type="date" onChange={e => setBookForm({...bookForm, date: e.target.value})} className="flex-1 bg-slate-50 p-4 rounded-xl font-black text-[10px] border border-slate-100" />
                    <input type="time" onChange={e => setBookForm({...bookForm, time: e.target.value})} className="w-28 bg-slate-50 p-4 rounded-xl font-black text-[10px] border border-slate-100" />
                  </div>
                  <input placeholder="WhatsApp Number" value={bookForm.phone} onChange={e => setBookForm({...bookForm, phone: e.target.value})} className="w-full bg-slate-50 p-4 rounded-xl font-black text-[10px] border border-slate-100 uppercase tracking-widest" />
                  <button disabled={!bookForm.date || !bookForm.time || !bookForm.phone || !bookForm.resId} onClick={handleBooking} className="w-full bg-emerald-600 text-white py-5 rounded-2xl font-black uppercase text-[11px] tracking-[0.2em] active:scale-[0.97] transition-all shadow-xl shadow-emerald-200 disabled:opacity-40">Issue Booking Ticket</button>
              </div>
            )}
          </div>
        )}
          ...
          <div className="pt-10 space-y-8 animate-in slide-in-from-bottom-8">
            ...
            <div className="bg-white p-8 rounded-[3.5rem] shadow-2xl border-t-8 border-emerald-500 text-center space-y-6 mx-1">
                ...
                <div className="bg-slate-50 p-5 rounded-3xl text-[9px] font-black uppercase text-left space-y-2 border border-slate-100 shadow-inner">
                  ...
                  <div className="flex justify-between leading-none italic tracking-widest"><span>Selection:</span><span>{tracked.serviceName}</span></div>
