...
const appId = typeof __app_id !== 'undefined' ? __app_id : 'chiplun-ultimate-v50';
...
export default function App() {
  ...
  const mData = useMemo(() => {
    ...
    if (!s) return null;
    const b = bookings.filter(x => x.storeId === profile.businessId && x.status === 'pending').sort((a,b) => a.timestamp - b.timestamp);
    const revenue = bookings.filter(x => x.storeId === profile.businessId && x.status === 'completed').reduce((a, c) => a + (Number(c.totalPrice) || 0), 0);
    return { store: s, queue: b, rev: revenue };
  }, [bookings, profile.businessId, stores]);

  const getSlotStats = (sId, rId, capacity) => {
    ...
    return { count: active.length, left: (Number(capacity) || 0) - taken };
  };

  const liveTrackStatus = useMemo(() => {
    if (!foundTicket) return null;
    const live = bookings.find(b => b.displayId === foundTicket.displayId);
    if (!live || live.status !== 'pending') return { error: 'Booking not active' };
  ...
  const handleBookingConfirm = async () => {
    ...
    try {
      ...
      const clockTime = now.getHours().toString().padStart(2, '0') + ":" + now.getMinutes().toString().padStart(2, '0');
      if (bookForm.date === today && bookForm.time < clockTime && selStore.category === 'salon') {
      ...
      const total = Number(selService.price) * (selStore.category === 'travel' ? (bookForm.seats || 1) : 1);
  ...
  const handleImage = async (e) => {
    ...
    if (!file || file.size > 900000) return notify("Image too large (Max 900kb)", "error");
      ...
      <header className="bg-emerald-600 text-white p-6 pb-12 rounded-b-[3.5rem] shadow-lg sticky top-0 z-50">
        <div className="flex justify-between items-center mb-6">
          <div onClick={() => setView('home')} className="cursor-pointer active:scale-95 transition-all">
            ...
            <p className="text-[10px] font-bold opacity-80 uppercase tracking-widest mt-1 italic">V50 Ultimate Pro</p>
          </div>
          <button 
        ...
        {view === 'home' && (
          <div className="space-y-8 pt-2">
            ...
            <section className="space-y-4">
              <h2 className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic px-1">Active chiplun Partners</h2>
        ...
        {view === 'track' && (
           <div className="pt-6 space-y-6 animate-in slide-in-from-bottom-8 px-1">
              ...
              <div className="bg-white p-6 rounded-[2.5rem] shadow-xl border border-slate-100 space-y-4">
                 <input placeholder="Enter Token ID (e.g. CH-X2A)" value={trackId} onChange={e => setTrackId(e.target.value.toUpperCase())} className="w-full bg-slate-50 border p-5 rounded-2xl text-lg font-black text-center outline-none tracking-[0.2em] focus:border-emerald-500" />
                 <button onClick={() => { const b = bookings.find(x => x.displayId === trackId); if(b) setFoundTicket(b); else notify("No active token found", "error"); }} className="w-full py-5 bg-emerald-600 text-white rounded-2xl font-black uppercase text-[10px] tracking-[0.2em] shadow-lg active:scale-95">Track My Position</button>
              </div>

              {liveTrackStatus && !liveTrackStatus.error ? (
                <div className="bg-white p-8 rounded-[3.5rem] shadow-2xl border-t-8 border-emerald-500 text-center space-y-6 animate-in zoom-in-95">
                   <div className="w-20 h-20 bg-emerald-50 text-emerald-600 rounded-full flex items-center justify-center mx-auto text-3xl font-black italic shadow-inner border border-emerald-100">{liveTrackStatus.pos}</div>
                   <div>
                      <h3 className="text-3xl font-black tracking-tighter uppercase italic">{liveTrackStatus.displayId}</h3>
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">Confirmed for {liveTrackStatus.name}</p>
                   </div>
                   <div className="grid grid-cols-2 gap-3">
                      <div className="bg-slate-50 p-4 rounded-3xl border border-slate-100 shadow-inner">
                         ...
                         <p className="text-xl font-black text-emerald-600">{liveTrackStatus.pos === 1 ? "NEXT" : liveTrackStatus.pos - 1 + " Ahead"}</p>
                      </div>
                      <div className="bg-slate-50 p-4 rounded-3xl border border-slate-100 shadow-inner">
                         <p className="text-[8px] font-black text-slate-400 uppercase">Est. Minutes</p>
                         <p className="text-xl font-black text-blue-600">{liveTrackStatus.wait}m</p>
                      </div>
                   </div>
                   <button onClick={() => setView('home')} className="w-full py-4 bg-slate-900 text-white rounded-2xl font-black uppercase text-[10px] active:scale-95">Home explorer</button>
                </div>
              ) : foundTicket && <p className="text-center text-rose-500 font-black uppercase text-[10px] tracking-widest py-10">This booking is no longer active.</p>}
           </div>
        )}

        {/* VIEW: MERCHANT (NO MIXED FEATURES) */}
        {view === 'merchant' && mData && (
          <div className="pt-6 space-y-6 animate-in slide-in-from-bottom-8 px-1">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-slate-900 p-6 rounded-[2.5rem] text-white flex flex-col justify-between shadow-lg h-44">
                 <div><p className="text-[8px] font-black uppercase opacity-50 mb-1">Live visibility</p><p className="text-lg font-black uppercase italic tracking-tighter leading-none">{mData.store.isLive ? 'Publicly Live' : 'Hidden Mode'}</p></div>
              ...
              <div className="bg-white p-2 rounded-[2.5rem] border border-slate-100 shadow-lg relative overflow-hidden h-44 group">
                 ...
                 <label className="absolute inset-0 flex flex-col items-center justify-center cursor-pointer">
                    ...
                    <span className="text-[8px] font-black uppercase mt-1 text-slate-800">Upload Photo</span>
            ...
            {mTab === 'ledger' && (
               <section className="space-y-4 pb-20 px-1">
                  ...
                  {mData.queue.length === 0 && <p className="text-center py-20 text-slate-300 font-black uppercase text-[10px] tracking-widest italic">Ledger Empty</p>}
               </section>
            )}
            ...
            {mTab === 'assets' && (
              <div className="bg-white p-8 rounded-[3rem] border border-slate-100 space-y-6 mx-1 animate-in fade-in">
                ...
                }} className="w-full py-4 border-2 border-dashed border-slate-200 rounded-2xl font-black uppercase text-[10px] text-slate-400 hover:border-emerald-300 transition-all">+ Add {mData.store.category === 'salon' ? 'Professional' : 'Trip Slot'}</button>
                <div className="space-y-3">
                   {mData.store.resources?.map((r, i) => (
                     <div key={i} className="flex justify-between items-center p-4 bg-slate-50 rounded-2xl border border-slate-100">
                        ...
                        <button onClick={() => updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'stores', profile.businessId), { resources: arrayRemove(r) })} className="p-2 text-rose-500 active:scale-90"><Lucide.X size={18}/></button>
                     </div>
                   ))}
                </div>
              </div>
          ...
          <div className="pt-4 space-y-6 animate-in slide-in-from-right-4">
            ...
            <div className="relative mx-1">
              ...
              <div className="absolute bottom-6 left-8 right-8 text-white">
                ...
                <p className="text-white/80 text-[10px] font-bold uppercase flex items-center mt-1 leading-none tracking-widest"><Lucide.MapPin size={12} className="mr-1"/> {selStore.address}</p>
              </div>
            </div>
            ...
            {selService && (
              <div className="bg-white p-6 rounded-[3rem] shadow-sm border border-slate-100 animate-in slide-in-from-bottom-6 space-y-5 mx-1">
                  <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-center italic leading-none">2. Booking Matrix</h3>
                  ...
                  <div className="space-y-2">
                    <label className="text-[8px] font-black text-slate-400 uppercase ml-2 tracking-widest">Available {selStore.category === 'salon' ? 'Staff' : 'Trip Timing'}</label>
                    <div className="space-y-2">
                      {selStore.resources?.map(r => {
                        const { count, left } = getSlotStats(selStore.id, r.id, r.capacity);
                        ...
                        return (
                          <div key={r.id} onClick={() => !isFull && setBookForm({...bookForm, resId: r.id})} className={`p-4 rounded-2xl border-2 transition-all cursor-pointer flex justify-between items-center ${bookForm.resId === r.id ? 'border-emerald-600 bg-emerald-50' : 'border-slate-50 opacity-60'} ${isFull ? 'opacity-30 cursor-not-allowed grayscale' : ''}`}>
                            <div className="text-left">
                               ...
                               {r.time && <p className="text-[8px] font-black text-blue-500 mt-1 uppercase tracking-widest">Timing: {r.time}</p>}
                            </div>
                            <span className={`text-[8px] font-black uppercase tracking-tighter ${isFull ? 'text-rose-500' : 'text-emerald-600'} leading-none`}>
                              {selStore.category === 'salon' ? `Queue: ${count}` : isFull ? 'FULL' : `${left} S Left`}
                            </span>
                          </div>
                        );
                      })}
                  ...
                  <button disabled={!bookForm.date || (!bookForm.time && selStore.category === 'salon') || !bookForm.phone || !bookForm.resId || !bookForm.name} onClick={() => setPayModal(true)} className="w-full bg-emerald-600 text-white py-5 rounded-2xl font-black uppercase text-[11px] tracking-[0.2em] active:scale-[0.97] transition-all shadow-xl shadow-emerald-200 disabled:opacity-40">Lock My Slot</button>
              </div>
            )}
          </div>
        )}
        ...
        {view === 'business' && profile.role !== 'vendor' && (
          <div className="pt-6 space-y-6 animate-in slide-in-from-bottom-8 px-1">
            ...
            <div className="flex bg-slate-200 p-1.5 rounded-[1.8rem] shadow-inner border border-slate-300 relative mx-1">
               ...
               <button onClick={() => setHubMode('register')} className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest relative z-10 transition-colors ${hubMode === 'register' ? 'text-white' : 'text-slate-500'}`}>Join Us</button>
        ...
        <div className="fixed inset-0 bg-emerald-600 z-[1001] flex items-center justify-center p-6 animate-in zoom-in-95 duration-200 text-white">
           <div className="text-center space-y-8">
              ...
              <div className="space-y-3 pt-6">
                 <button onClick={handleBookingSubmit} className="w-64 py-6 bg-white text-emerald-600 rounded-full font-black uppercase shadow-2xl active:scale-90 transition-all text-lg tracking-widest">YES, CONFIRM</button>
