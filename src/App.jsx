...
const appId = typeof __app_id !== 'undefined' ? __app_id : 'chiplun-final-master-v50';
...
export default function App() {
  ...
  const liveTrackStatus = useMemo(() => {
    ...
    if (!live || live.status !== 'pending') return { error: 'Booking closed or deleted.' };
  ...
  const handleBookingFinal = async () => {
    // ANTI-CHEAT: Check for past time
    const now = new Date();
  ...
  const handleAdminApprove = async (req) => {
    const mId = prompt("New Merchant ID:");
    ...
    try {
      ...
      notify("Approved!");
    } catch (e) { notify("Approval Error", "error"); }
    setIsProcessing(false);
  };
        ...
        {view === 'track' && (
           <div className="pt-6 space-y-6 animate-in slide-in-from-bottom-8 px-1">
              ...
              <div className="bg-white p-6 rounded-[2.5rem] shadow-xl border border-slate-100 space-y-4">
                 ...
                 <button onClick={() => { const b = bookings.find(x => x.displayId === trackInput); if(b) setActiveReceipt(b); else notify("Booking not found", "error"); }} className="w-full py-5 bg-emerald-600 text-white rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-lg">View Status</button>
              </div>
              ...
              {liveTrackStatus && !liveTrackStatus.error ? (
                <div className="bg-white p-8 rounded-[3.5rem] shadow-2xl border-t-8 border-emerald-500 text-center space-y-6 animate-in zoom-in-95">
                   <div className="w-20 h-20 bg-emerald-50 text-emerald-600 rounded-full flex items-center justify-center mx-auto text-3xl font-black italic border border-emerald-100">{liveTrackStatus.pos}</div>
                   <div>
                   ...
                   <div className="grid grid-cols-2 gap-3">
                      <div className="bg-slate-50 p-4 rounded-3xl border border-slate-100">
                         ...
                         <p className="text-xl font-black text-emerald-600">{liveTrackStatus.pos === 1 ? "NEXT" : liveTrackStatus.pos - 1 + " People"}</p>
                      </div>
                      <div className="bg-slate-50 p-4 rounded-3xl border border-slate-100">
                   ...
                   <button onClick={() => setView('home')} className="w-full py-4 bg-slate-900 text-white rounded-2xl font-black uppercase text-[10px] active:scale-95">Go to Explorer</button>
                </div>
              ) : activeReceipt && <p className="text-center text-rose-500 font-black uppercase text-[10px] tracking-widest py-10">This token is no longer active.</p>}
           </div>
        )}
          ...
          <div className="pt-6 space-y-6 animate-in slide-in-from-bottom-8 px-1">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-slate-900 p-6 rounded-[2.5rem] text-white flex flex-col justify-between shadow-lg h-44">
                 <div><p className="text-[8px] font-black uppercase opacity-50 mb-1">Status</p><p className="text-lg font-black uppercase italic tracking-tighter leading-none">{mData.store.isLive ? 'Online' : 'Offline'}</p></div>
            ...
            {mTab === 'ledger' && (
               <section className="space-y-4 pb-20 px-1">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-white p-6 rounded-[2rem] border border-slate-100 text-center shadow-sm"><p className="text-[8px] font-black text-slate-400 mb-1">TOTAL SALES</p><p className="text-2xl font-black text-emerald-600">₹{mData.rev}</p></div>
                    <div className="bg-white p-6 rounded-[2rem] border border-slate-100 text-center shadow-sm"><p className="text-[8px] font-black text-slate-400 mb-1">ACTIVE QUEUE</p><p className="text-2xl font-black text-blue-600">{mData.queue.length}</p></div>
                  </div>
                  {mData.queue.map((b, i) => (
                    <div key={i} className="bg-white p-5 rounded-[2rem] border-l-8 border-emerald-500 shadow-sm space-y-4 animate-in slide-in-from-left-4">
                      ...
                      <div className="flex gap-2">
                        ...
                        <button onClick={() => updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'bookings', b.id), { status: 'completed' })} className="flex-1 p-3 bg-emerald-50 text-emerald-600 rounded-xl flex items-center justify-center gap-2"><Lucide.CheckCircle2 size={16}/><span className="text-[9px] font-black uppercase tracking-widest">Complete</span></button>
                        <button onClick={async () => { if(window.confirm("Cancel Booking? User position will shift!")) await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'bookings', b.id)); }} className="p-3 bg-rose-50 text-rose-500 rounded-xl active:scale-90"><Lucide.Trash2 size={18}/></button>
                      </div>
                    </div>
                  ))}
               </section>
            ...
            {mTab === 'assets' && (
              <div className="bg-white p-8 rounded-[3rem] border border-slate-100 space-y-6 mx-1 animate-in fade-in">
                <button onClick={() => {
                   const n = prompt(mData.store.category === 'salon' ? "Expert Name:" : "Trip Route (e.g. Mumbai 9AM):");
                ...
                <div className="space-y-3">
                   {mData.store.resources?.map((r, i) => (
                     <div key={i} className="flex justify-between items-center p-4 bg-slate-50 rounded-2xl border border-slate-100 shadow-inner">
                        <div><p className="font-black text-xs uppercase italic">{r.name}</p>{r.time && <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mt-1">Leaves: {r.time} • Seats: {r.capacity}</p>}</div>
            ...
            {mTab === 'prices' && (
               <div className="bg-white p-8 rounded-[3rem] border border-slate-100 space-y-6 mx-1 animate-in fade-in">
                  ...
                  }} className="w-full py-4 border-2 border-dashed border-slate-200 rounded-2xl font-black uppercase text-[10px] text-slate-400 hover:border-emerald-300">+ Add New Service</button>
                  <div className="space-y-3">
          ...
          <div className="pt-4 space-y-6 animate-in slide-in-from-right-4">
            ...
            {selService && (
              <div className="bg-white p-6 rounded-[3rem] shadow-sm border border-slate-100 animate-in slide-in-from-bottom-6 space-y-5 mx-1">
                  ...
                  <div className="space-y-2">
                    <label className="text-[8px] font-black text-slate-400 uppercase ml-2 tracking-widest">Select {selStore.category === 'salon' ? 'Staff' : 'Trip Timing'}</label>
