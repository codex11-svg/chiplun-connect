...
const App = () => {
  ...
  if (loading) return <div className="h-screen flex flex-col items-center justify-center bg-emerald-50"><Loader2 className="animate-spin text-emerald-600 mb-4" size={48}/><p className="text-emerald-900 font-black tracking-widest animate-pulse">CHIPLUN CONNECT</p></div>;
      ...
      <main className="flex-1 -mt-6 px-4 pb-32 overflow-y-auto z-10">
               ...
               <div className="space-y-4">
                 ...
                 {stores.filter(s => s.isLive).length === 0 && (
                   <div className="text-center py-20 bg-white rounded-[3rem] border-2 border-dashed border-gray-100 flex flex-col items-center">
                     ...
                     <p className="text-gray-400 text-xs font-bold tracking-widest uppercase">No Businesses Live Yet</p>
                   </div>
                 )}
               </div>
             </section>
               ...
               <div className="bg-white p-6 rounded-[3rem] shadow-sm animate-in slide-in-from-bottom-6 border border-gray-50">
                 ...
                 <div className="space-y-4 mb-8">
                   <div className="space-y-1.5">
                     <p className="text-[9px] font-bold text-gray-400 ml-1 uppercase tracking-widest text-center">Select Expert</p>
                     <select onChange={e => setBookingMeta({...bookingMeta, staffName: e.target.value})} className="w-full bg-gray-50 p-4 rounded-2xl outline-none font-bold text-sm border-2 border-transparent focus:border-emerald-500 transition-all appearance-none text-center">
                       <option value="">Any Staff (Quickest)</option>
                 ...
                 <div className="space-y-4 pt-6 border-t border-gray-100">
                    ...
                    <input placeholder="Phone Number" type="tel" value={bookingMeta.custPhone} onChange={e => setBookingMeta({...bookingMeta, custPhone: e.target.value})} className="w-full bg-gray-50 p-4 rounded-2xl font-bold text-sm outline-none border-2 border-transparent focus:border-emerald-500" />
                 </div>
        ...
        {view === 'dashboard' && profile.role === 'vendor' && (
          <div className="pt-4 space-y-6 animate-in slide-in-from-bottom-8 duration-500">
            <div className="flex justify-between items-end">
               <div><h2 className="text-2xl font-black text-emerald-900 tracking-tighter">{profile.businessName}</h2><p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">Dashboard</p></div>
               <button onClick={() => setView('home')} className="bg-white p-3 rounded-2xl text-gray-400 shadow-sm active:scale-90 transition-all"><XCircle size={20}/></button>
            </div>
            <div className={`p-6 rounded-[3rem] border-2 flex justify-between items-center transition-all ${stores.find(s => s.ownerId === profile.businessId)?.isLive ? 'bg-emerald-50 border-emerald-200' : 'bg-gray-50 border-gray-100 opacity-80'}`}>
            ...
            <section className="bg-white p-7 rounded-[3.5rem] shadow-sm border border-gray-100"><h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-6">Service Catalog</h3><div className="space-y-3 mb-10">{stores.find(s => s.ownerId === profile.businessId)?.services?.map((s, i) => (<div key={i} className="flex justify-between items-center p-5 bg-gray-50 rounded-[2rem] font-bold text-sm border border-gray-100"><div><p className="text-gray-700">{s.name}</p><p className="text-emerald-600 text-xs">₹{s.price}</p></div><button onClick={() => { const s_ = stores.find(st => st.ownerId === profile.businessId); updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'stores', profile.businessId), { services: s_.services.filter((_, idx) => idx !== i) }); }} className="bg-white p-2.5 rounded-xl text-rose-400 shadow-sm border border-gray-100 active:scale-90 transition-all"><Trash2 size={16}/></button></div>))}</div>
            <div className="space-y-3 pt-6 border-t border-gray-50"><input placeholder="New Service Title" value={newSvc.name} onChange={e => setNewSvc({...newSvc, name: e.target.value})} className="w-full bg-gray-50 p-4 rounded-2xl outline-none font-bold text-sm" /><input placeholder="Price in ₹" type="number" value={newSvc.price} onChange={e => setNewSvc({...newSvc, price: e.target.value})} className="w-full bg-gray-50 p-4 rounded-2xl outline-none font-bold text-sm" /><button onClick={() => { if(!newSvc.name || !newSvc.price) return; const s = stores.find(st => st.ownerId === profile.businessId); updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'stores', profile.businessId), { services: [...(s.services || []), newSvc] }); setNewSvc({name:'', price:''}); }} className="w-full bg-emerald-600 text-white py-4 rounded-3xl font-black shadow-xl active:scale-95 transition-all">Save Service</button></div></section>
          </div>
        )}
        ...
        {view === 'admin_panel' && (
          <div className="pt-4 space-y-6 animate-in slide-in-from-top-8 duration-500">
            <h2 className="text-2xl font-black tracking-tighter">Admin Control</h2>
            ...
            <section className="space-y-4">{requests.filter(r => r.status === 'pending').map(req => (
              <div key={req.id} className="bg-white p-7 rounded-[3.5rem] border border-gray-100 shadow-sm space-y-5 animate-in fade-in">
                ...
                <div className="bg-gray-50 p-5 rounded-3xl space-y-3">
                  <input id={`id-${req.id}`} placeholder="Issue Unique Merchant ID" className="w-full bg-white p-4 rounded-2xl text-xs font-bold outline-none border-2 border-transparent focus:border-emerald-500" />
                  <input id={`pw-${req.id}`} placeholder="Set Security Key" className="w-full bg-white p-4 rounded-2xl text-xs font-bold outline-none border-2 border-transparent focus:border-emerald-500" />
                </div>
                <button onClick={async () => {
                  ...
                  }} className="w-full bg-emerald-600 text-white py-4 rounded-2xl font-black shadow-xl shadow-emerald-100 active:scale-95 transition-all">Approve & Issue Access</button>
              </div>
            ))}</section>
          </div>
        )}
        ...
        {view === 'admin_auth' && (
           <div className="pt-20 text-center animate-in zoom-in-95 duration-500">
              <div className="w-24 h-24 bg-emerald-50 text-emerald-600 rounded-[2.5rem] flex items-center justify-center mx-auto mb-8 shadow-inner"><Lock size={48} /></div>
              <h2 className="text-3xl font-black mb-2 tracking-tighter">Gateway</h2>
              <p className="text-[10px] text-gray-400 mb-12 uppercase tracking-widest text-center">Administrative Access Key</p>
           ...
           <div className="pt-4 space-y-6 animate-in slide-in-from-bottom-4 duration-500">
              ...
              {profile.status === 'none' ? (
                <div className="bg-white p-7 rounded-[3.5rem] space-y-4 shadow-sm mt-4 border border-gray-50">
                  ...
                  <input placeholder="Store Location (Chiplun)" value={regForm.addr} onChange={e => setRegForm({...regForm, addr: e.target.value})} className="w-full bg-gray-50 p-4 rounded-2xl font-bold text-sm outline-none border-2 border-transparent focus:border-emerald-500 transition-all" />
                  <button onClick={submitApplication} className="w-full bg-emerald-600 text-white py-4 rounded-3xl font-black shadow-xl mt-4 active:scale-95 transition-all uppercase tracking-widest">Apply Now</button>
                </div>
        ...
        {view === 'vendor_login' && (
          <div className="pt-4 space-y-6 animate-in slide-in-from-right-8 duration-500">
             ...
             <div className="bg-white p-7 rounded-[3.5rem] space-y-4 shadow-sm border border-gray-100">
                <input placeholder="Assigned Merchant User ID" value={vendorLogin.id} onChange={e => setVendorLogin({...vendorLogin, id: e.target.value})} className="w-full bg-gray-50 p-4 rounded-2xl font-bold text-sm outline-none focus:ring-2 ring-emerald-500" />
                ...
                <button onClick={handleVendorAuth} className="w-full bg-emerald-600 text-white py-4 rounded-3xl font-black shadow-2xl shadow-emerald-200 active:scale-95 transition-all uppercase tracking-widest">Secure Access</button>
             </div>
          </div>
        )}
        ...
        {view === 'confirmation' && (
           <div className="text-center pt-24 animate-in zoom-in-90 duration-700">
             ...
             <p className="text-gray-400 mb-14 italic px-12 leading-relaxed font-bold uppercase text-[10px] tracking-[0.2em]">"Booking secured for {bookingMeta.custName} at {selectedStore?.name}. See you soon!"</p>
