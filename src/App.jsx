...
const appId = "chiplun-supreme-v50-final-stable"; 
...
export default function App() {
  ...
  const handleAdminApprove = async (req) => {
    const mId = prompt("Enter Merchant ID (CH-XXXX):");
    ...
    try {
      ...
      notify("Approved! Merchant can now configure.");
  ...
  if (loading) return <div className="h-screen flex items-center justify-center bg-emerald-600 text-white font-black animate-pulse uppercase tracking-[0.3em]">CHIPLUNCONNECT</div>;
      ...
      <header className="bg-emerald-600 text-white p-6 pb-12 rounded-b-[3.5rem] shadow-lg sticky top-0 z-50">
        <div className="flex justify-between items-center mb-6">
          <div onClick={() => setView('home')} className="cursor-pointer active:scale-95 transition-all">
            ...
            <p className="text-[10px] font-bold opacity-80 uppercase tracking-widest mt-1">V50 Supreme Final</p>
          </div>
          <button 
      ...
      <main className="flex-1 -mt-6 px-4 pb-32 z-10">
            ...
            <section className="space-y-4">
              ...
              <div className="space-y-4">
                {filteredStores.map(store => (
                  <div key={store.id} onClick={() => { setSelectedStore(store); setView('detail'); setCart([]); }} className="bg-white p-3 rounded-[2.5rem] flex gap-4 items-center shadow-sm border border-slate-100 group active:scale-[0.98] transition-all">
        ...
        {view === 'business' && profile.role !== 'vendor' && (
          <div className="pt-6 space-y-6 animate-in slide-in-from-bottom-8">
            <div className="text-center">
              ...
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Grow your business in Chiplun</p>
            </div>
            ...
            {bizSubView === 'register' ? (
              ...
              <div className="bg-white p-8 rounded-[3rem] shadow-xl border border-slate-100 space-y-4 text-center">
                ...
                <button onClick={handleVLogin} className="w-full bg-emerald-600 text-white py-5 rounded-[1.5rem] font-black uppercase text-[10px] tracking-[0.2em] shadow-xl active:scale-95 transition-all">Unlock Terminal</button>
              </div>
            )}
          </div>
        )}
        ...
        {view === 'merchant' && activeMerchant && (
          <div className="pt-6 space-y-6 animate-in slide-in-from-bottom-8 px-1">
            ...
            <div className={`p-6 rounded-[2.5rem] flex items-center justify-between shadow-lg transition-all ${activeMerchant.store.isLive ? 'bg-emerald-600 text-white' : 'bg-slate-900 text-white'}`}>
              <div><p className="text-[9px] font-black uppercase opacity-70 tracking-widest">Visibility Status</p><p className="text-xl font-black uppercase italic tracking-tighter">{activeMerchant.store.isLive ? 'Online & Open' : 'Offline (Hidden)'}</p></div>
            ...
            <div className="flex bg-white p-1 rounded-2xl border border-slate-100 shadow-sm overflow-x-auto">
                {['ledger', 'inventory', 'pricing'].map(t => (
                    <button key={t} onClick={() => setMTab(t)} className={`flex-1 py-3 px-2 rounded-xl text-[8px] font-black uppercase tracking-widest transition-all ${mTab === t ? 'bg-emerald-50 text-emerald-600' : 'text-slate-400'}`}>{t}</button>
                ))}
            </div>
            ...
            {mTab === 'ledger' && (
               <section className="space-y-4 pb-20">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-white p-6 rounded-[2rem] border border-slate-100 text-center shadow-sm"><p className="text-[8px] font-black text-slate-400 mb-1">REVENUE</p><p className="text-2xl font-black text-emerald-600">₹{activeMerchant.rev}</p></div>
                    <div className="bg-white p-6 rounded-[2rem] border border-slate-100 text-center shadow-sm"><p className="text-[8px] font-black text-slate-400 mb-1">WAITLIST</p><p className="text-2xl font-black text-blue-600">{activeMerchant.pending.length}</p></div>
                  </div>
            ...
            {mTab === 'inventory' && (
              <div className="bg-white p-8 rounded-[3rem] border border-slate-100 space-y-6 animate-in fade-in">
                <button onClick={() => {
                   const n = prompt(activeMerchant.store.category === 'salon' ? "Enter Barber Name:" : "Enter Route (e.g. Mumbai 9AM):");
                   const c = activeMerchant.store.category === 'travel' ? prompt("Vehicle Total Seats:") : 1;
                ...
                <div className="space-y-3">
                   {activeMerchant.store.resources?.map((r, i) => (
                     <div key={i} className="flex justify-between items-center p-4 bg-slate-50 rounded-2xl border border-slate-100">
                        <div><p className="font-black text-xs uppercase italic leading-none">{r.name}</p>{activeMerchant.store.category === 'travel' && <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mt-1">Total Capacity: {r.capacity} Seats</p>}</div>
            ...
            {mTab === 'services' && (
               <div className="bg-white p-8 rounded-[3rem] border border-slate-100 space-y-6 animate-in fade-in">
                  <button onClick={() => {
                     ...
                     const p = prompt(activeMerchant.store.category === 'travel' ? "Rate per KM (₹):" : "Flat Price (₹):");
                     if (n && p) updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'stores', profile.businessId), { services: arrayUnion({ name: n, price: Number(p) }) });
                  }} className="w-full py-4 border-2 border-dashed border-slate-200 rounded-2xl font-black uppercase text-[10px] text-slate-400 hover:border-emerald-300 transition-all">+ Add Rate</button>
                  <div className="space-y-3">
        ...
        {view === 'detail' && selectedStore && (
          <div className="pt-4 space-y-6 animate-in slide-in-from-right-4">
            <button onClick={() => setView('home')} className="flex items-center text-emerald-600 font-black text-[10px] uppercase tracking-widest active:scale-95 transition-all px-2"><Lucide.ArrowLeft size={16} className="mr-2"/> Back to Discovery</button>
            ...
            <section className="bg-white p-6 rounded-[3rem] border border-slate-100 shadow-sm space-y-4 mx-1">
              ...
              <div className="space-y-2">
                ...
                )) : <p className="text-center text-[9px] text-slate-300 font-black uppercase py-4">No rates listed by partner</p>}
              </div>
            </section>
            ...
            {cart.length > 0 && (
              <div className="bg-white p-6 rounded-[3rem] shadow-sm border border-slate-100 animate-in slide-in-from-bottom-6 space-y-5 mx-1">
                    ...
                    <div className="space-y-3">
                       <div className="grid grid-cols-2 gap-2">
                          <div className="space-y-1">
                             ...
                             <select value={bookMeta.pickup} onChange={e => setBookMeta({...bookMeta, pickup: e.target.value})} className="w-full bg-slate-50 p-4 rounded-xl font-black text-[10px] outline-none border border-slate-100 appearance-none shadow-inner">
                          ...
                          <div className="space-y-1">
                             ...
                             <select value={bookMeta.drop} onChange={e => setBookMeta({...bookMeta, drop: e.target.value})} className="w-full bg-slate-50 p-4 rounded-xl font-black text-[10px] outline-none border border-slate-100 appearance-none shadow-inner">
                               {ROUTE_STOPS.map(s => <option key={s.n} value={s.n}>{s.n}</option>)}
                             </select>
                          </div>
                       </div>
                  ...
                  <div className="space-y-2">
                    <label className="text-[8px] font-black text-slate-400 uppercase ml-2 tracking-widest">Available {selectedStore.category === 'salon' ? 'Experts' : 'Trip Timings'}</label>
                    <div className="space-y-2">
                        ...
                        return (
                          <div key={r.id} onClick={() => !isFull && setBookMeta({...bookMeta, resId: r.id})} className={`p-4 rounded-2xl border-2 transition-all cursor-pointer flex justify-between items-center ${bookMeta.resId === r.id ? 'border-emerald-600 bg-emerald-50' : 'border-slate-50 opacity-60'} ${isFull ? 'opacity-30 cursor-not-allowed' : ''}`}>
                            ...
                            <span className={`text-[8px] font-black uppercase ${isFull ? 'text-rose-500' : 'text-emerald-600'} tracking-tighter`}>
                              {selectedStore.category === 'salon' ? `Queue: ${count} (~${count * 20}m)` : isFull ? 'Full' : `${remaining} S Left`}
                            </span>
                          </div>
                        );
                      })}
                  ...
                  {selectedStore.category === 'travel' && bookMeta.resId && (
                    <div className="grid grid-cols-2 gap-3 bg-blue-50/50 p-4 rounded-2xl border border-blue-100 items-center">
                       ...
                       <div className="text-right">
                          <label className="text-[8px] font-black text-blue-400 uppercase block tracking-widest">Route Total</label>
                  ...
                  <button disabled={!bookMeta.date || !bookMeta.time || !bookMeta.phone || !bookMeta.resId} onClick={handleBooking} className="w-full bg-emerald-600 text-white py-5 rounded-2xl font-black uppercase text-[11px] tracking-[0.2em] active:scale-[0.97] transition-all shadow-xl shadow-emerald-200 disabled:opacity-40">Issue Official Ticket</button>
              </div>
            )}
          </div>
        )}
          ...
          <div className="pt-10 space-y-8 animate-in slide-in-from-bottom-8">
            ...
            <div className="bg-white p-8 rounded-[3.5rem] shadow-2xl border-t-8 border-emerald-500 text-center space-y-6 mx-1">
                ...
                <div className="bg-slate-50 p-5 rounded-3xl text-[9px] font-black uppercase text-left space-y-2 border border-slate-100 shadow-inner">
                  <div className="flex justify-between italic tracking-widest"><span>Merchant:</span><span className="text-emerald-700 font-black tracking-tighter">{tracked.storeName}</span></div>
                  ...
                  <div className="flex justify-between pt-2 border-t border-slate-200 text-xs font-black"><span>Total Payable:</span><span className="text-emerald-600 text-lg tracking-tighter">₹{tracked.totalPrice}</span></div>
                </div>
