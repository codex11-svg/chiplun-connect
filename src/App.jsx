...
export default function App() {
  ...
  // Unique Logic States
  const [queueInfo, setQueueInfo] = useState({ position: 0, estTime: '' });
  ...
  const calculateQueue = (storeId, date, time, resource) => {
    const ahead = allBookings.filter(b => 
      b.storeId === storeId && 
      b.date === date && 
      b.time === time && 
      (resource === "" || b.resourceName === resource)
    ).length;

    const position = ahead + 1;
    // Estimate: base time + (30 mins per person ahead)
    const baseMinutes = ahead * 30;
  ...
  const handleFullStoreDeletion = async () => {
    ...
    try {
      ...
      const mId = deleteTarget.merchantId; // We'll start saving merchantId on store docs now
      ...
      // 3. Delete Request
      await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'requests', storeId));
      
      setDeleteTarget(null);
      alert("Store and all associated access keys deleted.");
  ...
  const finalizeBooking = async () => {
    ...
    const q = calculateQueue(selectedStore.id, bookingMeta.date, bookingMeta.time, bookingMeta.resourceName);
    ...
    setTimeout(async () => {
      ...
      setQueueInfo({ position: q.position, estTime: q.baseMinutes });
      setIsProcessing(false);
      ...
      {deleteTarget && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[200] flex items-center justify-center p-6 animate-in fade-in">
           <div className="bg-white w-full rounded-[2.5rem] p-8 shadow-2xl">
              <Lucide.AlertTriangle size={48} className="text-rose-500 mx-auto mb-4" />
              <h3 className="text-xl font-black text-center text-gray-900">Destroy Business?</h3>
              <p className="text-center text-gray-400 text-xs mt-2 leading-relaxed">This will permanently delete <span className="font-bold text-gray-900">"{deleteTarget.name}"</span> and revoke the owner's Merchant ID and Password forever.</p>
              <div className="flex flex-col gap-3 mt-8">
                 <button onClick={handleFullStoreDeletion} className="w-full bg-rose-500 text-white py-4 rounded-2xl font-black uppercase tracking-widest text-xs">Confirm Destruction</button>
                 <button onClick={() => setDeleteTarget(null)} className="w-full bg-gray-100 text-gray-400 py-4 rounded-2xl font-black uppercase tracking-widest text-xs">Cancel</button>
              </div>
           </div>
        </div>
      )}
      ...
      <header className="bg-emerald-600 text-white p-6 pb-12 rounded-b-[4rem] shadow-lg sticky top-0 z-50">
        <div className="flex justify-between items-center mb-6">
          <div onClick={() => setView('home')} className="cursor-pointer active:scale-95 transition-all">
            ...
            <p className="text-[9px] font-bold opacity-70 uppercase tracking-widest">Business Intelligence MH-08</p>
          </div>
      ...
      <main className="flex-1 -mt-6 px-4 pb-32 overflow-y-auto z-10">
           ...
           <div className="pt-4 space-y-6 animate-in slide-in-from-right-4 pb-10">
             ...
             <section className="bg-white p-6 rounded-[3rem] shadow-sm border border-gray-100">
               ...
               <div className="space-y-3">
                 {selectedStore.services?.map((s, i) => (
                   <div key={i} onClick={() => { if(cart.find(c => c.name === s.name)) setCart(cart.filter(c => c.name !== s.name)); else setCart([...cart, s]); }} className={`p-5 rounded-3xl border-2 transition-all flex justify-between items-center ${cart.find(c => c.name === s.name) ? 'border-emerald-600 bg-emerald-50 shadow-md' : 'border-gray-50 bg-gray-50'}`}>
                     <div><p className="font-bold text-sm leading-none">{s.name}</p><p className="text-[8px] text-gray-400 mt-1 uppercase font-bold">{s.duration || '30'} mins</p></div>
             ...
             {cart.length > 0 && (
               <div className="bg-white p-6 rounded-[3rem] shadow-sm border border-gray-100 animate-in slide-in-from-bottom-4">
                 <h3 className="text-[10px] font-black text-gray-400 uppercase mb-6 text-center tracking-widest">Final Specs</h3>
                 <div className="space-y-4">
                   ...
                   <div className="p-4 bg-emerald-50 rounded-2xl border border-emerald-100 flex items-center gap-3">
                      <Lucide.Info size={16} className="text-emerald-600" />
                      <p className="text-[9px] font-bold text-emerald-800 uppercase leading-tight">Your expected time and queue position will be calculated after confirmation.</p>
                   </div>

                   <button onClick={() => setView('payment')} className="w-full bg-emerald-600 text-white py-5 rounded-3xl font-black shadow-xl uppercase active:scale-95 transition-all tracking-widest">Book Appointment</button>
                 </div>
              </div>
            )}
           </div>
        ...
        {view === 'payment' && (
           <div className="pt-4 space-y-6 animate-in zoom-in-95 duration-500">
              <div className="text-center mb-8"><Lucide.Lock className="mx-auto text-emerald-600 mb-4" size={40}/><h2 className="text-2xl font-black uppercase tracking-tighter italic">Securing Slot</h2></div>
              <div className="bg-white p-8 rounded-[3.5rem] shadow-xl border border-gray-100 space-y-8">
                 <div className="flex justify-between items-center pb-4 border-b border-gray-50">
                    <p className="font-black text-emerald-900 uppercase italic">Confirm Booking</p><p className="text-3xl font-black text-emerald-600">₹{cart.reduce((a, b) => a + Number(b.price), 0)}</p>
                 </div>
                 <div className="grid grid-cols-2 gap-3 opacity-60">
                    <button onClick={() => alert("UPI maintenance. Use 'Pay at Store'.")} className="flex flex-col items-center p-5 rounded-[2rem] border-2 border-gray-100 bg-gray-50 active:scale-90 transition-all"><Lucide.Smartphone size={24}/><span className="text-[8px] font-black mt-2 uppercase">GPay / UPI</span></button>
                    <button onClick={() => alert("Card gateway maintenance. Use 'Pay at Store'.")} className="flex flex-col items-center p-5 rounded-[2rem] border-2 border-gray-100 bg-gray-50 active:scale-90 transition-all"><Lucide.CreditCard size={24}/><span className="text-[8px] font-black mt-2 uppercase">Debit Card</span></button>
                 </div>
        ...
        {/* VIEW: CONFIRMATION (Queue Intelligence) */}
        {view === 'confirmation' && (
           <div className="text-center pt-10 animate-in zoom-in-90 duration-700 pb-10">
             ...
             <h2 className="text-3xl font-black text-emerald-900 mb-6 uppercase tracking-tighter">Verified</h2>
             ...
             <div className="bg-emerald-600 text-white p-8 rounded-[3.5rem] shadow-2xl mx-4 space-y-6 relative overflow-hidden">
                ...
                <div className="grid grid-cols-2 gap-4 border-t border-white/10 pt-6">
                   <div className="text-center border-r border-white/10">
                      <p className="text-[8px] font-black uppercase opacity-60">Position</p>
                      <p className="text-2xl font-black">#{queueInfo.position}</p>
                   </div>
                   <div className="text-center">
                      <p className="text-[8px] font-black uppercase opacity-60">Estimated Start</p>
                      <p className="text-2xl font-black">+{queueInfo.estTime}m</p>
                   </div>
                </div>
                
                <p className="text-[8px] font-bold uppercase bg-white/10 py-2 rounded-xl">Expected Time: {bookingMeta.time || '10:00 AM'} (approx.)</p>
             </div>

             <p className="text-gray-400 my-10 px-12 leading-relaxed font-bold uppercase text-[9px] opacity-60 italic tracking-widest">
                "Present this Token to {selectedStore?.name}. Screenshot saved!"
             </p>
             <button onClick={() => setView('home')} className="w-full bg-emerald-700 text-white py-5 rounded-[2.5rem] font-black shadow-2xl active:scale-95 uppercase tracking-widest">Done</button>
           </div>
        )}
        ...
        {view === 'merchant_login' && (
           <div className="pt-20 space-y-6 animate-in zoom-in-95 duration-500">
              ...
              <div className="bg-white p-8 rounded-[3.5rem] shadow-2xl space-y-4 border border-gray-100">
                 ...
                 <input type="password" placeholder="Security Passcode" value={vendorLogin.pass} onChange={e => setVendorLogin({...vendorLogin, pass: e.target.value})} className="w-full bg-gray-50 p-4 rounded-2xl font-bold text-sm outline-none" />
                 <button onClick={handleVendorLogin} className="w-full bg-emerald-600 text-white py-5 rounded-[2rem] font-black shadow-xl active:scale-95 transition-all uppercase tracking-widest">Authorize Access</button>
              </div>
           </div>
        )}
        ...
        {view === 'dashboard' && isSessionVerified && profile.role === 'vendor' && (
          <div className="pt-4 space-y-6 animate-in slide-in-from-bottom-8 pb-10">
            <div className="flex justify-between items-center"><h2 className="text-2xl font-black text-emerald-900 uppercase italic tracking-tighter">{profile.businessName}</h2><button onClick={() => { setIsSessionVerified(false); setView('home'); }} className="bg-rose-50 px-4 py-2 rounded-xl text-rose-500 font-black text-[8px] uppercase tracking-widest">Logout</button></div>

            <section className="bg-white p-7 rounded-[3rem] shadow-sm border border-gray-100 space-y-4">
               <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest flex items-center"><Lucide.Edit size={14} className="mr-2 text-emerald-600"/> Modify storefront</h3>
               <input placeholder="Shop Name" value={editForm.name} onChange={e => setEditForm({...editForm, name: e.target.value})} className="w-full bg-gray-50 p-4 rounded-xl font-bold text-xs outline-none" />
               ...
               <button onClick={async () => { await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'stores', profile.businessId), editForm); alert("Updated Successfully!"); }} className="w-full bg-emerald-600 text-white py-3 rounded-xl font-black uppercase text-[10px]">Sync Changes</button>
            </section>

            <section className="bg-white p-7 rounded-[3rem] shadow-sm border border-gray-100">
               <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-6">Manage Team</h3>
               ...
               <div className="flex gap-2"><input placeholder="New Name" value={newStaff} onChange={e => setNewStaff(e.target.value)} className="flex-1 bg-gray-50 p-3 rounded-lg font-bold text-sm outline-none" /><button onClick={() => { if(!newStaff) return; updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'stores', profile.businessId), { staff: [...(myStore.staff || []), newStaff] }); setNewStaff(''); }} className="bg-emerald-600 text-white p-3 rounded-lg"><Lucide.Plus size={18}/></button></div>
            </section>
        ...
        {view === 'vendor_portal' && (
           <div className="pt-4 space-y-6 animate-in slide-in-from-bottom-4">
              <div className="text-center mb-8">
                 ...
                 <h2 className="text-3xl font-black uppercase tracking-tighter text-emerald-900">Partner Program</h2>
              ...
              <div className="bg-white p-7 rounded-[3.5rem] shadow-xl space-y-4 border border-gray-50">
                 <input placeholder="Business Name" value={regForm.bizName} onChange={e => setRegForm({...regForm, bizName: e.target.value})} className="w-full bg-gray-50 p-4 rounded-xl font-bold text-sm outline-none border border-transparent focus:border-emerald-200" />
                 <input placeholder="Chiplun Area Location" value={regForm.addr} onChange={e => setRegForm({...regForm, addr: e.target.value})} className="w-full bg-gray-50 p-4 rounded-xl font-bold text-sm outline-none border border-transparent focus:border-emerald-200" />
                 <select value={regForm.cat} onChange={e => setRegForm({...regForm, cat: e.target.value})} className="w-full bg-gray-50 p-4 rounded-xl font-bold text-sm outline-none border border-transparent focus:border-emerald-200">
                    <option value="salon">Salon</option><option value="travel">Travel</option><option value="clinic">Clinic</option><option value="repair">Other</option>
                 </select>
                 <button 
                  onClick={async () => {
                    ...
                    alert("Application Submitted! Admin will verify your location.");
                    setView('home');
                  }} 
        ...
        {view === 'admin_auth' && (
           <div className="pt-20 text-center animate-in zoom-in-95 duration-500">
              ...
              <h2 className="text-2xl font-black mb-10 tracking-tighter text-emerald-900 uppercase font-black">Admin Core</h2>
        ...
        {view === 'admin_panel' && (
          <div className="pt-4 space-y-6 pb-20">
            <h2 className="text-2xl font-black text-emerald-900 uppercase italic">Admin Management</h2>
            <div className="space-y-4">
              <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Merchant Directory</h3>
              {stores.map(s => (
                <div key={s.id} className="bg-white p-5 rounded-[2.5rem] border border-gray-100 flex items-center gap-4 animate-in fade-in">
                   ...
                   <div className="flex-1 font-bold text-sm uppercase">{s.name} <span className="block text-[8px] opacity-40 font-black tracking-widest italic">{s.category} • ID: {s.merchantId || 'NEW'}</span></div>
                   <button onClick={() => setDeleteTarget(s)} className="text-rose-500 p-2 active:bg-rose-50 rounded-xl transition-all"><Lucide.Trash2 size={20}/></button>
                </div>
              ))}
              ...
              {requests.filter(r => r.status === 'pending').map(req => (
                  <div key={req.id} className="bg-amber-50 p-7 rounded-[3rem] space-y-4 border border-amber-100 shadow-sm animate-in fade-in">
                    ...
                    <div className="flex gap-2">
                      ...
                      <input id={`pw-${req.id}`} placeholder="Pass" className="w-20 p-3 rounded-xl text-xs font-black outline-none border border-amber-200" />
                    </div>
        ...
        {view === 'bookings' && (
           <div className="pt-4 space-y-6 animate-in slide-in-from-bottom-4">
              ...
              <div className="bg-white p-2 rounded-[2rem] shadow-sm border border-gray-100 flex gap-2">
                 <input placeholder="Token ID (CH-XXXX)" value={bookingSearchQuery} onChange={e => setBookingSearchQuery(e.target.value)} className="flex-1 bg-transparent px-4 py-3 outline-none font-black tracking-widest text-sm uppercase" />
                 <button onClick={() => {
                 ...
                 }} className="bg-emerald-600 text-white p-3 rounded-2xl active:scale-90 transition-all shadow-lg shadow-emerald-100"><Lucide.Search size={20}/></button>
              </div>
                ...
                <div className="bg-emerald-600 text-white p-8 rounded-[3.5rem] shadow-2xl animate-in zoom-in-95">
                   ...
                   <div className="space-y-4 text-sm font-bold border-t border-white/10 pt-6 mt-6">
                      <div className="grid grid-cols-2 gap-4">
                         <div><p className="opacity-60 text-[8px] uppercase mb-1">Queue Pos</p>#{searchedBooking.queuePosition}</div>
