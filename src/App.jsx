...
const appId = "chiplun-pro-v11"; 
const ADMIN_PIN = "112607";

export default function App() {
  const [user, setUser] = useState(null);
  const [profile, setProfile] = useState({ role: 'customer', status: 'none' });
  const [view, setView] = useState('home'); // home, store_detail, vendor_portal, account_portal, dashboard, bookings, admin_panel, confirmation, payment
  ...
  const [bookingMeta, setBookingMeta] = useState({
    date: '', returnDate: '', time: '', resourceName: '', custName: '', custPhone: '',
    paymentMethod: 'cash', source: '', destination: '', tripType: 'One-Way', vehicleType: 'AC Sedan'
  });
  ...
  const [newSvc, setNewSvc] = useState({ name: '', price: '', duration: '30 min', category: 'General' });
  ...
  const handleVendorLogin = async () => {
    if (!vendorLogin.id || !vendorLogin.pass) return alert("Enter Merchant ID & Passcode");
    ...
    if (snap.exists() && snap.data().password === vendorLogin.pass) {
      ...
      setView('dashboard');
    } else alert("Access Denied: Check ID/Pass");
  ...
  if (loading) return <div className="h-screen flex items-center justify-center bg-emerald-600 text-white font-black animate-pulse">CHIPLUNCONNECT PRO</div>;
  ...
  return (
    <div className="max-w-md mx-auto bg-gray-50 min-h-screen flex flex-col shadow-2xl relative font-sans text-gray-900 overflow-x-hidden">
      ...
      <header className="bg-emerald-600 text-white p-6 pb-12 rounded-b-[4rem] shadow-lg sticky top-0 z-50">
        <div className="flex justify-between items-center mb-6">
          <div onClick={() => setView('home')} className="cursor-pointer active:scale-95 transition-all">
            <h1 className="text-2xl font-black tracking-tighter italic uppercase">ChiplunConnect</h1>
            <p className="text-[9px] font-bold opacity-70 uppercase tracking-widest">Digital Service Hub • MH-08</p>
          </div>
          <button onClick={() => setView('admin_auth')} className="w-10 h-10 bg-white/10 rounded-2xl flex items-center justify-center border border-white/10 active:bg-white/20 transition-all">
      ...
      <main className="flex-1 -mt-6 px-4 pb-32 overflow-y-auto z-10">
          ...
          <div className="space-y-8 pt-2">
             <div className="grid grid-cols-4 gap-4 animate-in zoom-in-95">
               {[{id:'salon', n:'Salon', i:<Lucide.Scissors/>, c:'bg-rose-50 text-rose-500'},{id:'travel', n:'Travel', i:<Lucide.Bus/>, c:'bg-blue-50 text-blue-500'},{id:'clinic', n:'Clinic', i:<Lucide.Stethoscope/>, c:'bg-emerald-50 text-emerald-500'},{id:'repair', n:'Repair', i:<Lucide.Info/>, c:'bg-amber-50 text-amber-500'}].map(cat => (
                 <button key={cat.id} className="flex flex-col items-center gap-2 active:scale-90 transition-transform">
        ...
        {view === 'store_detail' && selectedStore && (
           <div className="pt-4 space-y-6 animate-in slide-in-from-right-4 pb-10">
             ...
             <section className="bg-white p-6 rounded-[3rem] shadow-sm border border-gray-100">
               <h2 className="text-2xl font-black italic text-emerald-900 uppercase leading-none mb-2">{selectedStore.name}</h2>
               ...
               {selectedStore.category === 'travel' && (
                 <div className="space-y-3 mb-8 p-5 bg-blue-50/50 rounded-[2.5rem] border border-blue-100 animate-in slide-in-from-top-2">
                    <div className="flex gap-2">
                       <select onChange={e => setBookingMeta({...bookingMeta, tripType: e.target.value})} className="flex-1 bg-white p-3 rounded-xl text-xs font-bold border border-blue-100"><option>One-Way</option><option>Round-Trip</option></select>
                       <select onChange={e => setBookingMeta({...bookingMeta, vehicleType: e.target.value})} className="flex-1 bg-white p-3 rounded-xl text-xs font-bold border border-blue-100"><option>AC Sedan</option><option>SUV</option><option>Traveler</option></select>
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                       <input placeholder="Source" onChange={e => setBookingMeta({...bookingMeta, source: e.target.value})} className="bg-white p-3 rounded-xl text-xs font-bold border border-blue-100 uppercase" />
                       <input placeholder="Destination" onChange={e => setBookingMeta({...bookingMeta, destination: e.target.value})} className="bg-white p-3 rounded-xl text-xs font-bold border border-blue-100 uppercase" />
                    </div>
                 </div>
               )}
               ...
               <div className="space-y-3">
                 {selectedStore.services?.map((s, i) => (
                   <div key={i} onClick={() => { if(cart.find(c => c.name === s.name)) setCart(cart.filter(c => c.name !== s.name)); else setCart([...cart, s]); }} className={`p-5 rounded-3xl border-2 transition-all flex justify-between items-center ${cart.find(c => c.name === s.name) ? 'border-emerald-600 bg-emerald-50 shadow-md scale-[1.02]' : 'border-gray-50 bg-gray-50'}`}>
                     <div>
                        <p className="text-[7px] font-black text-emerald-600 uppercase mb-1">{s.category}</p>
                        <p className="font-bold text-sm leading-none">{s.name}</p>
                        <p className="text-[8px] text-gray-400 mt-1 uppercase font-bold">{s.duration}</p>
                     </div>
               ...
               <div className="bg-white p-6 rounded-[3rem] shadow-sm border border-gray-100 animate-in slide-in-from-bottom-4">
                 <div className="space-y-4">
                   <select onChange={e => setBookingMeta({...bookingMeta, resourceName: e.target.value})} className="w-full bg-gray-50 p-4 rounded-2xl outline-none font-bold text-sm">
                      <option value="">{selectedStore.category === 'clinic' ? 'Any Doctor' : selectedStore.category === 'travel' ? 'Any Driver' : 'Any Specialist'}</option>
                   ...
                   <input placeholder="Mobile Number" type="tel" value={bookingMeta.custPhone} onChange={e => setBookingMeta({...bookingMeta, custPhone: e.target.value})} className="w-full bg-gray-50 p-4 rounded-2xl font-bold text-sm outline-none uppercase" />
                   <button onClick={() => setView('payment')} className="w-full bg-emerald-600 text-white py-5 rounded-3xl font-black shadow-xl active:scale-95 transition-all">Reserve Now</button>
                 </div>
              </div>
            )}
           </div>
        ...
        {view === 'payment' && (
           <div className="pt-4 space-y-6 animate-in zoom-in-95">
              <div className="text-center mb-8"><Lucide.Lock className="mx-auto text-emerald-600 mb-4" size={40}/><h2 className="text-2xl font-black uppercase tracking-tighter italic">Secured Checkout</h2></div>
              <div className="bg-white p-8 rounded-[3.5rem] shadow-xl border border-gray-100 space-y-8">
                 <div className="flex justify-between items-center pb-4 border-b border-gray-50">
                    <p className="font-bold text-emerald-900 uppercase">{selectedStore?.name}</p><p className="text-3xl font-black text-emerald-600 tracking-tighter">₹{cart.reduce((a, b) => a + Number(b.price), 0)}</p>
                 </div>
                 <div className="grid grid-cols-2 gap-3 opacity-60">
                    <button onClick={() => alert("UPI maintenance. Please use 'Cash'.")} className="flex flex-col items-center p-5 rounded-[2rem] border-2 border-gray-100 bg-gray-50 active:scale-90 transition-all"><Lucide.Smartphone size={24}/><span className="text-[8px] font-black mt-2 uppercase tracking-widest">UPI Pay</span></button>
                    <button onClick={() => alert("Card gateway maintenance. Please use 'Cash'.")} className="flex flex-col items-center p-5 rounded-[2rem] border-2 border-gray-100 bg-gray-50 active:scale-90 transition-all"><Lucide.CreditCard size={24}/><span className="text-[8px] font-black mt-2 uppercase tracking-widest">Debit</span></button>
                 </div>
                 <button onClick={() => { setBookingMeta({...bookingMeta, paymentMethod: 'Cash'}); handleBooking(); }} className="w-full p-6 rounded-[2.5rem] border-2 border-emerald-600 bg-emerald-50 flex items-center justify-center gap-3 active:scale-95 transition-all shadow-lg"><Lucide.Wallet className="text-emerald-600"/><span className="text-sm font-black uppercase tracking-widest text-emerald-900">Pay at Store</span></button>
              </div>
           </div>
        )}

        {/* VIEW: BUSINESS TAB (PURE APPLICATION) */}
        {view === 'vendor_portal' && (
           <div className="pt-4 space-y-6 animate-in slide-in-from-bottom-4">
              <div className="text-center mb-8">
                 <Lucide.Store size={48} className="mx-auto text-emerald-600 mb-2" />
                 <h2 className="text-3xl font-black uppercase tracking-tighter italic">Join The App</h2>
                 <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">Apply to list your Chiplun business</p>
              </div>
              <div className="bg-white p-7 rounded-[3.5rem] shadow-xl space-y-4 border border-gray-50">
                 <input placeholder="Official Shop Name" value={regForm.bizName} onChange={e => setRegForm({...regForm, bizName: e.target.value})} className="w-full bg-gray-50 p-4 rounded-xl font-bold text-sm outline-none border-2 border-transparent focus:border-emerald-500 uppercase" />
                 <input placeholder="Exact Location (Area)" value={regForm.addr} onChange={e => setRegForm({...regForm, addr: e.target.value})} className="w-full bg-gray-50 p-4 rounded-xl font-bold text-sm outline-none border-2 border-transparent focus:border-emerald-500 uppercase" />
                 ...
                 <button onClick={async () => {
                    if(!regForm.bizName || !regForm.addr) return alert("All fields are mandatory");
                    ...
                    setView('home');
                 }} className="w-full bg-emerald-600 text-white py-5 rounded-[2rem] font-black shadow-xl uppercase tracking-widest active:scale-95 transition-all">Submit Application</button>
              </div>
           </div>
        )}

        {/* VIEW: ACCOUNT PORTAL (LOGIN SCREEN) */}
        {view === 'account_portal' && (
           <div className="pt-20 space-y-6 animate-in zoom-in-95 duration-500">
              <div className="text-center mb-8">
                 <Lucide.UserCircle size={60} className="mx-auto text-emerald-600 mb-2"/>
                 <h2 className="text-3xl font-black tracking-tighter uppercase italic">Merchant Hub</h2>
                 <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">Verify Secure ID to enter</p>
              </div>
              <div className="bg-white p-8 rounded-[3.5rem] shadow-2xl space-y-4 border border-gray-100">
                 ...
                 <input type="password" placeholder="Passcode" value={vendorLogin.pass} onChange={e => setVendorLogin({...vendorLogin, pass: e.target.value})} className="w-full bg-gray-50 p-4 rounded-2xl font-bold text-sm outline-none" />
                 <button onClick={handleVendorLogin} className="w-full bg-emerald-600 text-white py-5 rounded-[2rem] font-black shadow-xl active:scale-95 transition-all uppercase tracking-widest">Authorize Session</button>
              </div>
           </div>
        )}

        {/* VIEW: MERCHANT DASHBOARD (After Login) */}
        {view === 'dashboard' && isSessionVerified && profile.role === 'vendor' && (
          <div className="pt-4 space-y-6 animate-in slide-in-from-bottom-8">
            <div className="flex justify-between items-center"><h2 className="text-2xl font-black text-emerald-900 uppercase italic tracking-tighter">{profile.businessName}</h2><button onClick={() => { setIsSessionVerified(false); setView('home'); }} className="bg-rose-50 px-4 py-2 rounded-xl text-rose-500 font-black text-[10px] active:scale-90 transition-all">LOGOUT</button></div>

            <section className="bg-white p-7 rounded-[3rem] shadow-sm border border-gray-100 space-y-5">
               <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest flex items-center font-black"><Lucide.Edit size={14} className="mr-2 text-emerald-600"/> Modify Storefront</h3>
               <input placeholder="Cover Image URL" value={editForm.image} onChange={e => setEditForm({...editForm, image: e.target.value})} className="w-full bg-gray-50 p-4 rounded-xl font-bold text-xs outline-none" />
               <input placeholder="Shop Name" value={editForm.name} onChange={e => setEditForm({...editForm, name: e.target.value})} className="w-full bg-gray-50 p-4 rounded-xl font-bold text-xs outline-none uppercase" />
               <button onClick={async () => { await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'stores', profile.businessId), editForm); alert("Updated!"); }} className="w-full bg-emerald-600 text-white py-3 rounded-xl font-black uppercase text-[10px]">Save Changes</button>
            </section>
            ...
            <section className="bg-white p-7 rounded-[3rem] shadow-sm border border-gray-100">
               ...
               <div className="space-y-3 mb-6">
                 {myStore?.services?.map((s, i) => (<div key={i} className="flex justify-between items-center p-4 bg-gray-50 rounded-2xl font-bold text-sm"><div><p className="text-[8px] uppercase text-emerald-600 font-black">{s.category}</p><p className="uppercase">{s.name}</p><p className="text-gray-400 text-[10px]">₹{s.price} • {s.duration}</p></div><button onClick={() => { updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'stores', profile.businessId), { services: myStore.services.filter((_, idx) => idx !== i) }); }} className="text-rose-400 p-2.5 bg-white rounded-xl shadow-sm"><Lucide.Trash2 size={16}/></button></div>))}
               </div>
               <div className="space-y-3 pt-4 border-t border-gray-50">
                  <input placeholder="Service Title" value={newSvc.name} onChange={e => setNewSvc({...newSvc, name: e.target.value})} className="w-full bg-gray-50 p-3 rounded-xl text-xs outline-none font-bold uppercase" />
                  <div className="grid grid-cols-2 gap-2">
                     ...
                     <input placeholder="Duration" value={newSvc.duration} onChange={e => setNewSvc({...newSvc, duration: e.target.value})} className="bg-gray-50 p-3 rounded-xl text-xs outline-none font-bold uppercase" />
                  </div>
                  <input placeholder="Section (e.g. PREMIUM)" value={newSvc.category} onChange={e => setNewSvc({...newSvc, category: e.target.value})} className="w-full bg-gray-50 p-3 rounded-xl text-xs outline-none font-bold uppercase tracking-widest" />
                  <button onClick={() => { if(!newSvc.name || !newSvc.price) return; updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'stores', profile.businessId), { services: [...(myStore.services || []), newSvc] }); setNewSvc({name:'', price:'', duration:'30 min', category:'General'}); }} className="w-full bg-emerald-600 text-white py-3 rounded-xl font-black uppercase text-[10px]">Add To Menu</button>
               </div>
            </section>
            
            <button onClick={() => updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'stores', profile.businessId), { isLive: !myStore?.isLive })} className={`w-full py-5 rounded-[2.5rem] font-black uppercase shadow-2xl active:scale-95 ${myStore?.isLive ? 'bg-rose-500 text-white' : 'bg-emerald-600 text-white'}`}>
               {myStore?.isLive ? 'Switch to Offline' : 'Go Live on Explore'}
            </button>
          </div>
        )}

        {/* ... (Admin & Track logic kept stable) ... */}
        {view === 'admin_auth' && (
           <div className="pt-20 text-center animate-in zoom-in-95">
              <Lucide.Lock className="mx-auto text-emerald-600 mb-6" size={40}/>
              <h2 className="text-2xl font-black mb-10 tracking-tighter uppercase font-black tracking-widest">Admin Core</h2>
              ...
              <button onClick={() => { if(adminPinInput===ADMIN_PIN) setView('admin_panel'); else { alert("DENIED"); setAdminPinInput(''); } }} className="w-full bg-emerald-600 text-white py-5 rounded-[2rem] font-black shadow-2xl active:scale-95 transition-all">Verify Key</button>
           </div>
        )}

        {view === 'admin_panel' && (
          <div className="pt-4 space-y-6 pb-10">
            <h2 className="text-2xl font-black text-emerald-900 uppercase tracking-tighter italic font-black">Management</h2>
            <div className="space-y-4">
              {stores.map(s => (
                <div key={s.id} className="bg-white p-4 rounded-3xl border border-gray-100 flex items-center gap-4">
                   <div className="w-10 h-10 bg-emerald-50 rounded-xl flex items-center justify-center font-black text-emerald-600 shadow-inner uppercase">{s.name[0]}</div>
                   <div className="flex-1 font-bold text-sm uppercase leading-none">{s.name} <span className="block text-[8px] opacity-40 mt-1">{s.category}</span></div>
              ...
              <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1 mt-8">Application Inbox</h3>
              {requests.filter(r => r.status === 'pending').map(req => (
                  <div key={req.id} className="bg-amber-50 p-6 rounded-3xl space-y-4 border border-amber-100 animate-in fade-in">
                    ...
                    <div className="flex gap-2">
                      <input id={`id-${req.id}`} placeholder="Issue ID" className="flex-1 p-2 rounded-xl text-xs font-bold outline-none uppercase border border-amber-100" />
                      <input id={`pw-${req.id}`} placeholder="Key" className="w-20 p-2 rounded-xl text-xs font-bold outline-none border border-amber-100" />
                    </div>
                    <button onClick={async () => {
                        ...
                        await setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'vendor_creds', uid.toUpperCase()), { password: pass, uid: req.uid, businessName: req.bizName });
                        ...
                        alert("Approved!");
                      }} className="w-full bg-emerald-600 text-white py-3 rounded-xl font-black uppercase text-[10px] shadow-lg">Authorize Account</button>
                  </div>
                ))}
            </div>
          </div>
        ...
        {view === 'bookings' && (
           <div className="pt-4 space-y-6 animate-in slide-in-from-bottom-4">
              <h2 className="text-2xl font-black text-emerald-900 uppercase tracking-tighter italic font-black">Track Token</h2>
              ...
              {searchedBooking && (
                <div className="bg-emerald-600 text-white p-7 rounded-[3rem] shadow-2xl animate-in zoom-in-95">
                   <div className="flex justify-between items-start mb-6">
                      <div><p className="text-[10px] font-black uppercase opacity-60 font-black">Verified Booking</p><h3 className="text-xl font-black italic">{searchedBooking.storeName}</h3></div>
                   ...
                   <div className="space-y-4 text-sm font-bold pt-4 border-t border-white/10">
                      <p className="opacity-60 text-[8px] uppercase font-black">Schedule</p><p className="italic">{searchedBooking.date} • {searchedBooking.time}</p>
                      <p className="opacity-60 text-[8px] uppercase mt-2 font-black">Assigned To</p><p className="italic uppercase">{searchedBooking.resourceName || 'First Available'}</p>
                      <div className="flex justify-between items-center text-[10px] uppercase font-black"><span>Payment: {searchedBooking.paymentMethod}</span><span className="text-emerald-200 text-lg">₹{searchedBooking.totalPrice}</span></div>
                   </div>
        ...
        {view === 'confirmation' && (
           <div className="text-center pt-24 animate-in zoom-in-90 duration-700">
             <div className="w-24 h-24 bg-emerald-100 text-emerald-600 rounded-full flex items-center justify-center mx-auto mb-10 animate-bounce shadow-inner border border-emerald-200"><Lucide.CheckCircle2 size={56}/></div>
             <h2 className="text-4xl font-black text-emerald-900 mb-4 tracking-tighter uppercase font-black">Success!</h2>
             <div className="bg-white p-8 rounded-[3.5rem] shadow-xl border-2 border-emerald-50 mx-4 mb-10 text-center">
             ...
             <p className="text-gray-400 mb-14 px-12 leading-relaxed font-bold uppercase text-[10px] opacity-60 italic">"Save this ID. No registration required for customers."</p>
      ...
      {/* BOTTOM NAVIGATION (Updated Logic) */}
      <nav className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-white/90 backdrop-blur-2xl border-t border-gray-100 px-8 py-5 pb-9 flex justify-between items-center z-50 rounded-t-[3.5rem] shadow-2xl">
        ...
        <button onClick={() => setView(isSessionVerified ? 'dashboard' : 'account_portal')} className={`flex flex-col items-center gap-1.5 transition-all ${['account_portal', 'dashboard'].includes(view) ? 'text-emerald-600 scale-110' : 'text-gray-300'}`}><Lucide.User size={22} /><span className="text-[9px] font-black uppercase tracking-tighter">Account</span></button>
      </nav>
