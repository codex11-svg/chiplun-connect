...
export default function App() {
  ...
  useEffect(() => {
    if (!user) return;
    const unsubs = [
      onSnapshot(doc(db, 'artifacts', appId, 'users', user.uid, 'profile', 'data'), (s) => s.exists() ? setProfile(s.data()) : setDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'profile', 'data'), { role: 'customer', uid: user.uid })),
  ...
  const handleImg = async (e) => {
    ...
    if (!file || file.size > 1000000) return notify("Image too heavy", "error");
  ...
  if (loading) return <div className="h-screen flex items-center justify-center bg-emerald-600 text-white font-black animate-pulse uppercase tracking-[0.5em]">CHIPLUNCONNECT</div>;
      ...
      <header className="bg-emerald-600 text-white p-6 pb-12 rounded-b-[3.5rem] shadow-lg sticky top-0 z-50">
        <div className="flex justify-between items-center mb-6">
          <div onClick={() => setView('home')} className="cursor-pointer active:scale-95 transition-all">
            ...
            <p className="text-[10px] font-bold opacity-80 uppercase tracking-widest mt-1">Final Master Build</p>
          </div>
          <div className="flex gap-2">
            <button onClick={() => setView('admin')} className="w-10 h-10 rounded-2xl flex items-center justify-center bg-white/10 border border-white/10 active:bg-white active:text-emerald-600"><Lucide.Shield size={18} /></button>
            <button onClick={() => setView(profile.role === 'vendor' ? 'merchant' : 'business')} className={`w-10 h-10 rounded-2xl flex items-center justify-center border transition-all ${view === 'merchant' ? 'bg-white text-emerald-600' : 'bg-white/10'}`}>
        ...
        {view === 'home' && (
          <div className="relative animate-in slide-in-from-top-4">
            ...
            <input type="text" placeholder="Search verified services..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full bg-white/10 border border-white/20 rounded-2xl py-4 pl-12 pr-4 text-white placeholder-emerald-200 outline-none" />
          </div>
        )}
      </header>
      ...
      <main className="flex-1 -mt-6 px-4 pb-32 z-10">
        ...
        {view === 'home' && (
          <div className="space-y-8 pt-2">
            <div className="grid grid-cols-4 gap-3">
               <button onClick={() => setSearch('salon')} className="flex flex-col items-center gap-2 animate-in fade-in"><div className="p-4 bg-white rounded-2xl border border-slate-100 shadow-sm"><Lucide.Scissors size={20} className="text-rose-500"/></div><span className="text-[9px] font-black uppercase text-slate-400">Salon</span></button>
               <button onClick={() => setSearch('travel')} className="flex flex-col items-center gap-2 animate-in fade-in"><div className="p-4 bg-white rounded-2xl border border-slate-100 shadow-sm"><Lucide.Bus size={20} className="text-blue-500"/></div><span className="text-[9px] font-black uppercase text-slate-400">Travel</span></button>
               <button onClick={() => setView('track')} className="flex flex-col items-center gap-2 animate-in fade-in"><div className="p-4 bg-white rounded-2xl border border-slate-100 shadow-sm"><Lucide.Ticket size={20} className="text-emerald-500"/></div><span className="text-[9px] font-black uppercase text-slate-400">Tracker</span></button>
            </div>
        ...
        {view === 'track' && (
           <div className="pt-6 space-y-6 animate-in slide-in-from-bottom-8 px-1">
              <h2 className="text-2xl font-black text-emerald-900 uppercase italic tracking-tighter text-center leading-none">Status Tracker</h2>
              <div className="bg-white p-6 rounded-[2.5rem] shadow-xl border border-slate-100 space-y-4">
                 <input placeholder="Enter CH-XXXX Token ID" value={trackInput} onChange={e => setTrackInput(e.target.value.toUpperCase())} className="w-full bg-slate-50 border p-5 rounded-2xl text-lg font-black text-center outline-none tracking-widest" />
                 <button onClick={() => { const b = bookings.find(x => x.displayId === trackInput); if(b) setActiveReceipt(b); else notify("Token ID invalid", "error"); }} className="w-full py-5 bg-emerald-600 text-white rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-lg">Check Position</button>
              </div>

              {liveRankData && !liveRankData.error ? (
                <div className="bg-white p-8 rounded-[3.5rem] shadow-2xl border-t-8 border-emerald-500 text-center space-y-6 animate-in zoom-in-95">
                   <div className="w-20 h-20 bg-emerald-50 text-emerald-600 rounded-full flex items-center justify-center mx-auto text-3xl font-black italic shadow-inner border border-emerald-100">{liveRankData.pos}</div>
                   <div>
                      <h3 className="text-3xl font-black tracking-tighter uppercase italic leading-none">{liveRankData.displayId}</h3>
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">Passenger: {liveRankData.name}</p>
                   </div>
                   <div className="grid grid-cols-2 gap-3">
                      <div className="bg-slate-50 p-4 rounded-3xl border border-slate-100">
                         <p className="text-[8px] font-black text-slate-400 uppercase">Wait Rank</p>
                         <p className="text-xl font-black text-emerald-600">{liveRankData.pos === 1 ? "NEXT" : (liveRankData.pos - 1) + " Ahead"}</p>
                      </div>
                      <div className="bg-slate-50 p-4 rounded-3xl border border-slate-100">
                         <p className="text-[8px] font-black text-slate-400 uppercase">Est. Minutes</p>
                         <p className="text-xl font-black text-blue-600">~{liveRankData.wait}m</p>
                      </div>
                   </div>
                   <button onClick={() => setView('home')} className="w-full py-4 bg-slate-900 text-white rounded-2xl font-black uppercase text-[10px] tracking-widest">Back to explorer</button>
                </div>
              ) : activeReceipt && <p className="text-center text-rose-500 font-black uppercase text-[10px] py-10 tracking-widest italic opacity-50">Booking expired or finished.</p>}
           </div>
        )}

        {/* VIEW: MERCHANT (CATEGORICALLY PROTECTED) */}
        {view === 'merchant' && mData && (
          <div className="pt-6 space-y-6 animate-in slide-in-from-bottom-8 px-1">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-slate-900 p-6 rounded-[2.5rem] text-white flex flex-col justify-between shadow-lg h-44">
                 <div><p className="text-[8px] font-black uppercase opacity-50 mb-1 tracking-widest">LIVE STATUS</p><p className="text-lg font-black uppercase italic tracking-tighter leading-none">{mData.store.isLive ? 'Online' : 'Offline'}</p></div>
              ...
              <div className="bg-white p-2 rounded-[2.5rem] border border-slate-100 shadow-lg relative overflow-hidden h-44 group">
                 <img src={mData.store.image} className="w-full h-full object-cover rounded-[2rem] opacity-50 transition-all" alt="Profile" />
                 <label className="absolute inset-0 flex flex-col items-center justify-center cursor-pointer">
                    <Lucide.Camera size={24} className="text-slate-800" />
                    <span className="text-[8px] font-black uppercase mt-1 text-slate-800">Change Logo</span>
            ...
            {mTab === 'ledger' && (
               <section className="space-y-4 pb-20 px-1">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-white p-6 rounded-[2rem] border border-slate-100 text-center shadow-sm"><p className="text-[8px] font-black text-slate-400 mb-1 uppercase tracking-widest">Earnings</p><p className="text-2xl font-black text-emerald-600 italic">₹{mData.rev}</p></div>
                    <div className="bg-white p-6 rounded-[2rem] border border-slate-100 text-center shadow-sm"><p className="text-[8px] font-black text-slate-400 mb-1 uppercase tracking-widest">Waiting</p><p className="text-2xl font-black text-blue-600 italic">{mData.queue.length}</p></div>
                  </div>
                  {(mData.queue || []).map((b, i) => (
                    <div key={i} className="bg-white p-5 rounded-[2rem] border-l-8 border-emerald-500 shadow-sm space-y-4 animate-in slide-in-from-left-4">
                      <div className="flex justify-between items-start">
                        <div>
                           <p className="font-black text-sm uppercase italic leading-none">{b.name || 'Anonymous'}</p>
                      ...
                      <div className="flex gap-2">
                        <button onClick={() => window.open(`tel:${b.phone}`)} className="flex-1 p-3 bg-blue-50 text-blue-600 rounded-xl flex items-center justify-center gap-2 active:scale-95"><Lucide.Phone size={16}/><span className="text-[9px] font-black uppercase tracking-widest">Call</span></button>
                        ...
                        <button onClick={async () => { if(window.confirm("Cancel? Seats restore instantly.")) await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'bookings', b.id)); }} className="p-3 bg-rose-50 text-rose-500 rounded-xl active:scale-90"><Lucide.X size={18}/></button>
                      </div>
                    </div>
                  ))}
               </section>
            ...
            {mTab === 'assets' && (
              <div className="bg-white p-8 rounded-[3rem] border border-slate-100 space-y-6 mx-1">
                <button onClick={() => {
                   ...
                   const t = mData.store.category === 'travel' ? prompt("Fixed Start Time (e.g. 09:30 AM):") : "";
                   const c = mData.store.category === 'travel' ? prompt("Car Total Seats:") : 1;
                ...
                <div className="space-y-3">
                   {(mData.store.resources || []).map((r, i) => (
                     <div key={i} className="flex justify-between items-center p-4 bg-slate-50 rounded-2xl border border-slate-100 shadow-inner">
                        <div><p className="font-black text-xs uppercase italic leading-none">{r.name}</p>{r.time && <p className="text-[8px] font-black text-slate-400 uppercase mt-1">Leaves: {r.time} • Seats: {r.capacity}</p>}</div>
            ...
            {mTab === 'prices' && (
               <div className="bg-white p-8 rounded-[3rem] border border-slate-100 space-y-6 mx-1 animate-in fade-in">
                  <button onClick={() => {
                     const n = prompt("Label (e.g. Haircut / SUV Ride):");
        ...
        {/* VIEW: ADMIN Master Terminal (FIXED PIN GATE) */}
        {view === 'admin' && (
          <div className="pt-10 space-y-6 animate-in fade-in px-2">
             <div className="flex justify-between items-center px-1">
                <h2 className="text-2xl font-black text-rose-600 uppercase italic tracking-tighter leading-none">Admin Console</h2>
             ...
             {!adminAuth ? (
               <div className="bg-white p-8 rounded-[3rem] shadow-2xl border border-rose-100 space-y-4 text-center">
                 ...
                 <input type="password" placeholder="System Terminal PIN" value={adminPin} onChange={(e) => setAdminPin(e.target.value)} className="w-full bg-slate-50 border p-5 rounded-2xl border font-black text-center text-lg outline-none tracking-widest focus:border-rose-500" />
                 <button onClick={() => { if (adminPin === ADMIN_PIN) setAdminAuth(true); else notify("PIN Verification Failed", "error"); }} className="w-full bg-rose-600 text-white py-5 rounded-2xl font-black shadow-xl uppercase active:scale-95 transition-all tracking-[0.2em]">Authorize Terminal</button>
               </div>
             ) : (
                    ...
                    <div key={r.id} className="bg-white p-6 rounded-[2.5rem] border border-slate-100 space-y-4 shadow-sm">
                        ...
                        <div className="flex gap-2">
                          ...
                          <button onClick={async () => {
                             const mId = prompt("Assigned ID:");
                             const key = prompt("Assigned Key:");
                             if (mId && key) {
                               setIsProcessing(true);
                          ...
                          }} className="flex-[2] py-3 bg-emerald-600 text-white rounded-2xl font-black text-[9px] uppercase shadow-lg active:scale-95 tracking-widest italic">Provision</button>
                        </div>
                    </div>
                 )) : (stores || []).map(s => (
        ...
        {/* VIEW: DETAIL (HARD LOGIC ISOLATION) */}
              ...
              <div className="bg-white p-6 rounded-[3rem] shadow-sm border border-slate-100 animate-in slide-in-from-bottom-6 space-y-5">
                  ...
                  <div className="space-y-2">
                    ...
                    {(selStore.resources || []).map(r => {
                      const { count, left } = checkCap(selStore.id, r.id, r.capacity);
                      ...
                      return (
                        <div key={r.id} onClick={() => !isFull && setBookForm({...bookForm, resId: r.id})} className={`p-4 rounded-2xl border-2 transition-all cursor-pointer flex justify-between items-center ${bookForm.resId === r.id ? 'border-emerald-600 bg-emerald-50' : 'border-slate-50 opacity-60'} ${isFull ? 'opacity-30 cursor-not-allowed grayscale' : ''}`}>
                          <div className="text-left"><p className="font-black text-[10px] uppercase italic tracking-tight leading-none">{r.name}</p>{r.time && <p className="text-[8px] font-black text-blue-500 mt-1 uppercase tracking-widest">Timing: {r.time}</p>}</div>
                          <span className={`text-[8px] font-black uppercase tracking-tighter ${isFull ? 'text-rose-500' : 'text-emerald-600'}`}>{selStore.category === 'salon' ? `Wait Rank: ${count + 1}` : isFull ? 'FULL' : `${left} S Left`}</span>
                        </div>
                      );
                    })}
                  </div>
                  ...
                  {isCheatTime ? (
                     <div className="p-4 bg-rose-50 border border-rose-100 rounded-2xl flex items-center gap-3 text-rose-500">
                        ...
                        <span className="text-[10px] font-black uppercase tracking-tighter">Cannot select a past time for today!</span>
                     </div>
                  ) : (
        ...
        {view === 'business' && profile.role !== 'vendor' && (
          <div className="pt-6 space-y-6 animate-in slide-in-from-bottom-8 px-1">
            ...
            <div className="flex bg-slate-200 p-1.5 rounded-[1.8rem] shadow-inner border border-slate-300 relative mx-1">
               ...
               <button onClick={() => setHubMode('register')} className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest relative z-10 transition-colors ${hubMode === 'register' ? 'text-white' : 'text-slate-500'}`}>New Apply</button>
               <button onClick={() => setHubMode('login')} className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest relative z-10 transition-colors ${hubMode === 'login' ? 'text-white' : 'text-slate-500'}`}>Login</button>
            </div>
            {hubMode === 'register' ? (
              <div className="bg-white p-8 rounded-[3rem] shadow-xl border border-slate-100 space-y-4 mx-1">
                ...
                <input value={regForm.address} onChange={e => setRegForm({...regForm, address: e.target.value})} className="w-full bg-slate-50 border p-4 rounded-xl text-[10px] font-black uppercase outline-none focus:border-emerald-500 shadow-inner" placeholder="Area Location" />
      ...
      {showConfirm && (
        <div className="fixed inset-0 bg-emerald-600 z-[1001] flex items-center justify-center p-6 animate-in zoom-in-95 duration-200 text-white text-center">
           <div className="space-y-8">
              ...
              <div className="space-y-2">
                 <h3 className="text-4xl font-black uppercase italic tracking-tighter leading-none">Confirm Booking?</h3>
