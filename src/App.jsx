...
const appId = "chiplun-pro-v37-enterprise"; 
...
export default function App() {
  ...
  const todayRevenue = useMemo(() => {
    if (!profile.businessId) return 0;
    return allBookings
      .filter(b => b.storeId === profile.businessId && b.status === 'completed')
  ...
  const isSpecialistBusy = (name, date, time) => {
    ...
    return allBookings.some(b => b.storeId === selectedStore?.id && b.resourceName === name && b.date === date && b.time === time && b.status !== 'completed');
  };
  ...
  if (loading) return <div className="h-screen flex items-center justify-center bg-emerald-600 text-white font-black animate-pulse">CHIPLUNCONNECT MASTER</div>;
  ...
  return (
    <div className="max-w-md mx-auto bg-gray-50 min-h-screen flex flex-col shadow-2xl relative font-sans text-gray-900 overflow-x-hidden">
      ...
      <header className="bg-emerald-600 text-white p-6 pb-12 rounded-b-[4rem] shadow-lg sticky top-0 z-50">
        <div className="flex justify-between items-center mb-6">
          <div onClick={() => setView('home')} className="cursor-pointer active:scale-95 transition-all">
            ...
            <p className="text-[9px] font-bold opacity-70 uppercase tracking-widest italic font-black">Live Enterprise MH-08</p>
          </div>
        ...
        {view === 'home' && (
          <div className="relative animate-in slide-in-from-top-2">
            ...
            <input type="text" placeholder="Search businesses or areas..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-white/10 border border-white/20 rounded-2xl py-3.5 pl-12 pr-4 text-white placeholder-emerald-200 outline-none" />
          </div>
        )}
      </header>
      ...
      <main className="flex-1 -mt-6 px-4 pb-32 overflow-y-auto z-10">
        ...
        {view === 'home' && (
          <div className="space-y-8 pt-2 text-center">
             ...
             <section className="animate-in slide-in-from-bottom-4 pb-10 text-left">
               <h2 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-4 ml-1 italic font-black">Live Marketplace</h2>
             ...
             <section className="bg-white p-6 rounded-[3rem] shadow-sm border border-gray-100">
               ...
               {selectedStore.category === 'travel' && (
                  <div className="space-y-4 mb-8 p-6 bg-blue-50/50 rounded-[2.5rem] border border-blue-100 animate-in fade-in">
                    <p className="text-[10px] font-black text-blue-600 uppercase tracking-widest text-center italic">Route Specs</p>
              ...
              {!isSessionVerified ? (
                 <div className="animate-in zoom-in-95 pt-10">
                    <div className="flex items-center justify-between mb-8">
                       <h2 className="text-3xl font-black uppercase tracking-tighter italic text-emerald-900">Account</h2>
                       <Lucide.UserCircle size={40} className="text-emerald-600 p-2 bg-emerald-50 rounded-2xl shadow-inner"/>
                    </div>
                    
                    {/* PERSISTENT TOKEN WALLET */}
                    <section className="space-y-4 mb-10 text-left">
                       ...
                       {myBookings.length > 0 ? (
                         <div className="space-y-3">
                            {myBookings.map((b, i) => (
                               <div key={i} className="bg-white p-5 rounded-[2rem] border border-gray-100 flex justify-between items-center shadow-sm hover:border-emerald-200 transition-all cursor-pointer" onClick={() => {setTrackInput(b.displayId); setView('track'); handleTrackToken();}}>
                                  <div><p className="font-bold text-sm text-gray-800 uppercase tracking-tight">{b.storeName}</p><p className="text-[8px] text-gray-400 uppercase font-black">{b.date} • {b.time}</p></div>
                                  <span className="bg-emerald-50 text-emerald-600 px-3 py-1 rounded-xl text-[10px] font-black tracking-widest italic shadow-sm">{b.displayId}</span>
                               </div>
                            ))}
                         </div>
                       ) : (
                         <div className="py-12 bg-white rounded-[2.5rem] border border-gray-100 text-center space-y-2">
                            ...
                            <p className="text-gray-300 font-black uppercase text-[8px] tracking-widest">No Tokens Yet</p>
                         </div>
                       )}
                    </section>
                    ...
                    <section className="bg-gray-100 p-6 rounded-[3rem] border border-gray-200 mt-12">
                       <h3 className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-4 flex items-center italic font-black uppercase"><Lucide.ShieldCheck size={14} className="mr-2"/> Merchant Entrance</h3>
                       <div className="space-y-3">
                          ...
                          <button onClick={handleVendorLogin} className="w-full bg-emerald-600 text-white py-4 rounded-2xl font-black shadow-xl active:scale-95 transition-all text-xs uppercase tracking-widest">Authorize Session</button>
                       </div>
                    </section>
                 </div>
              ) : (
                <div className="space-y-6 animate-in slide-in-from-bottom-8 pb-20">
                   ...
                   {/* MERCHANT PULSE (ANALYTICS) */}
                   <div className="grid grid-cols-2 gap-3">
                      <div className="bg-white p-5 rounded-[2.5rem] border border-gray-100 shadow-sm flex flex-col items-center">
                         <p className="text-[8px] font-black uppercase text-gray-400 mb-1">Today's Revenue</p>
                      ...
                      <div className="space-y-3 max-h-72 overflow-y-auto pr-1">
                        {merchantBookings.length > 0 ? merchantBookings.sort((a,b) => b.timestamp - a.timestamp).map((b, i) => (
                           <div key={i} className="bg-white/10 p-4 rounded-2xl border border-white/5 space-y-1 animate-in fade-in">
                              ...
                              <div className="bg-emerald-700/50 px-3 py-1 rounded-xl inline-block mt-1"><p className="text-[9px] font-black text-emerald-50 uppercase tracking-tighter italic font-black">Booked: {b.services?.map(s => s.name).join(', ')}</p></div>
                   ...
                   <section className="bg-white p-7 rounded-[3rem] shadow-sm border border-gray-100">
                      <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-6 font-black uppercase tracking-widest italic">Service Menu</h3>
                      ...
                      <div className="space-y-3 pt-6 border-t border-gray-100">
                        <input placeholder="New Service Title" value={newSvc.name} onChange={e => setNewSvc({...newSvc, name: e.target.value})} className="w-full bg-gray-50 p-3 rounded-xl text-xs outline-none uppercase font-bold border shadow-inner" />
                        <div className="grid grid-cols-2 gap-2">
                   ...
                   <section className="bg-white p-7 rounded-[3rem] shadow-sm border border-gray-100">
                      <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-6 font-black uppercase tracking-widest italic text-left">
                        {myStore?.category === 'travel' ? 'Manage Drivers & Vehicles' : myStore?.category === 'clinic' ? 'Manage Doctors' : myStore?.category === 'repair' ? 'Manage Technicians' : 'Staff Management'}
                      </h3>
                      <div className="flex flex-wrap gap-2 mb-6">
                        {myStore?.staff?.map((n, i) => (<div key={i} className="bg-emerald-50 text-emerald-700 px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-2">{n} <button onClick={async () => { const s_ = stores.find(st => st.ownerId === profile.businessId); await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'stores', profile.businessId), { staff: s_.staff.filter((_, idx) => idx !== i) }); }}><Lucide.X size={12}/></button></div>))}
                      </div>
                      <div className="flex gap-2"><input placeholder={myStore?.category === 'travel' ? 'Driver/Vehicle Details' : 'Professional Name'} value={newStaff} onChange={e => setNewStaff(e.target.value)} className="flex-1 bg-gray-50 p-3 rounded-lg font-bold text-sm outline-none border shadow-inner" /><button onClick={async () => { if(!newStaff) return; const s = stores.find(st => st.ownerId === profile.businessId); await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'stores', profile.businessId), { staff: [...(s.staff || []), newStaff] }); setNewStaff(''); }} className="bg-emerald-600 text-white p-3 rounded-lg active:scale-95 shadow-lg"><Lucide.Plus size={18}/></button></div>
                   </section>
        ...
        {view === 'track' && (
           <div className="pt-4 space-y-6 animate-in slide-in-from-bottom-4 px-4 pb-20">
              ...
              {searchedBooking ? (
                <div className="bg-emerald-600 text-white p-8 rounded-[3.5rem] shadow-2xl animate-in zoom-in-95 relative overflow-hidden text-left shadow-emerald-100">
                   <div className="absolute top-0 right-0 p-4 opacity-10"><Lucide.CheckCircle size={80}/></div>
                   <div className="flex justify-between items-start mb-8 relative z-10">
                      <div><p className="text-[10px] font-black uppercase opacity-60 tracking-widest">Verified Slot</p><h3 className="text-xl font-black italic uppercase tracking-tight">{searchedBooking.storeName}</h3></div>
                ...
                <p className="text-center py-20 text-gray-300 text-[10px] font-black uppercase tracking-widest animate-pulse italic font-black leading-relaxed">Privacy Guard Active • Use unique token</p>
              )}
           </div>
        )}

        {/* VIEW: ADMIN Portal (FIXED Overlapping) */}
        {view === 'admin' && (
        ...
        {view === 'admin_panel' && (
          <div className="pt-4 space-y-8 pb-32 px-4 text-left flex flex-col h-full">
            <div className="flex justify-between items-center mb-2">
               <h2 className="text-2xl font-black text-emerald-900 uppercase italic tracking-tighter">Enterprise Hub</h2>
              ...
              <div className="space-y-4 max-h-[35vh] overflow-y-auto pr-1">
                {stores.map(s => (
                  <div key={s.id} className="bg-white p-5 rounded-[2.5rem] border border-gray-100 flex items-center gap-4 animate-in fade-in shadow-sm">
                    ...
                    <button onClick={() => setDeleteTarget(s)} className="text-rose-400 p-3 active:bg-rose-50 rounded-xl transition-all shadow-sm"><Lucide.Trash2 size={24}/></button>
                  </div>
                ))}
                {stores.length === 0 && <p className="text-center py-6 text-[9px] text-gray-300 font-black uppercase italic">No active businesses</p>}
              </div>
            </div>
              ...
              <div className="space-y-4 pr-1">
                 {requests.filter(r => r.status === 'pending').map(req => (
                    <div key={req.id} className="bg-amber-50 p-7 rounded-[3rem] space-y-4 border border-amber-100 shadow-sm animate-in fade-in text-left">
                       ...
                       <div className="flex gap-2">
                         <input id={`id-${req.id}`} placeholder="Assign ID" className="flex-1 p-3 rounded-xl text-xs font-bold outline-none uppercase border border-amber-200 shadow-inner" />
                         <input id={`pw-${req.id}`} placeholder="Passcode" className="w-24 p-3 rounded-xl text-xs font-bold outline-none border border-amber-200 shadow-inner" />
                       </div>
                 ...
                 {requests.filter(r => r.status === 'pending').length === 0 && <p className="text-center py-6 text-[9px] text-gray-300 font-black uppercase italic">No pending requests</p>}
              </div>
            </div>
          </div>
        )}
        ...
        {view === 'onboarding' && (
           <div className="pt-4 space-y-6 animate-in slide-in-from-bottom-4 px-4 pb-20 text-center text-left">
              <div className="text-center mb-10">
                 ...
                 <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest text-center">Verification intake for Chiplun</p>
              </div>
              <div className="bg-white p-7 rounded-[3.5rem] shadow-xl space-y-4 border border-gray-100 text-left text-xs font-black uppercase">
                 ...
                 }} className="w-full bg-emerald-600 text-white py-5 rounded-[2.5rem] font-black shadow-xl uppercase active:scale-95 transition-all tracking-widest font-black uppercase italic">Submit Application</button>
              </div>
           </div>
        )}
        ...
        {view === 'confirmation' && (
           <div className="text-center pt-10 animate-in zoom-in-90 duration-700 pb-10 px-4">
             ...
             <h2 className="text-4xl font-black text-emerald-900 mb-6 uppercase tracking-tighter italic text-emerald-900 text-center">Reserved</h2>
             <div className="bg-emerald-600 text-white p-8 rounded-[3.5rem] shadow-2xl space-y-6 relative overflow-hidden text-center shadow-emerald-200">
                ...
                <div className="grid grid-cols-2 gap-4 border-t border-white/10 pt-6 font-black uppercase text-white">
                   <div className="text-center border-r border-white/10"><p className="text-[8px] opacity-60 font-black tracking-widest">Position</p><p className="text-2xl font-black">#{queueInfo.pos}</p></div>
                   <div className="text-center"><p className="text-[8px] opacity-60 font-black tracking-widest">Wait</p><p className="text-2xl font-black">+{queueInfo.delay}m</p></div>
                </div>
             </div>
             <button onClick={() => setView('home')} className="w-full bg-emerald-700 text-white py-5 rounded-[2.5rem] font-black shadow-2xl active:scale-95 uppercase tracking-widest mt-12 font-black italic">Return Home</button>
           </div>
        )}
      ...
      <nav className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-white/95 backdrop-blur-3xl border-t border-gray-100 px-8 py-5 pb-9 flex justify-between items-center z-[100] rounded-t-[3.5rem] shadow-2xl">
        ...
        <button onClick={() => setView('account')} className={`flex flex-col items-center gap-1.5 transition-all ${['account', 'merchant_hub', 'dashboard'].includes(view) ? 'text-emerald-600 scale-110' : 'text-gray-300'}`}><Lucide.UserCircle size={22} /><span className="text-[9px] font-black uppercase tracking-widest italic font-black">Account</span></button>
      </nav>
