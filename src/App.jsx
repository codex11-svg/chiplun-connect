...
export default function App() {
  ...
  const mySpot = useMemo(() => {
    ...
    const live = bookings.find(b => b.displayId === receipt.displayId);
  ...
  const adminApprove = async (req) => {
    const mid = prompt("Assign ID (CH-XXX):");
  ...
  if (loading) return <div className="h-screen flex items-center justify-center bg-emerald-600 text-white font-black animate-pulse uppercase tracking-[0.5em]">SYNCHRONIZING...</div>;
      ...
      <header className="bg-emerald-600 text-white p-6 pb-12 rounded-b-[3.5rem] shadow-lg sticky top-0 z-50">
        ...
        {view === 'home' && (
          <div className="relative animate-in slide-in-from-top-4 duration-500">
            ...
            <input type="text" placeholder="Find verified Chiplun partners..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full bg-white/10 border border-white/20 rounded-2xl py-4 pl-12 pr-4 text-white placeholder-emerald-200 outline-none focus:bg-white/20 transition-all shadow-inner" />
          </div>
        )}
      </header>
      ...
      <main className="flex-1 -mt-6 px-4 pb-32 z-10">
        ...
        {view === 'home' && (
          <div className="space-y-8 pt-2">
            <div className="grid grid-cols-4 gap-3 animate-in fade-in">
               ...
               <button onClick={() => notify("More categories coming soon", "info")} className="flex flex-col items-center gap-2 opacity-30"><div className="p-4 bg-white rounded-2xl border border-slate-100 shadow-sm"><Lucide.Plus size={20}/></div><span className="text-[9px] font-black uppercase text-slate-400">More</span></button>
            </div>
        ...
        {view === 'track' && (
           <div className="pt-6 space-y-6 animate-in slide-in-from-bottom-8 px-1">
              ...
              <div className="bg-white p-6 rounded-[2.5rem] shadow-xl border border-slate-100 space-y-4">
                 ...
                 <button onClick={() => { if(!trackId) notify("Enter Token", "error"); }} className="w-full py-5 bg-emerald-600 text-white rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-lg">Find My Rank</button>
              </div>
              ...
              {mySpot && !mySpot.error ? (
                <div className="bg-white p-8 rounded-[3.5rem] shadow-2xl border-t-8 border-emerald-500 text-center space-y-6 animate-in zoom-in-95">
                   ...
                   <button onClick={() => setView('home')} className="w-full py-4 bg-slate-900 text-white rounded-2xl font-black uppercase text-[10px] active:scale-95">Go to Home</button>
                </div>
        ...
        {view === 'detail' && selStore && (
          <div className="pt-4 space-y-6 animate-in slide-in-from-right-4 px-1 pb-32">
            <button onClick={() => setView('home')} className="flex items-center text-emerald-600 font-black text-[10px] uppercase tracking-widest px-2 active:scale-95 leading-none"><Lucide.ArrowLeft size={16} className="mr-2"/> Back explorer</button>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="space-y-2">
                      {(selStore.resources || []).map(r => {
                        const { count, left } = checkAvailability(selStore.id, r.id, r.capacity);
