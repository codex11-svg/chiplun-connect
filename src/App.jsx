...
export default function App() {
  ...
  const mData = useMemo(() => {
    ...
    const revenue = bookings.filter(x => x.storeId === profile.businessId && x.status === 'completed').reduce((a, c) => a + (Number(c.totalPrice) || 0), 0);
    return { store: s, queue: bPending, rev: revenue };
  }, [bookings, profile, stores]);

  const getSlotCap = (sId, rId, total) => {
  ...
  const handleFinalBooking = async () => {
    ...
    if (bookForm.date === today && bookForm.time < clock && selStore.category === 'salon') {
       notify("Cannot book past time!", "error");
    ...
    try {
      ...
      const payload = { ...bookForm, time: bookingTime, displayId: id, storeId: selStore.id, storeName: selStore.name, serviceName: selService.name, totalPrice: total, status: 'pending', timestamp: Date.now(), payment: 'Cash' };
      await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'bookings'), payload);
      setActiveReceipt(payload);
      setConfirmModal(false);
      setPayModal(false);
      setView('track');
    } catch (e) { notify("Booking failed", "error"); }
    setIsProcessing(false);
  };

  const handleImageUpload = async (e) => {
    const file = e.target.files[0];
    if (!file || file.size > 800000) return notify("Image too large", "error");
    const reader = new FileReader();
    reader.onloadend = async () => {
       await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'stores', profile.businessId), { image: reader.result });
       notify("Business Logo Updated!");
    };
      ...
      <header className="bg-emerald-600 text-white p-6 pb-12 rounded-b-[3.5rem] shadow-lg sticky top-0 z-50">
        <div className="flex justify-between items-center mb-6">
          <div onClick={() => setView('home')} className="cursor-pointer active:scale-95 transition-all">
            ...
            <p className="text-[10px] font-bold opacity-80 uppercase tracking-widest mt-1">V50 Final Supreme</p>
          </div>
          <button 
        ...
        {view === 'home' && (
          <div className="relative animate-in slide-in-from-top-4">
            ...
            <input type="text" placeholder="Search agencies or salons..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full bg-white/10 border border-white/20 rounded-2xl py-4 pl-12 pr-4 text-white placeholder-emerald-200 outline-none focus:bg-white/20 transition-all" />
          </div>
        )}
      </header>
      ...
      <main className="flex-1 -mt-6 px-4 pb-32 z-10">
        ...
        {view === 'home' && (
          <div className="space-y-8 pt-2">
            <div className="grid grid-cols-4 gap-3 animate-in fade-in">
               ...
               <button onClick={() => setView('admin')} className="flex flex-col items-center gap-2"><div className="p-4 bg-white rounded-2xl border border-slate-100 shadow-sm"><Lucide.Shield size={20} className="text-rose-400"/></div><span className="text-[9px] font-black uppercase text-slate-400">Terminal</span></button>
            </div>
              ...
              <div className="space-y-4">
                {marketplace.map(store => (
                  <div key={store.id} onClick={() => { setSelStore(store); setView('detail'); setSelService(null); setBookForm({ name:'', date:'', time:'', phone:'', resId:'', seats:1 }); }} className="bg-white p-3 rounded-[2.5rem] flex gap-4 items-center shadow-sm border border-slate-100 group active:scale-[0.98] transition-all">
                    ...
                    <Lucide.ChevronRight size={18} className="text-slate-200 group-hover:text-emerald-500" />
                  </div>
                ))}
              </div>
            </section>
        ...
        {view === 'track' && (
           <div className="pt-6 space-y-6 animate-in slide-in-from-bottom-8 px-1">
              <h2 className="text-2xl font-black text-emerald-900 uppercase italic tracking-tighter text-center">Queue Tracking</h2>
              <div className="bg-white p-6 rounded-[2.5rem] shadow-xl border border-slate-100 space-y-4">
                 ...
                 <button onClick={() => { const b = bookings.find(x => x.displayId === trackInput); if(b) setActiveReceipt(b); else notify("Booking ID not found", "error"); }} className="w-full py-5 bg-emerald-600 text-white rounded-2xl font-black uppercase text-[10px] tracking-[0.2em] shadow-lg">Track Live</button>
              </div>

              {liveTracking && !liveTracking.error ? (
                <div className="bg-white p-8 rounded-[3.5rem] shadow-2xl border-t-8 border-emerald-500 text-center space-y-6 animate-in zoom-in-95">
                   <div className="w-20 h-20 bg-emerald-50 text-emerald-600 rounded-full flex items-center justify-center mx-auto text-3xl font-black italic shadow-inner border border-emerald-100">{liveTracking.pos}</div>
                   <div>
                      <h3 className="text-3xl font-black tracking-tighter uppercase italic">{liveTracking.displayId}</h3>
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">Confirmed for {liveTracking.name}</p>
                   </div>
                   <div className="grid grid-cols-2 gap-3">
                      <div className="bg-slate-50 p-4 rounded-3xl border border-slate-100 shadow-inner">
                         ...
                         <p className="text-xl font-black text-emerald-600">{liveTracking.pos === 1 ? "NEXT" : (liveTracking.pos - 1) + " People"}</p>
                      </div>
                      <div className="bg-slate-50 p-4 rounded-3xl border border-slate-100 shadow-inner">
                         <p className="text-[8px] font-black text-slate-400 uppercase">Est. Wait</p>
                         <p className="text-xl font-black text-blue-600">~{liveTracking.wait}m</p>
                      </div>
                   </div>
                   <button onClick={() => setView('home')} className="w-full py-4 bg-slate-900 text-white rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-xl">Back Home</button>
                </div>
              ) : activeReceipt && <p className="text-center text-rose-500 font-black uppercase text-[10px] tracking-widest py-10 italic">Booking is no longer active.</p>}
           </div>
        )}

        {/* VIEW: MERCHANT DASHBOARD (STABILIZED & SEPARATED) */}
        {view === 'merchant' && mData && (
          <div className="pt-6 space-y-6 animate-in slide-in-from-bottom-8 px-1">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-slate-900 p-6 rounded-[2.5rem] text-white flex flex-col justify-between shadow-lg h-44">
                 <div><p className="text-[8px] font-black uppercase opacity-50 mb-1">Live visibility</p><p className="text-lg font-black uppercase italic tracking-tighter leading-none">{mData.store.isLive ? 'Online' : 'Offline'}</p></div>
              ...
              <div className="bg-white p-2 rounded-[2.5rem] border border-slate-100 shadow-lg relative overflow-hidden h-44 group">
                 <img src={mData.store.image} className="w-full h-full object-cover rounded-[2rem] opacity-50 group-hover:opacity-100 transition-all" alt="Profile" />
            ...
            <div className="flex bg-white p-1 rounded-2xl border border-slate-100 shadow-sm overflow-x-auto">
                {['ledger', 'assets', 'prices'].map(t => (
                    <button key={t} onClick={() => setMTab(t)} className={`flex-1 py-3 px-2 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${mTab === t ? 'bg-emerald-50 text-emerald-600' : 'text-slate-400'}`}>
                      {t === 'assets' ? (mData.store.category === 'salon' ? 'Staff' : 'Fleet') : t}
                    </button>
                ))}
            </div>
            ...
            {mTab === 'ledger' && (
               <section className="space-y-4 pb-20 px-1">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-white p-6 rounded-[2rem] border border-slate-100 text-center shadow-sm"><p className="text-[8px] font-black text-slate-400 mb-1">REVENUE</p><p className="text-2xl font-black text-emerald-600 italic">₹{mData.rev}</p></div>
                    <div className="bg-white p-6 rounded-[2rem] border border-slate-100 text-center shadow-sm"><p className="text-[8px] font-black text-slate-400 mb-1">IN QUEUE</p><p className="text-2xl font-black text-blue-600 italic">{mData.queue.length}</p></div>
                  </div>
                  {mData.queue.map((b, i) => (
                    <div key={i} className="bg-white p-5 rounded-[2rem] border-l-8 border-emerald-500 shadow-sm space-y-4 animate-in slide-in-from-left-4">
                      <div className="flex justify-between items-start">
                        <div>
                           ...
                           <p className="text-[8px] text-blue-600 font-black mt-1 uppercase tracking-widest italic">{b.seats || 1} Seats • {b.payment}</p>
                        </div>
                      ...
                      <div className="flex gap-2">
                        ...
                        <button onClick={() => updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'bookings', b.id), { status: 'completed' })} className="flex-1 p-3 bg-emerald-50 text-emerald-600 rounded-xl flex items-center justify-center gap-2 active:scale-95"><Lucide.CheckCircle2 size={16}/><span className="text-[9px] font-black uppercase tracking-widest">Confirm</span></button>
                        <button onClick={async () => { if(window.confirm("Cancel? Capacity restores instantly.")) await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'bookings', b.id)); }} className="p-3 bg-rose-50 text-rose-500 rounded-xl active:scale-90"><Lucide.Trash2 size={18}/></button>
                      </div>
                    </div>
                  ))}
                  {mData.queue.length === 0 && <p className="text-center py-20 text-slate-300 font-black uppercase text-[10px] tracking-widest opacity-50 italic">Dashboard Clear</p>}
               </section>
            )}
            ...
            {mTab === 'assets' && (
              <div className="bg-white p-8 rounded-[3rem] border border-slate-100 space-y-6 mx-1 animate-in fade-in">
                <button onClick={() => {
                   const n = prompt(mData.store.category === 'salon' ? "Staff Name:" : "Trip (e.g. Mumbai 9AM Bus):");
                   const t = mData.store.category === 'travel' ? prompt("Start Time (e.g. 09:30 AM):") : "";
                   const c = mData.store.category === 'travel' ? prompt("Total Seats Available:") : 1;
                ...
                }} className="w-full py-4 border-2 border-dashed border-slate-200 rounded-2xl font-black uppercase text-[10px] text-slate-400 hover:border-emerald-300 hover:text-emerald-500 transition-all">+ Add {mData.store.category === 'salon' ? 'Professional' : 'Vehicle Slot'}</button>
                <div className="space-y-3">
                   {mData.store.resources?.map((r, i) => (
                     <div key={i} className="flex justify-between items-center p-4 bg-slate-50 rounded-2xl border border-slate-100">
                        ...
                        <button onClick={() => updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'stores', profile.businessId), { resources: arrayRemove(r) })} className="p-2 text-rose-500 active:scale-90"><Lucide.X size={16}/></button>
                     </div>
                   ))}
                </div>
              </div>
            ...
            {mTab === 'prices' && (
               <div className="bg-white p-8 rounded-[3rem] border border-slate-100 space-y-6 mx-1 animate-in fade-in">
                  ...
                  }} className="w-full py-4 border-2 border-dashed border-slate-200 rounded-2xl font-black uppercase text-[10px] text-slate-400 hover:border-emerald-300">+ Add Entry</button>
                  <div className="space-y-3">
        ...
        {/* VIEW: DETAIL (USER BOOKING ISOLATION) */}
          ...
          <div className="pt-4 space-y-6 animate-in slide-in-from-right-4 px-1">
            ...
            <section className="bg-white p-6 rounded-[3rem] border border-slate-100 shadow-sm space-y-4">
              ...
              <div className="space-y-2">
                {selStore.services?.length > 0 ? selStore.services.map((s, idx) => (
                ...
                )) : <p className="text-center text-[9px] text-slate-300 font-black uppercase py-4 tracking-widest">Wait for partner setup</p>}
              </div>
            </section>
            ...
            {selService && (
              <div className="bg-white p-6 rounded-[3rem] shadow-sm border border-slate-100 animate-in slide-in-from-bottom-6 space-y-5">
                  ...
                  <div className="space-y-2">
                    <label className="text-[8px] font-black text-slate-400 uppercase ml-2 tracking-widest">Assign {selStore.category === 'salon' ? 'Professional' : 'Trip Timing'}</label>
                    <div className="space-y-2">
                      {selStore.resources?.map(r => {
                        ...
                        return (
                          <div key={r.id} onClick={() => !isFull && setBookForm({...bookForm, resId: r.id})} className={`p-4 rounded-2xl border-2 transition-all cursor-pointer flex justify-between items-center ${bookForm.resId === r.id ? 'border-emerald-600 bg-emerald-50' : 'border-slate-50 opacity-60'} ${isFull ? 'opacity-30 cursor-not-allowed grayscale' : ''}`}>
                            <div className="text-left"><p className="font-black text-[10px] uppercase italic leading-none">{r.name}</p>{r.time && <p className="text-[8px] font-black text-blue-500 mt-1 uppercase tracking-widest">Start: {r.time}</p>}</div>
                            <span className={`text-[8px] font-black uppercase tracking-tighter ${isFull ? 'text-rose-500' : 'text-emerald-600'}`}>{selStore.category === 'salon' ? `Queue: ${count}` : isFull ? 'FULL' : `${left} Seats Left`}</span>
                          </div>
                        );
                      })}
                    </div>
                  ...
                  <div className="flex gap-2">
                    <input type="date" onChange={e => setBookForm({...bookForm, date: e.target.value})} className="flex-1 bg-slate-50 p-4 rounded-xl font-black text-[10px] border border-slate-100 shadow-inner outline-none" />
                    {selStore.category === 'salon' && (
                       <input type="time" onChange={e => setBookForm({...bookForm, time: e.target.value})} className="w-28 bg-slate-50 p-4 rounded-xl font-black text-[10px] border border-slate-100 shadow-inner outline-none" />
                    )}
                  </div>
                  ...
                  <button disabled={!bookForm.date || (!bookForm.time && selStore.category === 'salon') || !bookForm.phone || !bookForm.resId || !bookForm.name} onClick={() => setPayModal(true)} className="w-full bg-emerald-600 text-white py-5 rounded-2xl font-black uppercase text-[11px] tracking-[0.2em] active:scale-[0.97] transition-all shadow-xl shadow-emerald-200 disabled:opacity-40">Proceed</button>
              </div>
            )}
          </div>
        )}
        ...
        {view === 'admin' && (
          <div className="pt-10 space-y-6 animate-in fade-in px-2">
             <div className="flex justify-between items-center px-1">
                <h2 className="text-2xl font-black text-rose-600 uppercase italic tracking-tighter leading-none">Terminal 112607</h2>
                 ...
                 {adminTab === 'requests' ? requests.map(r => (
                    <div key={r.id} className="bg-white p-6 rounded-[2.5rem] border border-slate-100 space-y-4 shadow-sm animate-in slide-in-from-bottom-4">
                        ...
                        <div className="flex gap-2">
                          ...
                          <button onClick={() => handleAdminApprove(r)} className="flex-[2] py-3 bg-emerald-600 text-white rounded-2xl font-black text-[9px] uppercase shadow-lg active:scale-95 tracking-widest italic">Approve</button>
                        </div>
                    </div>
                 )) : stores.map(s => (
          ...
          <div className="pt-6 space-y-6 animate-in slide-in-from-bottom-8 px-1">
            ...
            {hubMode === 'register' ? (
              ...
              <div className="bg-white p-8 rounded-[3rem] shadow-xl border border-slate-100 space-y-4 text-center mx-1">
                ...
                <button onClick={handleMerchantLogin} className="w-full bg-emerald-600 text-white py-5 rounded-[1.5rem] font-black uppercase text-[10px] tracking-widest active:scale-95 shadow-xl">Unlock Dashboard</button>
              </div>
            )}
          </div>
        )}
        ...
        <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-md z-[1000] flex items-center justify-center p-6 animate-in fade-in duration-300">
           <div className="bg-white w-full max-w-sm rounded-[3rem] p-8 shadow-2xl space-y-6 text-center">
              ...
              <div className="space-y-3">
                 ...
                 <button onClick={() => setConfirmModal(true)} className="w-full p-6 bg-emerald-600 text-white rounded-[1.8rem] shadow-xl flex items-center justify-between active:scale-95 transition-all">
                    <span className="font-black text-sm uppercase">Pay with Cash</span>
                    <Lucide.Banknote size={20} />
                 </button>
              </div>
              <button onClick={() => setPayModal(false)} className="text-[10px] font-black uppercase text-slate-400">Cancel</button>
           </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-emerald-600 z-[1001] flex items-center justify-center p-6 animate-in zoom-in-95 duration-200 text-white">
           <div className="text-center space-y-8">
              ...
              <div className="space-y-2">
                 ...
                 <p className="text-sm font-bold uppercase opacity-70 tracking-widest leading-none">Confirm to lock your spot</p>
              </div>
              <div className="space-y-3 pt-6">
                 <button onClick={handleBookingFinal} className="w-64 py-6 bg-white text-emerald-600 rounded-full font-black uppercase shadow-2xl active:scale-90 transition-all text-lg tracking-widest">YES, CONFIRM</button>
                 <button onClick={() => setConfirmModal(false)} className="block w-full py-4 text-white/50 font-black uppercase text-xs tracking-widest">Cancel</button>
              </div>
           </div>
        </div>
      )}
