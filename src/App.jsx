...
const appId = typeof __app_id !== 'undefined' ? __app_id : 'chiplun-v50-supreme-final';
...
export default function App() {
  ...
  }, [bookForm.date, bookForm.time, activeStore]);
      ...
      <header className="bg-emerald-600 text-white p-6 pb-12 rounded-b-[3.5rem] shadow-lg sticky top-0 z-50">
        ...
        {view === 'home' && (
          <div className="relative animate-in slide-in-from-top-4">
            ...
            <input type="text" placeholder="Search verified partners..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full bg-white/10 border border-white/20 rounded-2xl py-4 pl-12 pr-4 text-white placeholder-emerald-200 outline-none focus:bg-white/20" />
          </div>
        )}
      </header>
      ...
      <main className="flex-1 -mt-6 px-4 pb-32 z-10">
        ...
        {view === 'track' && (
           <div className="pt-6 space-y-6 animate-in slide-in-from-bottom-8 px-1">
              <h2 className="text-2xl font-black text-emerald-900 uppercase italic tracking-tighter text-center">Live Token Search</h2>
              <div className="bg-white p-6 rounded-[2.5rem] shadow-xl border border-slate-100 space-y-4">
                 <input placeholder="CH-XXXX Token ID" value={trackId} onChange={e => setTrackId(e.target.value.toUpperCase())} className="w-full bg-slate-50 border p-5 rounded-2xl text-lg font-black text-center outline-none tracking-widest" />
                 <button onClick={() => { if(!trackId) notify("Enter Token ID", "error"); }} className="w-full py-5 bg-emerald-600 text-white rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-lg">Check Position</button>
              </div>
              ...
              {userLiveTracker && !userLiveTracker.error ? (
                <div className="bg-white p-8 rounded-[3.5rem] shadow-2xl border-t-8 border-emerald-500 text-center space-y-6 animate-in zoom-in-95">
                   <div className="w-20 h-20 bg-emerald-50 text-emerald-600 rounded-full flex items-center justify-center mx-auto text-3xl font-black italic shadow-inner">{userLiveTracker.pos}</div>
                   <div>
                      <h3 className="text-3xl font-black tracking-tighter uppercase italic">{userLiveTracker.displayId}</h3>
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">Passenger: {userLiveTracker.custName}</p>
                   </div>
                   <div className="grid grid-cols-2 gap-3">
                      <div className="bg-slate-50 p-4 rounded-3xl border border-slate-100">
                         <p className="text-[8px] font-black text-slate-400 uppercase">Wait Rank</p>
                         <p className="text-xl font-black text-emerald-600">{userLiveTracker.pos === 1 ? "NEXT" : (userLiveTracker.pos - 1) + " People"}</p>
                      </div>
                      <div className="bg-slate-50 p-4 rounded-3xl border border-slate-100">
              ...
              ) : trackId && <p className="text-center text-rose-500 font-black uppercase text-[10px] py-10">Search Results will show here...</p>}
           </div>
        )}

        {/* VIEW: MERCHANT (SAFE HYDRATION) */}
        {view === 'merchant' && mData && (
          <div className="pt-6 space-y-6 animate-in slide-in-from-bottom-8 px-1">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-slate-900 p-6 rounded-[2.5rem] text-white flex flex-col justify-between shadow-lg h-44">
                 <div><p className="text-[8px] font-black uppercase opacity-50 mb-1">Status</p><p className="text-lg font-black uppercase italic tracking-tighter leading-none">{mData.store.isLive ? 'Publicly Live' : 'Hidden Mode'}</p></div>
              ...
              <div className="bg-white p-2 rounded-[2.5rem] border border-slate-100 shadow-lg relative overflow-hidden h-44 group">
                 ...
                 <label className="absolute inset-0 flex flex-col items-center justify-center cursor-pointer">
                    ...
                    <span className="text-[8px] font-black uppercase mt-1 text-slate-800 text-center px-2">Update Business Photo</span>
          ...
          <div className="pt-10 space-y-6 animate-in fade-in px-2">
             ...
             {!adminAuth ? (
               <div className="bg-white p-8 rounded-[3rem] shadow-2xl border border-rose-100 space-y-4 text-center">
                 <input type="password" placeholder="System PIN" value={adminPin} onChange={(e) => setAdminPin(e.target.value)} className="w-full bg-slate-50 border p-5 rounded-2xl border font-black text-center text-lg outline-none tracking-widest" />
                 <button onClick={() => { if (adminPin === ADMIN_PIN) setAdminAuth(true); else notify("Denied", "error"); }} className="w-full bg-rose-600 text-white py-5 rounded-2xl font-black shadow-xl uppercase active:scale-95 transition-all tracking-[0.2em]">Verify PIN</button>
               </div>
             ) : (
              ...
              <div className="bg-white p-6 rounded-[3rem] shadow-sm border border-slate-100 animate-in slide-in-from-bottom-6 space-y-5">
                  ...
                  <div className="space-y-2">
                    ...
                    {activeStore.resources?.map(r => {
                      const { count, left } = getInventoryStatus(activeStore.id, r.id, r.capacity);
                      ...
                      return (
                        <div key={r.id} onClick={() => !isFull && setBookForm({...bookForm, resId: r.id})} className={`p-4 rounded-2xl border-2 transition-all cursor-pointer flex justify-between items-center ${bookForm.resId === r.id ? 'border-emerald-600 bg-emerald-50' : 'border-slate-50 opacity-60'} ${isFull ? 'opacity-30 cursor-not-allowed grayscale' : ''}`}>
                          ...
                          <span className={`text-[8px] font-black uppercase tracking-tighter ${isFull ? 'text-rose-500' : 'text-emerald-600'}`}>{activeStore.category === 'salon' ? `Queue: ${count}` : isFull ? 'FULL' : `${left} S Left`}</span>
                        </div>
                      );
                    })}
                  </div>
                  ...
                  <div className="flex gap-2">
                    ...
                    {activeStore.category === 'salon' && (
                       <input type="time" value={bookForm.time} onChange={e => {
                          ...
                          const clock = now.getHours().toString().padStart(2, '0') + ":" + now.getMinutes().toString().padStart(2, '0');
                          if (bookForm.date === today && e.target.value < clock) notify("Select future time!", "error");
                  ...
                  <button disabled={!bookForm.date || (!bookForm.time && activeStore.category === 'salon') || !bookForm.phone || !bookForm.resId || !bookForm.custName || isTimeInvalid} onClick={() => setShowPayment(true)} className="w-full bg-emerald-600 text-white py-5 rounded-2xl font-black uppercase text-[11px] tracking-[0.2em] active:scale-[0.97] transition-all shadow-xl shadow-emerald-200 disabled:opacity-40">Get Slot Token</button>
              </div>
            )}
          </div>
        )}
      ...
      {showPayment && (
        <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-md z-[1000] flex items-center justify-center p-6 animate-in fade-in duration-300">
           <div className="bg-white w-full max-w-sm rounded-[3rem] p-8 shadow-2xl space-y-6 text-center">
              ...
              <div className="space-y-3">
                 <button onClick={() => notify("UPI / Cards Coming Soon", "error")} className="w-full p-4 bg-slate-50 rounded-2xl border border-slate-100 flex items-center justify-between opacity-50">
        ...
        <div className="fixed inset-0 bg-emerald-600 z-[1001] flex items-center justify-center p-6 animate-in zoom-in-95 duration-200 text-white text-center">
           <div className="space-y-8">
              ...
              <div className="space-y-2">
                 ...
                 <p className="text-sm font-bold uppercase opacity-70 tracking-widest leading-none text-center">Confirming will lock your slot and issue Token ID</p>
              </div>
