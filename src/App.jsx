...
// --- LOGISTICS ENGINE (NH66 SEGMENTS) ---
const ROUTE_STOPS = [
  { n: "Chiplun", km: 0 },
  { n: "Khed", km: 45 },
  { n: "Mangaon", km: 105 },
...
export default function App() {
  ...
  const getResourceStatus = (sId, rId, capacity) => {
    ...
    return { count: active.length, remaining: (Number(capacity) || 0) - occupied };
  };
  ...
  const handleAdminApprove = async (req) => {
    const mId = prompt("Assign Merchant ID (e.g. CHI-SAL-01):");
    const key = prompt("Set Security Key:");
    if (!mId || !key) return;
    setIsProcessing(true);
    try {
      ...
      notify("Approved! Merchant can now login.");
  ...
  return (
    <div className="max-w-md mx-auto bg-slate-50 min-h-screen flex flex-col shadow-2xl relative font-sans text-slate-900 selection:bg-emerald-100 overflow-x-hidden">
      ...
      <header className="bg-emerald-600 text-white p-6 pb-12 rounded-b-[3.5rem] shadow-lg sticky top-0 z-50">
        <div className="flex justify-between items-center mb-6">
          <div onClick={() => setView('home')} className="cursor-pointer active:scale-95 transition-all">
            ...
            <p className="text-[10px] font-bold opacity-80 uppercase tracking-widest mt-1">V50 Pro Master</p>
          </div>
          <button 
        ...
        {view === 'home' && (
          <div className="relative animate-in slide-in-from-top-4">
            ...
            <input type="text" placeholder="Search Chiplun marketplace..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-white/10 border border-white/20 rounded-2xl py-4 pl-12 pr-4 text-white placeholder-emerald-200 outline-none focus:bg-white/20" />
          </div>
        )}
      </header>
      ...
      <main className="flex-1 -mt-6 px-4 pb-32 z-10">
        ...
        {view === 'home' && (
          <div className="space-y-8 pt-2">
            ...
            <section className="space-y-4">
              <h2 className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic px-1">Live Partners</h2>
              <div className="space-y-4">
                {filteredStores.map(store => (
                  <div key={store.id} onClick={() => { setSelectedStore(store); setView('detail'); setCart([]); }} className="bg-white p-3 rounded-[2.5rem] flex gap-4 items-center shadow-sm border border-slate-100 active:scale-[0.98] transition-all">
        ...
        {/* VIEW: MERCHANT DASHBOARD (REFACTORED MANAGEMENT MATRIX) */}
        {view === 'merchant' && activeMerchant && (
          <div className="pt-6 space-y-6 animate-in slide-in-from-bottom-8 px-1">
            ...
            <div className={`p-6 rounded-[2.5rem] flex items-center justify-between shadow-lg transition-all ${activeMerchant.store.isLive ? 'bg-emerald-600 text-white' : 'bg-slate-900 text-white'}`}>
              <div><p className="text-[9px] font-black uppercase opacity-70 tracking-widest leading-none mb-1">Public Visibility</p><p className="text-xl font-black uppercase italic tracking-tighter">{activeMerchant.store.isLive ? 'Online & Active' : 'Offline (Hidden)'}</p></div>
            ...
            <div className="flex bg-white p-1 rounded-2xl border border-slate-100 shadow-sm overflow-x-auto">
                <button onClick={() => setMTab('ledger')} className={`flex-1 py-3 px-2 rounded-xl text-[8px] font-black uppercase tracking-widest ${mTab === 'ledger' ? 'bg-emerald-50 text-emerald-600' : 'text-slate-400'}`}>Ledger</button>
                <button onClick={() => setMTab('resources')} className={`flex-1 py-3 px-2 rounded-xl text-[8px] font-black uppercase tracking-widest ${mTab === 'resources' ? 'bg-emerald-50 text-emerald-600' : 'text-slate-400'}`}>{activeMerchant.store.category === 'salon' ? 'Staff' : 'Fleet'}</button>
                <button onClick={() => setMTab('pricing')} className={`flex-1 py-3 px-2 rounded-xl text-[8px] font-black uppercase tracking-widest ${mTab === 'pricing' ? 'bg-emerald-50 text-emerald-600' : 'text-slate-400'}`}>Pricing</button>
            </div>
            ...
            {mTab === 'ledger' && (
               <section className="space-y-4 pb-20 px-1">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-white p-6 rounded-[2rem] border border-slate-100 text-center shadow-sm"><p className="text-[8px] font-black text-slate-400 mb-1 tracking-widest uppercase leading-none">Revenue</p><p className="text-2xl font-black text-emerald-600 tracking-tighter">₹{activeMerchant.rev}</p></div>
                    <div className="bg-white p-6 rounded-[2rem] border border-slate-100 text-center shadow-sm"><p className="text-[8px] font-black text-slate-400 mb-1 tracking-widest uppercase leading-none">In Queue</p><p className="text-2xl font-black text-blue-600 tracking-tighter">{activeMerchant.pending.length}</p></div>
                  </div>
                  {activeMerchant.pending.map((b, i) => (
                    <div key={i} className="bg-white p-5 rounded-[2rem] border-l-8 border-emerald-500 shadow-sm flex justify-between items-center animate-in slide-in-from-left-4">
                      <div className="flex-1 pr-4">
                        <p className="font-black text-sm uppercase italic tracking-tight leading-none">{b.custName || 'User'}</p>
                      ...
                      <div className="flex gap-2">
                        <button onClick={() => window.open(`https://wa.me/${b.phone}`)} className="p-3 bg-slate-50 text-slate-400 rounded-xl active:scale-90 transition-all"><Lucide.MessageCircle size={18}/></button>
            ...
            {mTab === 'resources' && (
              <div className="bg-white p-8 rounded-[3rem] border border-slate-100 space-y-6 animate-in fade-in">
                <h3 className="text-[10px] font-black uppercase text-slate-400 italic px-2">Manage {activeMerchant.store.category === 'salon' ? 'Staff Members' : 'Vehicle Fleet'}</h3>
                <button onClick={() => {
                   const n = prompt(activeMerchant.store.category === 'salon' ? "Enter Staff/Barber Name:" : "Enter Vehicle/Slot (e.g. Innova - 9AM):");
                   const c = activeMerchant.store.category === 'travel' ? prompt("Car Seat Capacity (e.g. 4):") : 1;
                ...
                }} className="w-full py-4 border-2 border-dashed border-slate-200 rounded-2xl font-black uppercase text-[10px] text-slate-400 hover:border-emerald-300 hover:text-emerald-500 transition-all">+ Add {activeMerchant.store.category === 'salon' ? 'Staff' : 'Vehicle'}</button>
                <div className="space-y-3">
                   {activeMerchant.store.resources?.map((r, i) => (
                     <div key={i} className="flex justify-between items-center p-4 bg-slate-50 rounded-2xl border border-slate-100 shadow-inner">
                        <div><p className="font-black text-xs uppercase italic leading-none">{r.name}</p>{activeMerchant.store.category === 'travel' && <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mt-1">Seats: {r.capacity}</p>}</div>
            ...
            {/* TAB: PRICING (SALON FLAT / TRAVEL KM) */}
            {mTab === 'pricing' && (
               <div className="bg-white p-8 rounded-[3rem] border border-slate-100 space-y-6 animate-in fade-in">
                  <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-center italic opacity-60 leading-none">Service Catalog</h3>
                  <button onClick={() => {
                     const n = prompt(activeMerchant.store.category === 'salon' ? "Service Name (e.g. Haircut):" : "Vehicle Class (e.g. Premium SUV):");
                     const p = prompt(activeMerchant.store.category === 'travel' ? "Base Rate per KM (₹):" : "Fixed Service Price (₹):");
                     if (n && p) updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'stores', profile.businessId), { services: arrayUnion({ name: n, price: Number(p) }) });
                  }} className="w-full py-4 border-2 border-dashed border-slate-200 rounded-2xl font-black uppercase text-[10px] text-slate-400 hover:border-emerald-300 transition-all">+ Add Pricing Entry</button>
                  <div className="space-y-3">
                     {activeMerchant.store.services?.map((s, i) => (
                       <div key={i} className="flex justify-between items-center p-4 bg-slate-50 rounded-2xl border border-slate-100 shadow-inner">
                          <p className="font-black text-xs uppercase italic leading-none">{s.name} • ₹{s.price}{activeMerchant.store.category === 'travel' ? '/KM' : ''}</p>
        ...
        {/* VIEW: ADMIN, BUSINESS HUB, DETAIL, TRACKING (Logic Integrated) */}
        {view === 'admin' && (
          <div className="pt-10 space-y-6 animate-in fade-in px-2">
             <div className="flex justify-between items-center px-1">
                <h2 className="text-2xl font-black text-rose-600 uppercase italic tracking-tighter">System Terminal</h2>
             ...
             {!adminAuth ? (
               <div className="bg-white p-8 rounded-[3rem] shadow-2xl border border-rose-100 space-y-4 text-center">
                 ...
                 <button onClick={() => { if (adminPin === ADMIN_PIN) setAdminAuth(true); else notify("Denied", "error"); }} className="w-full bg-rose-600 text-white py-5 rounded-2xl font-black shadow-xl uppercase active:scale-95 transition-all">Verify Access</button>
               </div>
             ) : (
          ...
          <div className="pt-6 space-y-6 animate-in slide-in-from-bottom-8">
            ...
            {bizSubView === 'register' ? (
              <div className="bg-white p-8 rounded-[3rem] shadow-xl border border-slate-100 space-y-4 mx-1">
                ...
                <input value={regForm.address} onChange={e => setRegForm({...regForm, address: e.target.value})} className="w-full bg-slate-50 border p-4 rounded-xl text-[10px] font-black uppercase outline-none focus:border-emerald-500 shadow-inner" placeholder="Location Details" />
        ...
        {view === 'detail' && selectedStore && (
          <div className="pt-4 space-y-6 animate-in slide-in-from-right-4">
            <button onClick={() => setView('home')} className="flex items-center text-emerald-600 font-black text-[10px] uppercase tracking-widest active:scale-95 px-2"><Lucide.ArrowLeft size={16} className="mr-2"/> Back</button>
            ...
            <section className="bg-white p-6 rounded-[3rem] border border-slate-100 shadow-sm space-y-4 mx-1">
              <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic px-2 leading-none">1. Choose Service Type</h3>
              <div className="space-y-2">
                ...
                )) : <p className="text-center text-[9px] text-slate-300 font-black uppercase py-4">Wait for partner setup</p>}
              </div>
            </section>
            ...
            {cart.length > 0 && (
              <div className="bg-white p-6 rounded-[3rem] shadow-sm border border-slate-100 animate-in slide-in-from-bottom-6 space-y-5 mx-1">
                  <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-center italic leading-none">2. Logistics & Time</h3>
                  
                  {selectedStore.category === 'travel' && (
                    <div className="space-y-3">
                       <div className="grid grid-cols-2 gap-2">
                          <div className="space-y-1">
                             <label className="text-[8px] font-black text-slate-400 uppercase ml-2 tracking-widest">Pickup Stop</label>
                             <select value={bookMeta.pickup} onChange={e => setBookMeta({...bookMeta, pickup: e.target.value})} className="w-full bg-slate-50 p-4 rounded-xl font-black text-[10px] outline-none border border-slate-100 shadow-inner">
                               {ROUTE_STOPS.map(s => <option key={s.n} value={s.n}>{s.n}</option>)}
                             </select>
                          </div>
                          <div className="space-y-1">
                             <label className="text-[8px] font-black text-slate-400 uppercase ml-2 tracking-widest">Drop Stop</label>
                             <select value={bookMeta.drop} onChange={e => setBookMeta({...bookMeta, drop: e.target.value})} className="w-full bg-slate-50 p-4 rounded-xl font-black text-[10px] outline-none border border-slate-100 shadow-inner">
                               {ROUTE_STOPS.map(s => <option key={s.n} value={s.n}>{s.n}</option>)}
                             </select>
                          </div>
                       </div>
                  ...
                  <div className="space-y-2">
                    <label className="text-[8px] font-black text-slate-400 uppercase ml-2 tracking-widest">Available {selectedStore.category === 'salon' ? 'Staff' : 'Slots'}</label>
                    <div className="space-y-2">
                      {selectedStore.resources?.map(r => {
                        const { count, remaining } = getResourceStatus(selectedStore.id, r.id, r.capacity);
                        ...
                        return (
                          <div key={r.id} onClick={() => !isFull && setBookMeta({...bookMeta, resId: r.id})} className={`p-4 rounded-2xl border-2 transition-all cursor-pointer flex justify-between items-center ${bookMeta.resId === r.id ? 'border-emerald-600 bg-emerald-50' : 'border-slate-50 opacity-60'} ${isFull ? 'opacity-30 cursor-not-allowed' : ''}`}>
                            ...
                            <span className={`text-[8px] font-black uppercase ${isFull ? 'text-rose-500' : 'text-emerald-600'} tracking-tighter`}>
                              {selectedStore.category === 'salon' ? `Queue: ${count} (~${count * 20}m)` : isFull ? 'Sold Out' : `${remaining} S Left`}
                            </span>
                          </div>
                        );
                      })}
                  ...
                  {selectedStore.category === 'travel' && bookMeta.resId && (
                    <div className="grid grid-cols-2 gap-3 bg-blue-50/50 p-4 rounded-2xl border border-blue-100 items-center">
                       <div className="space-y-1">
                          <label className="text-[8px] font-black text-blue-400 uppercase leading-none">Passengers</label>
                       ...
                       <div className="text-right">
                          <label className="text-[8px] font-black text-blue-400 uppercase block tracking-widest leading-none">Est. Total</label>
                          <p className="text-xl font-black text-blue-600 mt-1 tracking-tighter leading-none italic">₹{fareCalc}</p>
                       </div>
                    </div>
                  )}

                  <div className="flex gap-2">
                    <input type="date" onChange={e => setBookMeta({...bookMeta, date: e.target.value})} className="flex-1 bg-slate-50 p-4 rounded-xl font-black text-[10px] border border-slate-100 shadow-inner" />
                    <input type="time" onChange={e => setBookMeta({...bookMeta, time: e.target.value})} className="w-28 bg-slate-50 p-4 rounded-xl font-black text-[10px] border border-slate-100 shadow-inner" />
                  </div>
                  <input placeholder="WhatsApp Phone" value={bookMeta.phone} onChange={e => setBookMeta({...bookMeta, phone: e.target.value})} className="w-full bg-slate-50 p-4 rounded-xl font-black text-[10px] border border-slate-100 uppercase tracking-widest shadow-inner" />
                  <button disabled={!bookMeta.date || !bookMeta.time || !bookMeta.phone || !bookMeta.resId} onClick={handleBooking} className="w-full bg-emerald-600 text-white py-5 rounded-2xl font-black uppercase text-[11px] tracking-[0.2em] active:scale-[0.97] transition-all shadow-xl shadow-emerald-200 disabled:opacity-40">Confirm Slot</button>
              </div>
            )}
          </div>
        )}
          ...
          <div className="pt-10 space-y-8 animate-in slide-in-from-bottom-8">
            ...
            <div className="bg-white p-8 rounded-[3.5rem] shadow-2xl border-t-8 border-emerald-500 text-center space-y-6 mx-1">
                ...
                <div className="bg-slate-50 p-5 rounded-3xl text-[9px] font-black uppercase text-left space-y-2 border border-slate-100 shadow-inner">
                  <div className="flex justify-between italic tracking-widest leading-none"><span>Merchant:</span><span className="text-emerald-700 font-black">{tracked.storeName}</span></div>
                  <div className="flex justify-between leading-none"><span>Service:</span><span>{tracked.serviceName}</span></div>
                  {tracked.pickup && <div className="flex justify-between text-blue-600 border-t border-slate-200 mt-1 pt-1 tracking-tighter font-black uppercase leading-none"><span>Trip:</span><span>{tracked.pickup} ➔ {tracked.drop}</span></div>}
                  <div className="flex justify-between pt-2 border-t border-slate-200 text-xs font-black leading-none"><span>Amount:</span><span className="text-emerald-600 text-lg tracking-tighter leading-none">₹{tracked.totalPrice}</span></div>
                </div>
                <button onClick={() => setView('home')} className="w-full bg-slate-900 text-white py-4 rounded-2xl font-black uppercase text-xs active:scale-95 transition-all shadow-lg tracking-[0.2em]">Go Home</button>
            </div>
          </div>
        )}
      </main>
