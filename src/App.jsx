...
const appId = typeof __app_id !== 'undefined' ? __app_id : 'chiplun-ultimate-stable-v50';
...
export default function App() {
  ...
  const getCapacityData = (sId, rId, total) => {
  ...
  const trackerStats = useMemo(() => {
    ...
    if (!live || live.status !== 'pending') return { error: 'Invalid' };
  ...
  const handleTimeChange = (val) => {
    ...
    if (bookForm.date === today && val < clock) {
      notify("Cheat Block: Past time not allowed!", "error");
  ...
  const handleImage = async (e) => {
    ...
    if (!file) return;
    if (file.size > 2000000) return notify("Image too large. Max 2MB.", "error");
    ...
    reader.onloadend = async () => {
       ...
       img.onload = async () => {
          ...
          await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'stores', profile.businessId), { image: compressed });
          notify("Business Photo Updated!");
  ...
  const handleBookingConfirm = async () => {
    if (!bookForm.custName || !bookForm.resId) return notify("Missing Info", "error");
    setIsProcessing(true);
    try {
  ...
  const handleVLogin = async () => {
    if (!vLogin.id || !vLogin.pass) return notify("Enter Credentials", "error");
    setIsProcessing(true);
    try {
      ...
      if (snap.exists() && snap.data().password === vLogin.pass) {
        ...
        setView('merchant');
        notify("Merchant Console Unlocked");
      ...
      <header className="bg-emerald-600 text-white p-6 pb-12 rounded-b-[3.5rem] shadow-lg sticky top-0 z-50">
        <div className="flex justify-between items-center mb-6">
          <div onClick={() => setView('home')} className="cursor-pointer active:scale-95 transition-all">
            ...
            <p className="text-[10px] font-bold opacity-80 uppercase tracking-widest mt-1">Ultimate Supreme V50</p>
          </div>
          <button onClick={() => setView(profile.role === 'vendor' ? 'merchant' : 'business')} className={`w-10 h-10 rounded-2xl flex items-center justify-center border transition-all ${view === 'business' || view === 'merchant' ? 'bg-white text-emerald-600 border-white shadow-inner' : 'bg-white/10'}`}>
        ...
        {view === 'home' && (
          <div className="relative animate-in slide-in-from-top-4">
            ...
            <input type="text" placeholder="Find salons, travel, and more..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full bg-white/10 border border-white/20 rounded-2xl py-4 pl-12 pr-4 text-white placeholder-emerald-200 outline-none focus:bg-white/20 transition-all shadow-inner" />
          </div>
        )}
      </header>
      ...
      <main className="flex-1 -mt-6 px-4 pb-32 z-10">
        ...
        {view === 'home' && (
          <div className="space-y-8 pt-2">
            ...
            <section className="space-y-4">
              <h2 className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic px-1">Live chiplun Marketplace</h2>
              <div className="space-y-4">
                {marketplace.map(store => (
                  <div key={store.id} onClick={() => { setSelStore(store); setView('detail'); setSelService(null); setBookForm({ custName:'', date:'', time:'', phone:'', resId:'', seats:1 }); }} className="bg-white p-3 rounded-[2.5rem] flex gap-4 items-center shadow-sm border border-slate-100 group active:scale-[0.98] transition-all">
        ...
        {view === 'track' && (
           <div className="pt-6 space-y-6 animate-in slide-in-from-bottom-8 px-1">
              <h2 className="text-2xl font-black text-emerald-900 uppercase italic tracking-tighter text-center">Live Status Tracking</h2>
              <div className="bg-white p-6 rounded-[2.5rem] shadow-xl border border-slate-100 space-y-4">
                 <input placeholder="Token ID (e.g. CH-X2A)" value={trackInput} onChange={e => setTrackInput(e.target.value.toUpperCase())} className="w-full bg-slate-50 border p-5 rounded-2xl text-lg font-black text-center outline-none tracking-widest focus:border-emerald-500" />
                 <button onClick={() => { const b = bookings.find(x => x.displayId === trackInput); if(b) setActiveReceipt(b); else notify("No active token found", "error"); }} className="w-full py-5 bg-emerald-600 text-white rounded-2xl font-black uppercase text-[10px] tracking-[0.2em] shadow-lg active:scale-95">Find My Spot</button>
              </div>

              {trackerStats && !trackerStats.error ? (
                <div className="bg-white p-8 rounded-[3.5rem] shadow-2xl border-t-8 border-emerald-500 text-center space-y-6 animate-in zoom-in-95">
                   <div className="w-20 h-20 bg-emerald-50 text-emerald-600 rounded-full flex items-center justify-center mx-auto text-3xl font-black italic border border-emerald-100 shadow-inner">{trackerStats.pos}</div>
                   <div>
                      <h3 className="text-3xl font-black tracking-tighter uppercase italic">{trackerStats.displayId}</h3>
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">Confirmed for {trackerStats.custName}</p>
                   </div>
                   <div className="grid grid-cols-2 gap-3">
                      <div className="bg-slate-50 p-4 rounded-3xl border border-slate-100">
                         <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Queue Position</p>
                         <p className="text-xl font-black text-emerald-600">{trackerStats.pos === 1 ? "YOU ARE NEXT" : (trackerStats.pos - 1) + " People Ahead"}</p>
                      </div>
                      <div className="bg-slate-50 p-4 rounded-3xl border border-slate-100">
                         <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Est. Time Remaining</p>
                         <p className="text-xl font-black text-blue-600">~{trackerStats.wait} mins</p>
                      </div>
                   </div>
                   <button onClick={() => setView('home')} className="w-full py-4 bg-slate-900 text-white rounded-2xl font-black uppercase text-[10px] active:scale-95">Back to Home</button>
                </div>
        ...
        {/* VIEW: MERCHANT (STABILIZED & ISOLATED) */}
        {view === 'merchant' && mData && (
          <div className="pt-6 space-y-6 animate-in slide-in-from-bottom-8 px-1">
            ...
            {mTab === 'ledger' && (
               <section className="space-y-4 pb-20 px-1">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-white p-6 rounded-[2rem] border border-slate-100 text-center shadow-sm"><p className="text-[8px] font-black text-slate-400 mb-1 uppercase">Total Sales</p><p className="text-2xl font-black text-emerald-600 italic">₹{mData.rev}</p></div>
                    <div className="bg-white p-6 rounded-[2rem] border border-slate-100 text-center shadow-sm"><p className="text-[8px] font-black text-slate-400 mb-1 uppercase">Queue</p><p className="text-2xl font-black text-blue-600 italic">{mData.queue.length}</p></div>
                  </div>
                  {mData.queue.map((b, i) => (
                    <div key={i} className="bg-white p-5 rounded-[2rem] border-l-8 border-emerald-500 shadow-sm space-y-4 animate-in slide-in-from-left-4">
                      <div className="flex justify-between items-start">
                        <div>
                           ...
                           <p className="text-[8px] text-blue-600 font-black uppercase mt-1 tracking-widest italic">Seats: {b.seats || 1} • {b.payment}</p>
                        </div>
                        <span className="bg-slate-50 px-2 py-1 rounded text-[10px] font-black italic tracking-widest">#{b.displayId}</span>
                      </div>
                      <div className="flex gap-2">
                        <button onClick={() => window.open(`tel:${b.phone}`)} className="flex-1 p-3 bg-blue-50 text-blue-600 rounded-xl flex items-center justify-center gap-2 active:scale-95 transition-all"><Lucide.Phone size={16}/><span className="text-[9px] font-black uppercase">Call</span></button>
                        <button onClick={() => updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'bookings', b.id), { status: 'completed' })} className="flex-1 p-3 bg-emerald-50 text-emerald-600 rounded-xl flex items-center justify-center gap-2 active:scale-95 transition-all"><Lucide.CheckCircle2 size={16}/><span className="text-[9px] font-black uppercase tracking-widest">Complete</span></button>
                        <button onClick={async () => { if(window.confirm("Cancel & Release Seats?")) await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'bookings', b.id)); }} className="p-3 bg-rose-50 text-rose-500 rounded-xl active:scale-90 transition-all"><Lucide.X size={18}/></button>
                      </div>
                    </div>
                  ))}
               </section>
            ...
            {mTab === 'assets' && (
              <div className="bg-white p-8 rounded-[3rem] border border-slate-100 space-y-6 mx-1 animate-in fade-in">
                <button onClick={() => {
                   ...
                   const t = mData.store.category === 'travel' ? prompt("Starting Time (e.g. 09:00 AM):") : "";
                ...
                }} className="w-full py-4 border-2 border-dashed border-slate-200 rounded-2xl font-black uppercase text-[10px] text-slate-400 hover:border-emerald-300 hover:text-emerald-500 transition-all">+ Add {mData.store.category === 'salon' ? 'Professional' : 'Trip Slot'}</button>
            ...
            {mTab === 'prices' && (
               <div className="bg-white p-8 rounded-[3rem] border border-slate-100 space-y-6 mx-1 animate-in fade-in">
                  <button onClick={() => {
                     const n = prompt("Item (e.g. Premium Haircut / Chiplun to Khed):");
                  ...
                  }} className="w-full py-4 border-2 border-dashed border-slate-200 rounded-2xl font-black uppercase text-[10px] text-slate-400 hover:border-emerald-300 hover:text-emerald-500 transition-all">+ Add Price Entry</button>
                  <div className="space-y-3">
        ...
        {/* VIEW: DETAIL (HARD CATEGORY ISOLATION) */}
              ...
              <div className="bg-white p-6 rounded-[3rem] shadow-sm border border-slate-100 animate-in slide-in-from-bottom-6 space-y-5">
                  ...
                  <div className="space-y-2">
                    ...
                    {selStore.resources?.map(r => {
                      const { count, left } = getCapacityData(selStore.id, r.id, r.capacity);
                      ...
                      return (
                        <div key={r.id} onClick={() => !isFull && setBookForm({...bookForm, resId: r.id})} className={`p-4 rounded-2xl border-2 transition-all cursor-pointer flex justify-between items-center ${bookForm.resId === r.id ? 'border-emerald-600 bg-emerald-50' : 'border-slate-50 opacity-60'} ${isFull ? 'opacity-30 cursor-not-allowed grayscale' : ''}`}>
                          <div className="text-left"><p className="font-black text-[10px] uppercase italic tracking-tight leading-none">{r.name}</p>{r.time && <p className="text-[8px] font-black text-blue-500 mt-1 uppercase">Leaves at {r.time}</p>}</div>
                  ...
                  <button disabled={!bookForm.date || (!bookForm.time && selStore.category === 'salon') || !bookForm.phone || !bookForm.resId || !bookForm.custName} onClick={() => setPayModal(true)} className="w-full bg-emerald-600 text-white py-5 rounded-2xl font-black uppercase text-[11px] tracking-[0.2em] active:scale-[0.97] transition-all shadow-xl shadow-emerald-200 disabled:opacity-40">Proceed to Payment</button>
              </div>
            )}
          </div>
        )}
          ...
          <div className="pt-10 space-y-6 animate-in fade-in px-2">
             ...
             {!adminAuth ? (
               <div className="bg-white p-8 rounded-[3rem] shadow-2xl border border-rose-100 space-y-4 text-center">
                 <input type="password" placeholder="Admin PIN" value={adminPin} onChange={(e) => setAdminPin(e.target.value)} className="w-full bg-slate-50 p-5 rounded-2xl border font-black text-center text-lg outline-none tracking-widest" />
          ...
          <div className="pt-6 space-y-6 animate-in slide-in-from-bottom-8 px-1">
            ...
            {hubMode === 'register' ? (
              <div className="bg-white p-8 rounded-[3rem] shadow-xl border border-slate-100 space-y-4 mx-1">
                ...
                <input value={regForm.address} onChange={e => setRegForm({...regForm, address: e.target.value})} className="w-full bg-slate-50 border p-4 rounded-xl text-[10px] font-black uppercase outline-none focus:border-emerald-500 shadow-inner" placeholder="Location" />
                <button onClick={() => { setIsProcessing(true); addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'requests'), { ...regForm, status: 'pending', timestamp: Date.now() }).then(() => { notify("Wait for Admin review!"); setView('home'); setIsProcessing(false); }) }} className="w-full bg-emerald-600 text-white py-5 rounded-[1.5rem] font-black uppercase text-[10px] tracking-widest active:scale-[0.97] transition-all shadow-xl">Submit Application</button>
              </div>
            ) : (
        ...
        <div className="fixed inset-0 bg-emerald-600 z-[1001] flex items-center justify-center p-6 animate-in zoom-in-95 duration-200 text-white">
           <div className="text-center space-y-8">
              ...
              <div className="space-y-3 pt-6">
                 <button onClick={handleBookingConfirm} className="w-64 py-6 bg-white text-emerald-600 rounded-full font-black uppercase shadow-2xl active:scale-90 transition-all text-lg tracking-widest">YES, CONFIRM</button>
