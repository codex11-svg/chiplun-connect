...
export default function App() {
  ...
  const getSlotStats = (sId, rId, total) => {
  ...
  const handleBookingFinalSubmit = async () => {
    // ANTI-CHEAT TIME LOGIC
    const now = new Date();
    const today = now.toISOString().split('T')[0];
    const clock = now.getHours().toString().padStart(2, '0') + ":" + now.getMinutes().toString().padStart(2, '0');
    if (bookForm.date === today && bookForm.time < clock && activeStore.category === 'salon') {
  ...
  const handleImageUpload = async (e) => {
    ...
    if (!file || file.size > 800000) return notify("Image too large (Max 800KB)", "error");
  ...
  return (
    <div className="max-w-md mx-auto bg-slate-50 min-h-screen flex flex-col shadow-2xl relative font-sans text-slate-900 selection:bg-emerald-100 overflow-x-hidden">
      ...
      <header className="bg-emerald-600 text-white p-6 pb-12 rounded-b-[3.5rem] shadow-lg sticky top-0 z-50">
        <div className="flex justify-between items-center mb-6">
          <div onClick={() => setView('home')} className="cursor-pointer active:scale-95 transition-all">
            ...
            <p className="text-[10px] font-bold opacity-80 uppercase tracking-widest mt-1">Final Supreme • V50</p>
          </div>
          <div className="flex gap-2">
             ...
             <button onClick={() => setView(profile.role === 'vendor' ? 'merchant' : 'business')} className={`w-10 h-10 rounded-2xl flex items-center justify-center border transition-all ${view === 'business' || view === 'merchant' ? 'bg-white text-emerald-600 border-white' : 'bg-white/10 text-white'}`}><Lucide.Briefcase size={20} /></button>
          </div>
        </div>
        {view === 'home' && (
      ...
      <main className="flex-1 -mt-6 px-4 pb-32 z-10">
            ...
            <section className="space-y-4">
              ...
              <div className="space-y-4">
                {marketplace.map(store => (
                  <div key={store.id} onClick={() => { setActiveStore(store); setView('detail'); setActiveCart(null); setBookForm({ name: '', date:'', time:'', phone:'', resId:'', seats:1 }); }} className="bg-white p-3 rounded-[2.5rem] flex gap-4 items-center shadow-sm border border-slate-100 active:scale-[0.98] transition-all group">
                    ...
                    <div className="flex-1">
                      ...
                      <h3 className="font-bold text-slate-800 text-sm uppercase mt-1 italic leading-tight">{store.name}</h3>
        ...
        {view === 'track' && (
           <div className="pt-6 space-y-6 animate-in slide-in-from-bottom-8 px-1">
              ...
              <div className="bg-white p-6 rounded-[2.5rem] shadow-xl border border-slate-100 space-y-4">
                 <input placeholder="Enter Token ID (CH-XXXX)" value={trackInput} onChange={e => setTrackInput(e.target.value.toUpperCase())} className="w-full bg-slate-50 border p-5 rounded-2xl text-lg font-black text-center outline-none tracking-widest focus:border-emerald-500" />
                 <button onClick={() => { const b = bookings.find(x => x.displayId === trackInput); if(b) setActiveReceipt(b); else notify("Token not found", "error"); }} className="w-full py-5 bg-emerald-600 text-white rounded-2xl font-black uppercase text-[10px] tracking-[0.2em] shadow-lg">Check Position</button>
              </div>

              {userLiveTracker && !userLiveTracker.error ? (
                <div className="bg-white p-8 rounded-[3.5rem] shadow-2xl border-t-8 border-emerald-500 text-center space-y-6 animate-in zoom-in-95">
                   <div className="w-20 h-20 bg-emerald-50 text-emerald-600 rounded-full flex items-center justify-center mx-auto text-3xl font-black italic shadow-inner border border-emerald-100">{userLiveTracker.pos}</div>
                   <div>
                      <h3 className="text-3xl font-black tracking-tighter uppercase italic">{userLiveTracker.displayId}</h3>
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">Confirmed for {userLiveTracker.name}</p>
                   </div>
                   <div className="grid grid-cols-2 gap-3">
                      <div className="bg-slate-50 p-4 rounded-3xl border border-slate-100">
                         <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Rank</p>
                         <p className="text-xl font-black text-emerald-600">{userLiveTracker.pos === 1 ? "NEXT" : (userLiveTracker.pos - 1) + " Ahead"}</p>
                      </div>
                      <div className="bg-slate-50 p-4 rounded-3xl border border-slate-100">
                         <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Est. Wait</p>
                         <p className="text-xl font-black text-blue-600">~{userLiveTracker.wait}m</p>
                      </div>
                   </div>
                   <button onClick={() => setView('home')} className="w-full py-4 bg-slate-900 text-white rounded-2xl font-black uppercase text-[10px] active:scale-95">Explorer Mode</button>
                </div>
        ...
        {/* VIEW: MERCHANT (SAFE DATA HYDRATION) */}
          ...
          <div className="pt-6 space-y-6 animate-in slide-in-from-bottom-8 px-1">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-slate-900 p-6 rounded-[2.5rem] text-white flex flex-col justify-between shadow-lg h-44">
                 <div><p className="text-[8px] font-black uppercase opacity-50 mb-1 tracking-widest">Live Visibility</p><p className="text-lg font-black uppercase italic tracking-tighter leading-none">{merchantData.store.isLive ? 'Active' : 'Offline'}</p></div>
              ...
              <div className="bg-white p-2 rounded-[2.5rem] border border-slate-100 shadow-lg relative overflow-hidden h-44 group">
                 ...
                 <label className="absolute inset-0 flex flex-col items-center justify-center cursor-pointer">
                    ...
                    <span className="text-[8px] font-black uppercase mt-1 text-slate-800">Change Photo</span>
            ...
            {mTab === 'ledger' && (
               <section className="space-y-4 pb-20">
                  <div className="grid grid-cols-2 gap-4 px-1">
                    <div className="bg-white p-6 rounded-[2rem] border border-slate-100 text-center shadow-sm"><p className="text-[8px] font-black text-slate-400 mb-1 uppercase">TOTAL EARNED</p><p className="text-2xl font-black text-emerald-600 italic">₹{merchantData.rev}</p></div>
                    <div className="bg-white p-6 rounded-[2rem] border border-slate-100 text-center shadow-sm"><p className="text-[8px] font-black text-slate-400 mb-1 uppercase">PENDING</p><p className="text-2xl font-black text-blue-600 italic">{merchantData.queue.length}</p></div>
                  </div>
                  {merchantData.queue.map((b, i) => (
                    <div key={i} className="bg-white p-5 rounded-[2rem] border-l-8 border-emerald-500 shadow-sm space-y-4 mx-1 animate-in slide-in-from-left-4">
                      <div className="flex justify-between items-start">
                        <div>
                           ...
                           <p className="text-[8px] text-blue-600 font-black uppercase mt-1 tracking-widest">Seats: {b.seats || 1} • Cash</p>
                        </div>
            ...
            {mTab === 'assets' && (
              <div className="bg-white p-8 rounded-[3rem] border border-slate-100 space-y-6 mx-1 animate-in fade-in">
                <button onClick={() => {
                   const n = prompt(merchantData.store.category === 'salon' ? "Staff Name:" : "Trip Description (e.g. Mumbai 9AM):");
        ...
        {view === 'detail' && selStore && (
          <div className="pt-4 space-y-6 animate-in slide-in-from-right-4 px-1">
            <button onClick={() => setView('home')} className="flex items-center text-emerald-600 font-black text-[10px] uppercase tracking-widest px-2 active:scale-95"><Lucide.ArrowLeft size={16} className="mr-2"/> Back explorer</button>
            ...
            {selService && (
              <div className="bg-white p-6 rounded-[3rem] shadow-sm border border-slate-100 animate-in slide-in-from-bottom-6 space-y-5">
                  ...
                  <div className="space-y-2">
                    <label className="text-[8px] font-black text-slate-400 uppercase ml-2 tracking-widest">Assign {selStore.category === 'salon' ? 'Expert' : 'Trip Timing'}</label>
                  ...
                  <div className="flex gap-2">
                    ...
                    {selStore.category === 'salon' && (
                       <input type="time" value={bookForm.time} onChange={e => {
                          ...
                          const today = now.toISOString().split('T')[0];
                          const clock = now.getHours().toString().padStart(2, '0') + ":" + now.getMinutes().toString().padStart(2, '0');
                          if (bookForm.date === today && e.target.value < clock) {
                  ...
                  <input placeholder="WhatsApp Phone" value={bookForm.phone} onChange={e => setBookForm({...bookForm, phone: e.target.value})} className="w-full bg-slate-50 p-4 rounded-xl font-black text-[10px] border border-slate-100 uppercase tracking-widest shadow-inner" />
                  <button disabled={!bookForm.date || (!bookForm.time && selStore.category === 'salon') || !bookForm.phone || !bookForm.resId || !bookForm.name} onClick={() => setShowPay(true)} className="w-full bg-emerald-600 text-white py-5 rounded-2xl font-black uppercase text-[11px] tracking-[0.2em] active:scale-[0.97] transition-all shadow-xl">Get Slot Token</button>
              </div>
            )}
          </div>
        )}
          ...
          <div className="pt-10 space-y-6 animate-in fade-in px-2">
             ...
             {!adminAuth ? (
               <div className="bg-white p-8 rounded-[3rem] shadow-2xl border border-rose-100 space-y-4 text-center">
                 <input type="password" placeholder="Terminal PIN" value={adminPin} onChange={(e) => setAdminPin(e.target.value)} className="w-full bg-slate-50 border p-5 rounded-2xl border font-black text-center text-lg outline-none tracking-widest" />
                 <button onClick={() => { if (adminPin === ADMIN_PIN) setAdminAuth(true); else notify("Denied", "error"); }} className="w-full bg-rose-600 text-white py-5 rounded-2xl font-black shadow-xl uppercase active:scale-95 transition-all tracking-[0.2em]">Authorize Session</button>
               </div>
             ) : (
        ...
        <div className="fixed inset-0 bg-emerald-600 z-[1001] flex items-center justify-center p-6 animate-in zoom-in-95 duration-200 text-white text-center">
           <div className="space-y-8">
              ...
              <div className="space-y-2">
                 ...
                 <p className="text-sm font-bold uppercase opacity-70 tracking-widest leading-none">Final Step: Generate Token ID</p>
              </div>
              <div className="space-y-3 pt-6">
                 <button onClick={handleBookingFinalSubmit} className="w-64 py-6 bg-white text-emerald-600 rounded-full font-black uppercase shadow-2xl active:scale-90 transition-all text-lg tracking-widest italic">YES, CONFIRM</button>
