...
const appId = "chiplun-pro-v46-supreme"; 
...
export default function App() {
  ...
  // FIXED: Safety checks added here to prevent White Screen on add
  ...
  const getStoreStatus = (store) => {
    ...
    if (store.lunchStart && store.lunchEnd && timeStr >= store.lunchStart && timeStr <= store.lunchEnd) return { label: 'On Break', color: 'bg-amber-500', icon: <Lucide.Coffee size={10}/> };
  ...
  const calculateDynamicFare = (service, source, dest) => {
    ...
    // Simple segment logic: if travel is within partial stops, reduce fare
  ...
  const handleFinalConfirm = async () => {
    ...
    const payload = { ...bookingMeta, displayId: token, services: cart, totalPrice: finalTotal, status: 'pending', storeId: selectedStore.id, storeName: selectedStore.name, timestamp: Date.now() };
  ...
  return (
    <div className="max-w-md mx-auto bg-gray-50 min-h-screen flex flex-col shadow-2xl relative font-sans text-gray-900 overflow-x-hidden text-left">
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-[500] flex items-center justify-center p-6 animate-in zoom-in-95">
           <div className="bg-white w-full rounded-[3.5rem] p-10 shadow-2xl border-t-8 border-emerald-500">
              ...
              <div className="bg-gray-50 p-6 rounded-3xl space-y-3 mb-8 text-[10px] font-black uppercase border border-gray-100 shadow-inner text-gray-700">
                 ...
                 <div className="flex justify-between border-t border-gray-200 pt-3 text-left"><span>Payable:</span><span className="text-xl font-black text-emerald-600">₹{cart[0] ? calculateDynamicFare(cart[0], bookingMeta.source, bookingMeta.destination) * (selectedStore.category === 'travel' ? bookingMeta.numSeats : 1) : 0} (CASH)</span></div>
              </div>
      ...
      {deleteTarget && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-md z-[500] flex items-center justify-center p-6 animate-in fade-in text-center">
           <div className="bg-white w-full rounded-[3rem] p-10 shadow-2xl border-2 border-rose-100">
              ...
              <p className="text-gray-400 text-xs mt-3 mb-8 leading-relaxed font-bold uppercase tracking-widest italic text-center">Wiping <span className="underline">{deleteTarget.name}</span> will erase all records forever.</p>
      ...
      <header className="bg-emerald-600 text-white p-6 pb-12 rounded-b-[4rem] shadow-lg sticky top-0 z-50">
        <div className="flex justify-between items-center mb-6">
          <div onClick={() => setView('home')} className="cursor-pointer active:scale-95 transition-all text-left">
            ...
            <p className="text-[9px] font-bold opacity-70 uppercase tracking-widest italic font-black">Supreme V46 • MH-08</p>
          </div>
      ...
      <main className="flex-1 -mt-6 px-4 pb-32 overflow-y-auto z-10 text-left">
        ...
        {view === 'detail' && selectedStore && (
           <div className="pt-4 space-y-6 animate-in slide-in-from-right-4 pb-10 text-left">
             <button onClick={() => setView('home')} className="flex items-center text-emerald-600 font-black text-[10px] uppercase mb-4 active:scale-95 transition-all text-left"><Lucide.ArrowLeft size={16} className="mr-1"/> BACK</button>
             ...
             <section className="bg-white p-6 rounded-[3rem] shadow-sm border border-gray-100">
               ...
               {/* TRAVEL LOGISTICS: DYNAMIC VILLAGE LIST */}
               {selectedStore.category === 'travel' && cart[0] && (
                  <div className="space-y-4 mb-8 p-6 bg-blue-50/50 rounded-[2.5rem] border border-blue-100 animate-in fade-in">
                    <p className="text-[10px] font-black text-blue-600 uppercase tracking-widest text-center italic font-black">Select Village Drop</p>
                    <div className="grid grid-cols-2 gap-3 text-left">
                       <select value={bookingMeta.source} onChange={e => setBookingMeta({...bookingMeta, source: e.target.value})} className="bg-white p-3 rounded-2xl text-[10px] font-black border border-blue-200 uppercase">
                          <option>Chiplun</option>
                          {cart[0]?.stops?.split(',').map((v, idx) => <option key={idx} value={v.trim()}>{v.trim()}</option>)}
                       </select>
                       <select value={bookingMeta.destination} onChange={e => setBookingMeta({...bookingMeta, destination: e.target.value})} className="bg-white p-3 rounded-2xl text-[10px] font-black border border-blue-200 uppercase">
                    ...
                    <div className="space-y-1 text-left">
                      <p className="text-[8px] font-black text-blue-400 uppercase ml-1">Number of Seats</p>
                   ...
                   return (
                    <div key={i} onClick={() => { setCart([s]); }} className={`p-5 rounded-3xl border-2 transition-all flex justify-between items-center cursor-pointer ${cart[0]?.name === s.name ? 'border-emerald-600 bg-emerald-50 shadow-md scale-[1.02]' : 'border-gray-50 bg-gray-50'}`}>
                      <div className="text-left text-left">
                        ...
                        <p className="text-[8px] text-gray-400 mt-1.5 uppercase font-black tracking-widest">{selectedStore.category === 'travel' ? `${s.km} KM Main Route` : `${s.duration} MINS`}</p>
                      </div>
             ...
             {cart.length > 0 && (
               <div className="bg-white p-6 rounded-[3rem] shadow-sm border border-gray-100 animate-in slide-in-from-bottom-4">
                 ...
                 <div className="space-y-4 text-left">
                   ...
                   <input placeholder="WhatsApp Phone" type="tel" value={bookingMeta.custPhone} onChange={e => setBookingMeta({...bookingMeta, custPhone: e.target.value})} className="w-full bg-gray-50 p-4 rounded-2xl font-bold text-sm outline-none border border-gray-100 uppercase shadow-inner focus:ring-2 ring-emerald-500" />
                   <button disabled={!bookingMeta.date || !bookingMeta.time || isMerchantOnBreak(bookingMeta.time, selectedStore)} onClick={() => setShowConfirmModal(true)} className="w-full bg-emerald-600 text-white py-5 rounded-3xl font-black shadow-xl uppercase active:scale-95 transition-all tracking-widest disabled:opacity-40 text-center">
        ...
        {/* VIEW: MERCHANT DASHBOARD (Partner Hub) */}
        {view === 'merchant_dashboard' && isSessionVerified && (
           <div className="pt-4 space-y-6 px-4 pb-20 animate-in slide-in-from-bottom-8 text-left text-left">
              ...
              <section className="bg-emerald-600 text-white p-7 rounded-[3rem] shadow-xl space-y-4 border border-black/5 text-left text-left">
                 <h3 className="text-[10px] font-black uppercase tracking-widest flex items-center opacity-80 italic font-black font-black uppercase tracking-widest"><Lucide.Calendar size={14} className="mr-2"/> Live Ledger</h3>
                 <div className="space-y-3 max-h-72 overflow-y-auto pr-1 text-left text-left text-left text-left">
                   {merchantBookings.length > 0 ? merchantBookings.sort((a,b) => b.timestamp - a.timestamp).map((b, i) => (
                      <div key={i} className="bg-white/10 p-4 rounded-2xl border border-white/5 space-y-1 animate-in fade-in">
                         ...
                         <div className="bg-emerald-700/50 px-3 py-1 rounded-xl inline-block mt-1 text-left"><p className="text-[9px] font-black text-emerald-50 uppercase tracking-tighter italic font-black text-emerald-50 text-left">Svc: {b.services?.map(s => s.name).join(', ')} {b.numSeats ? `(${b.numSeats} Seats)` : ''}</p></div>
                         ...
                         <div className="pt-2 flex justify-between items-center text-left text-left text-left text-left">
                            ...
                            <div className="flex gap-1">
                               ...
                               <button onClick={async () => { await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'bookings', b.id), { status: 'cancelled' }); }} className="bg-rose-500 text-white px-3 py-1 rounded-lg text-[9px] font-black uppercase shadow-sm">Void</button>
                            </div>
                         </div>
                      </div>
              ...
              <section className="bg-white p-7 rounded-[3rem] shadow-sm border border-gray-100 space-y-6 text-left">
                 <div className="flex justify-between items-center text-left"><h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest flex items-center font-black italic uppercase tracking-widest text-left"><Lucide.Clock size={14} className="mr-2 text-emerald-600"/> Night/Lunch Shift</h3>{editForm.image && <img src={editForm.image} className="w-10 h-10 rounded-xl object-cover shadow-sm border border-gray-50 text-left" />}</div>
                 <div className="grid grid-cols-2 gap-2 text-left text-left">
                   <div className="space-y-1 text-left"><p className="text-[8px] font-black text-gray-400 ml-1 uppercase italic text-left font-black">Window Start</p><input type="time" value={editForm.lunchStart} onChange={e => setEditForm({...editForm, lunchStart: e.target.value})} className="w-full bg-gray-50 p-4 rounded-xl font-bold text-xs outline-none shadow-inner border uppercase text-left" /></div>
                   <div className="space-y-1 text-left"><p className="text-[8px] font-black text-gray-400 ml-1 uppercase italic text-left font-black">Window End</p><input type="time" value={editForm.lunchEnd} onChange={e => setEditForm({...editForm, lunchEnd: e.target.value})} className="w-full bg-gray-50 p-4 rounded-xl font-bold text-xs outline-none shadow-inner border uppercase text-left" /></div>
                 </div>
                 <input placeholder="Official Shop Name" value={editForm.name} onChange={e => setEditForm({...editForm, name: e.target.value})} className="w-full bg-gray-50 p-4 rounded-xl font-bold text-xs outline-none shadow-inner border text-left" />
                 <button onClick={async () => { await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'stores', profile.businessId), editForm); alert("Updated!"); }} className="w-full bg-emerald-600 text-white py-4 rounded-2xl font-black uppercase text-[10px] shadow-lg active:scale-95 transition-all text-center">Save Operations</button>
              </section>

              {/* LOGISTICS: RESOURCE CAPACITIES (SALON LOGIN BUG FIXED) */}
              <section className="bg-white p-7 rounded-[3rem] shadow-sm border border-gray-100 text-left">
                 ...
                 <div className="flex flex-wrap gap-2 mb-6 text-left text-left">
                 ...
                 <div className="flex gap-2 text-left text-left"><input placeholder={myStore?.category === 'salon' ? 'Name' : 'Expert/Vehicle Name'} value={newStaff.name} onChange={e => setNewStaff({...newStaff, name: e.target.value})} className="flex-1 bg-gray-50 p-3 rounded-lg font-bold text-sm outline-none border shadow-inner text-left" />
              ...
              <section className="bg-white p-7 rounded-[3rem] shadow-sm border border-gray-100 text-left text-left">
                 <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-6 font-black uppercase tracking-widest italic text-left">Service/Route Menu</h3>
                 ...
                 <div className="space-y-3 pt-6 border-t border-gray-100 text-left text-left text-left text-left text-left text-left">
                   ...
                   {myStore?.category === 'travel' && <input placeholder="Village Stops (e.g. Khed, Mahad, Mangaon)" value={newSvc.stops} onChange={e => setNewSvc({...newSvc, stops: e.target.value})} className="w-full bg-gray-50 p-3 rounded-xl text-[10px] font-bold border shadow-inner text-left" />}
                   <button onClick={async () => { if(!newSvc.name || !newSvc.price) return; const current = myStore?.services || []; await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'stores', profile.businessId), { services: [...current, newSvc] }); setNewSvc({name:'', price:'', km:'1', stops:''}); }} className="w-full bg-emerald-600 text-white py-3 rounded-xl font-black uppercase text-[10px] shadow-lg active:scale-95 transition-all text-center">Add to List</button>
                 </div>
              </section>
        ...
        {view === 'account' && (
           <div className="pt-4 space-y-6 px-4 pb-10 animate-in slide-in-from-bottom-4 text-left">
              <div className="flex items-center justify-between mb-8">
                 <h2 className="text-3xl font-black uppercase tracking-tighter italic text-emerald-900 leading-none text-left">Profile</h2>
              ...
              <section className="space-y-4 mb-10 text-left">
                 <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1 italic font-black text-left">My Smart Wallet</h3>
                 {myBookings.length > 0 ? (
                   <div className="space-y-3 text-left">
                      {myBookings.map((b, i) => (
                         <div key={i} className="bg-white p-5 rounded-[2rem] border border-gray-100 flex justify-between items-center shadow-sm hover:border-emerald-200 transition-all cursor-pointer text-left" onClick={() => {setTrackInput(b.displayId); setView('track'); handleTrackToken();}}>
                            <div className="text-left"><p className="font-bold text-sm text-gray-800 uppercase tracking-tight text-left">{b.storeName}</p><p className="text-[8px] text-gray-400 uppercase font-black italic text-left">{b.date} • {b.time} • {b.status}</p></div>
              ...
              <section className="bg-gray-100 p-6 rounded-[3rem] border border-gray-200 mt-12 text-left">
                 <h3 className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-4 flex items-center italic font-black uppercase tracking-widest text-left text-left text-left"><Lucide.ShieldCheck size={14} className="mr-2 text-left"/> Merchant Authorization</h3>
                 <div className="space-y-3 text-left">
                    ...
                    <button onClick={handleVendorLogin} className="w-full bg-emerald-600 text-white py-4 rounded-2xl font-black shadow-xl active:scale-95 transition-all text-xs uppercase tracking-widest text-center text-center">Login to Business Hub</button>
                 </div>
              </section>
           </div>
        )}
              ...
              {searchedBooking ? (
                <div className="bg-emerald-600 text-white p-8 rounded-[3.5rem] shadow-2xl animate-in zoom-in-95 relative overflow-hidden text-left shadow-emerald-100 text-left">
                   ...
                   <div className="space-y-4 text-sm font-bold relative z-10 border-t border-white/10 pt-6 text-white text-left">
                      ...
                      <div className="bg-white/10 p-4 rounded-3xl text-left"><p className="opacity-60 text-[8px] uppercase mb-1 italic text-emerald-100 font-black tracking-widest text-left">Reservation Time</p><p className="font-black italic text-left">{searchedBooking.date} • {searchedBooking.time}</p></div>
                   </div>
                   <button onClick={() => {setSearchedBooking(null); setTrackInput('');}} className="w-full mt-10 py-3 text-[10px] font-black uppercase bg-white/10 rounded-2xl active:scale-95 transition-all text-center">Clear & Reset</button>
                </div>
              ) : (
                <p className="text-center py-20 text-gray-300 text-[10px] font-black uppercase tracking-widest animate-pulse italic font-black leading-relaxed text-left text-center">Secure Engine • Privacy Guard Enabled</p>
              )}
           </div>
        )}

        {/* VIEW: ADMIN Portal (FIXED Multi-listing) */}
        {view === 'admin' && (
           <div className="pt-20 text-center animate-in zoom-in-95 px-4 text-center">
              ...
              <button onClick={() => { if(adminPinInput===ADMIN_PIN) setView('admin_panel'); else setAdminPinInput(''); }} className="w-full bg-emerald-600 text-white py-5 rounded-[2rem] font-black shadow-2xl active:scale-95 uppercase tracking-widest font-black uppercase italic text-center">Unlock Nexus Hub</button>
           </div>
        )}
        ...
        {view === 'admin_panel' && (
          <div className="pt-4 space-y-8 pb-32 px-4 text-left flex flex-col h-full text-left">
            <div className="flex justify-between items-center mb-6 text-left">
               ...
               <button onClick={() => setView('home')} className="bg-gray-100 p-2 rounded-xl text-gray-400 active:scale-90 text-left"><Lucide.XCircle size={18}/></button>
            </div>

            <div className="space-y-4 mb-10 text-left">
              <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1 italic font-black text-left">Verified Directory (Global Registry)</h3>
              <div className="space-y-4 max-h-[35vh] overflow-y-auto pr-1 text-left">
                {stores.map(s => (
                  <div key={s.id} className="bg-white p-5 rounded-[2.5rem] border border-gray-100 flex items-center gap-4 animate-in fade-in shadow-sm text-left">
                    <div className="w-12 h-12 bg-emerald-50 rounded-2xl flex items-center justify-center font-black text-emerald-600 uppercase shadow-inner text-xl text-left">{s.name[0]}</div>
            ...
            <div className="space-y-4 border-t border-gray-100 pt-8 text-left">
              <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1 italic font-black text-left">Pending Inbox</h3>
              <div className="space-y-4 text-left">
                 {requests.filter(r => r.status === 'pending').map(req => (
                    <div key={req.id} className="bg-amber-50 p-7 rounded-[3rem] space-y-4 border border-amber-100 shadow-sm animate-in fade-in text-left">
                       ...
                       <div className="flex gap-2 text-left">
                         <input id={`id-${req.id}`} placeholder="Merchant ID" className="flex-1 p-3 rounded-xl text-xs font-bold outline-none uppercase border border-amber-200 shadow-inner text-left" />
                         <input id={`pw-${req.id}`} placeholder="Passcode" className="w-24 p-3 rounded-xl text-xs font-bold outline-none border border-amber-200 shadow-inner text-left" />
                       </div>
                       <button onClick={async () => {
                        ...
                        }} className="w-full bg-emerald-600 text-white py-4 rounded-2xl font-black uppercase text-[10px] shadow-lg shadow-emerald-100 italic text-center">Authorize Merchant</button>
                    </div>
                 ))}
              </div>
            </div>
        ...
        {view === 'confirmation' && (
           <div className="text-center pt-10 animate-in zoom-in-90 duration-700 pb-10 px-4 text-center">
             ...
             <h2 className="text-4xl font-black text-emerald-900 mb-6 uppercase tracking-tighter italic text-center leading-none text-emerald-900 text-center">Reserved</h2>
             <div className="bg-emerald-600 text-white p-8 rounded-[3.5rem] shadow-2xl space-y-6 relative overflow-hidden text-center shadow-emerald-200 text-white text-left">
                <div className="absolute top-0 right-0 p-4 opacity-10 text-left"><Lucide.Clock size={100}/></div>
                <div className="text-left text-left"><p className="text-[10px] font-black uppercase opacity-60 mb-2 italic tracking-widest text-left text-center">Unique Booking Token</p><h3 className="text-5xl font-black tracking-widest italic text-left text-center">{lastBookingId}</h3></div>
                <div className="grid grid-cols-2 gap-4 border-t border-white/10 pt-6 font-black uppercase text-left">
                   <div className="text-center border-r border-white/10 text-left"><p className="text-[8px] opacity-60 font-black tracking-widest text-left text-center">Seats</p><p className="text-2xl font-black text-left text-center">{bookingMeta.numSeats || 1}</p></div>
                   <div className="text-center text-left"><p className="text-[8px] opacity-60 font-black tracking-widest text-left text-center">Wait</p><p className="text-2xl font-black text-left text-center">Live</p></div>
                </div>
             </div>
        ...
        {view === 'onboarding' && (
           <div className="pt-4 space-y-6 animate-in slide-in-from-bottom-4 px-4 pb-20 text-center text-left">
              <div className="text-center mb-10 text-left">
                 ...
                 <h2 className="text-3xl font-black uppercase tracking-tighter text-emerald-900 italic text-center leading-none">Partner Intake</h2>
                 <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest text-center mt-2 leading-tight">Apply to list your business on the smart network</p>
              </div>
              <div className="bg-white p-7 rounded-[3.5rem] shadow-xl space-y-4 border border-gray-100 text-left text-xs font-black uppercase text-left">
                 ...
                 <input placeholder="Area / Location" value={regForm.addr} onChange={e => setRegForm({...regForm, addr: e.target.value})} className="w-full bg-gray-50 p-4 rounded-xl font-bold text-sm outline-none border border-transparent focus:border-emerald-200 uppercase shadow-inner text-left" />
                 ...
                 <button onClick={async () => {
                    ...
                    alert("Application Submitted!"); setView('home');
                 }} className="w-full bg-emerald-600 text-white py-5 rounded-[2.5rem] font-black shadow-xl uppercase active:scale-95 transition-all tracking-widest font-black uppercase italic shadow-emerald-50 text-center">Verify Application</button>
              </div>
           </div>
        )}
      ...
      <nav className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-white/95 backdrop-blur-3xl border-t border-gray-100 px-8 py-5 pb-9 flex justify-between items-center z-[100] rounded-t-[3.5rem] shadow-2xl text-left">
        <button onClick={() => setView('home')} className={`flex flex-col items-center gap-1 transition-all ${view === 'home' ? 'text-emerald-600 scale-110' : 'text-gray-300'}`}><Lucide.Home size={22} /><span className="text-[9px] font-black uppercase tracking-widest italic text-center leading-none">Explore</span></button>
        <button onClick={() => setView(isSessionVerified ? 'merchant_dashboard' : 'onboarding')} className={`flex flex-col items-center gap-1 transition-all ${['onboarding', 'merchant_dashboard'].includes(view) ? 'text-emerald-600 scale-110' : 'text-gray-300'}`}><Lucide.Building2 size={22} /><span className="text-[9px] font-black uppercase tracking-widest italic text-center leading-none">Business</span></button>
        <button onClick={() => { setView('track'); setSearchedBooking(null); }} className={`flex flex-col items-center gap-1 transition-all ${view === 'track' ? 'text-emerald-600 scale-110' : 'text-gray-300'}`}><Lucide.Calendar size={22} /><span className="text-[9px] font-black uppercase tracking-widest italic text-center leading-none">Track</span></button>
        <button onClick={() => setView('account')} className={`flex flex-col items-center gap-1 transition-all ${view === 'account' ? 'text-emerald-600 scale-110' : 'text-gray-300'}`}><Lucide.UserCircle size={22} /><span className="text-[9px] font-black uppercase tracking-widest italic font-black text-center leading-none">Account</span></button>
      </nav>
