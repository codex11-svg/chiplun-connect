...
export default function App() {
  ...
  useEffect(() => {
    ...
    setLoading(false);
    return () => {
        unsubProfile(); unsubStores(); unsubBookings(); unsubRequests();
    };
  }, [user]);
  ...
  const handleBooking = async () => {
    ...
    try {
      ...
      setTracked(payload);
      setView('track');
    } catch (e) { notify("Error creating booking", "error"); }
  ...
  const handleAdminApprove = async (req) => {
    ...
    try {
      ...
      notify("Approved! Merchant can now configure dashboard.");
    } catch (e) { notify("Approval failed", "error"); }
    setIsProcessing(false);
  };

  const handleVLogin = async () => {
    if (!vLogin.id || !vLogin.pass) return notify("Enter credentials");
    setIsProcessing(true);
    try {
  ...
  if (loading) return (
    <div className="h-screen flex items-center justify-center bg-emerald-600 text-white font-black animate-pulse uppercase tracking-[0.3em]">
      ...
      <header className="bg-emerald-600 text-white p-6 pb-12 rounded-b-[3.5rem] shadow-lg sticky top-0 z-50">
        ...
        {view === 'home' && (
          <div className="relative animate-in slide-in-from-top-4">
            ...
            <input type="text" placeholder="Find salons, travel, clinics..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-white/10 border border-white/20 rounded-2xl py-4 pl-12 pr-4 text-white placeholder-emerald-200 outline-none focus:bg-white/20" />
          </div>
        )}
      </header>
      ...
      <main className="flex-1 -mt-6 px-4 pb-32 z-10">
          ...
          <div className="space-y-8 pt-2">
            ...
            <section className="space-y-4">
              ...
              <div className="space-y-4">
                {filtered.map(store => (
                  <div key={store.id} onClick={() => { setSelectedStore(store); setView('detail'); setCart([]); setBookMeta({ ...bookMeta, resId: '' }); }} className="bg-white p-3 rounded-[2.5rem] flex gap-4 items-center shadow-sm border border-slate-100 group active:scale-[0.98] transition-all">
                    <img src={store.image} className="w-20 h-20 rounded-[1.8rem] object-cover bg-slate-50 shadow-inner" />
                ...
                {filtered.length === 0 && <p className="text-center py-20 text-slate-300 font-black uppercase text-[10px] tracking-widest">Wait for partners to go Live</p>}
              </div>
            </section>
          </div>
        )}
        ...
        {view === 'business' && profile.role !== 'vendor' && (
          <div className="pt-6 space-y-6 animate-in slide-in-from-bottom-8">
            <div className="text-center">
              ...
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Connect to Chiplun's Digital Economy</p>
            </div>
            ...
            {bizSubView === 'register' ? (
              <div className="bg-white p-8 rounded-[3rem] shadow-xl border border-slate-100 space-y-4">
                ...
                <button onClick={() => setView('admin')} className="w-full text-[8px] font-black text-slate-300 uppercase tracking-widest pt-4 hover:text-rose-500 transition-all">Administrator Suite</button>
              </div>
            ) : (
              <div className="bg-white p-8 rounded-[3rem] shadow-xl border border-slate-100 space-y-4 text-center">
                ...
                <button onClick={handleVLogin} className="w-full bg-emerald-600 text-white py-5 rounded-[1.5rem] font-black uppercase text-[10px] tracking-[0.2em] shadow-xl active:scale-95">Unlock Terminal</button>
              </div>
            )}
          </div>
        )}

        {/* VIEW: MERCHANT CONSOLE (REFACTORED LOGIC) */}
        {view === 'merchant' && activeMerchant && (
          <div className="pt-6 space-y-6 animate-in slide-in-from-bottom-8 px-1">
            ...
            {/* LIVE TOGGLE - Only merchant can flip this */}
            <div className={`p-6 rounded-[2.5rem] flex items-center justify-between shadow-lg transition-all ${activeMerchant.store.isLive ? 'bg-emerald-600 text-white' : 'bg-slate-900 text-white'}`}>
              <div><p className="text-[9px] font-black uppercase opacity-70 tracking-widest">Visibility</p><p className="text-xl font-black uppercase italic tracking-tighter">{activeMerchant.store.isLive ? 'Publicly Live' : 'Hidden (Offline)'}</p></div>
            ...
            {mTab === 'ledger' && (
               <section className="space-y-4 pb-20">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-white p-6 rounded-[2rem] border border-slate-100 text-center"><p className="text-[8px] font-black text-slate-400 mb-1 tracking-widest">REVENUE</p><p className="text-2xl font-black text-emerald-600 tracking-tighter italic">₹{activeMerchant.rev}</p></div>
                    <div className="bg-white p-6 rounded-[2rem] border border-slate-100 text-center"><p className="text-[8px] font-black text-slate-400 mb-1 tracking-widest">PENDING</p><p className="text-2xl font-black text-blue-600 tracking-tighter italic">{activeMerchant.pending.length}</p></div>
                  </div>
                  {activeMerchant.pending.map((b, i) => (
                    <div key={i} className="bg-white p-5 rounded-[2rem] border-l-8 border-emerald-500 shadow-sm flex justify-between items-center animate-in slide-in-from-left-4">
                      <div className="flex-1">
                        ...
                        {b.pickup && <p className="text-[8px] text-blue-600 font-black uppercase mt-1 tracking-widest">{b.pickup} ➔ {b.drop} ({b.seats} Seats)</p>}
                      </div>
            ...
            {/* TAB: INVENTORY (RESOURCES) */}
            {mTab === 'inventory' && (
              <div className="bg-white p-8 rounded-[3rem] border border-slate-100 space-y-6 animate-in fade-in">
                <h3 className="text-[10px] font-black uppercase text-slate-400 italic px-2">Operational Assets</h3>
                <button onClick={() => {
                   const n = prompt(activeMerchant.store.category === 'salon' ? "Staff Name:" : "Trip Name (e.g. Mumbai Bus):");
                ...
                }} className="w-full py-4 border-2 border-dashed border-slate-200 rounded-2xl font-black uppercase text-[10px] text-slate-400 hover:border-emerald-500 transition-all">+ Add {activeMerchant.store.category === 'salon' ? 'Staff' : 'Resource'}</button>
                <div className="space-y-3">
                   {activeMerchant.store.resources?.map((r, i) => (
                     <div key={i} className="flex justify-between items-center p-4 bg-slate-50 rounded-2xl border border-slate-100">
                        <div><p className="font-black text-xs uppercase italic leading-none">{r.name}</p>{activeMerchant.store.category === 'travel' && <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mt-1">Total Seats: {r.capacity}</p>}</div>
            ...
            {mTab === 'services' && (
               <div className="bg-white p-8 rounded-[3rem] border border-slate-100 space-y-6 animate-in fade-in">
                  <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-center italic opacity-60">Service List</h3>
                  <button onClick={() => {
                     const n = prompt("Service Name:");
                     const p = prompt(activeMerchant.store.category === 'travel' ? "Fare Rate per KM (₹):" : "Flat Price (₹):");
                     if (n && p) updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'stores', profile.businessId), { services: arrayUnion({ name: n, price: Number(p) }) });
                  }} className="w-full py-4 border-2 border-dashed border-slate-200 rounded-2xl font-black uppercase text-[10px] text-slate-400 hover:border-emerald-500 transition-all">+ Add Pricing</button>
                  <div className="space-y-3">
        ...
        {/* VIEW: ADMINMaster */}
        {view === 'admin' && (
          <div className="pt-10 space-y-6 animate-in fade-in px-1">
             <div className="flex justify-between items-center">
             ...
             {!adminAuth ? (
               <div className="bg-white p-8 rounded-[3rem] shadow-2xl border border-rose-100 space-y-4 text-center">
                 ...
                 <button onClick={() => { if (adminPin === ADMIN_PIN) setAdminAuth(true); else notify("Access Denied", "error"); }} className="w-full bg-rose-600 text-white py-5 rounded-2xl font-black shadow-xl uppercase active:scale-95 transition-all tracking-widest">Verify Access</button>
               </div>
             ) : (
               <div className="space-y-6 pb-20">
                 ...
                 {adminTab === 'requests' ? requests.map(r => (
                    <div key={r.id} className="bg-white p-6 rounded-[2.5rem] border border-slate-100 space-y-4 animate-in slide-in-from-bottom-4">
        ...
        {view === 'detail' && selectedStore && (
          <div className="pt-4 space-y-6 animate-in slide-in-from-right-4">
            <button onClick={() => setView('home')} className="flex items-center text-emerald-600 font-black text-[10px] uppercase tracking-widest active:scale-95 transition-all"><Lucide.ArrowLeft size={16} className="mr-2"/> Back</button>
            ...
            <section className="bg-white p-6 rounded-[3rem] border border-slate-100 shadow-sm space-y-4">
              ...
              <div className="space-y-2">
                ...
                )) : <p className="text-center text-[9px] text-slate-300 font-black uppercase py-4">No rates listed</p>}
              </div>
            </section>

            {cart.length > 0 && (
              <div className="bg-white p-6 rounded-[3rem] shadow-sm border border-slate-100 animate-in slide-in-from-bottom-6 space-y-5">
                  <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-center italic leading-none">2. Booking Logistics</h3>
                  
                  {selectedStore.category === 'travel' && (
                    <div className="space-y-3">
                       <div className="grid grid-cols-2 gap-2">
                          <div className="space-y-1">
                             <label className="text-[8px] font-black text-slate-400 uppercase ml-2 tracking-widest">Origin</label>
                          ...
                          <div className="space-y-1">
                             <label className="text-[8px] font-black text-slate-400 uppercase ml-2 tracking-widest">End Point</label>
                  ...
                  <div className="space-y-2">
                    <label className="text-[8px] font-black text-slate-400 uppercase ml-2 tracking-widest">Assign {selectedStore.category === 'salon' ? 'Barber' : 'Trip Timing'}</label>
                    <div className="space-y-2">
                        ...
                        return (
                          <div key={r.id} onClick={() => !isFull && setBookMeta({...bookMeta, resId: r.id})} className={`p-4 rounded-2xl border-2 transition-all cursor-pointer flex justify-between items-center ${bookMeta.resId === r.id ? 'border-emerald-600 bg-emerald-50' : 'border-slate-50 opacity-60'} ${isFull ? 'opacity-30 cursor-not-allowed' : ''}`}>
                            ...
                            <span className={`text-[8px] font-black uppercase ${isFull ? 'text-rose-500' : 'text-emerald-600'} tracking-tighter`}>
                              {selectedStore.category === 'salon' ? `Wait: ${count} (~${count * 20}m)` : isFull ? 'Sold Out' : `${remaining} S Left`}
                            </span>
                          </div>
                        );
                      })}
                  ...
                  {selectedStore.category === 'travel' && bookMeta.resId && (
                    <div className="grid grid-cols-2 gap-3 bg-blue-50/50 p-4 rounded-2xl border border-blue-100 items-center">
                       <div className="space-y-1">
                          <label className="text-[8px] font-black text-blue-400 uppercase">Passenger(s)</label>
                       ...
                       <div className="text-right">
                          <label className="text-[8px] font-black text-blue-400 uppercase block tracking-widest">Segment Total</label>
                  ...
                  <input placeholder="WhatsApp Number" value={bookMeta.phone} onChange={e => setBookMeta({...bookMeta, phone: e.target.value})} className="w-full bg-slate-50 p-4 rounded-xl font-black text-[10px] outline-none border border-slate-100 uppercase tracking-widest" />
          ...
          <div className="pt-10 space-y-8 animate-in slide-in-from-bottom-8">
            ...
            <div className="bg-white p-8 rounded-[3.5rem] shadow-2xl border-t-8 border-emerald-500 text-center space-y-6">
                ...
                <div className="bg-slate-50 p-5 rounded-3xl text-[9px] font-black uppercase text-left space-y-2 border border-slate-100">
                  <div className="flex justify-between italic tracking-widest"><span>Merchant:</span><span className="text-emerald-700">{tracked.storeName}</span></div>
                  <div className="flex justify-between"><span>Selection:</span><span>{tracked.serviceName}</span></div>
                  {tracked.pickup && <div className="flex justify-between text-blue-600 border-t border-slate-200 mt-1 pt-1 tracking-tighter"><span>Route:</span><span>{tracked.pickup} ➔ {tracked.drop}</span></div>}
                  <div className="flex justify-between pt-2 border-t border-slate-200 text-xs font-black"><span>Amount Payable:</span><span className="text-emerald-600 text-lg">₹{tracked.totalPrice}</span></div>
                </div>
                <p className="text-[8px] font-bold text-slate-400 uppercase italic px-4 leading-relaxed">Present this ticket ID at the location to claim your slot.</p>
