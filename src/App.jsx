...
const appId = "chiplun-pro-v20-final"; 
...
export default function App() {
  ...
  const [bookingMeta, setBookingMeta] = useState({
    ...
    patientAge: '', brandModel: '' // Clinic/Repair
  });
  ...
  const handleFinalConfirm = async () => {
    ...
    // Logic: Position based on existing bookings for same time
    ...
    const payload = { 
      ...
      queuePos: ahead + 1, estWait: ahead * 15, // 15 mins per worker occupied
    ...
    try {
      ...
      setQueueInfo({ pos: ahead + 1, delay: ahead * 15 });
  ...
  const handleVendorLogin = async () => {
    ...
    if (snap.exists() && snap.data().password === vendorLogin.pass) {
      ...
      setView('merchant_hub');
    } else alert("Invalid Credentials");
    setIsProcessing(false);
  };

  if (loading) return <div className="h-screen flex items-center justify-center bg-emerald-600 text-white font-black animate-pulse uppercase tracking-widest text-xs">ChiplunConnect Pro V20</div>;
    ...
    <div className="max-w-md mx-auto bg-gray-50 min-h-screen flex flex-col shadow-2xl relative font-sans text-gray-900 overflow-x-hidden">
      ...
      {deleteTarget && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-md z-[500] flex items-center justify-center p-6">
           <div className="bg-white w-full rounded-[3rem] p-8 shadow-2xl text-center border-2 border-rose-100">
              <Lucide.Trash2 size={40} className="text-rose-500 mx-auto mb-4" />
              <h3 className="text-xl font-black italic">Destroy Store?</h3>
              <p className="text-gray-400 text-xs mt-3 mb-8">Erase <span className="font-bold text-gray-900">"{deleteTarget.name}"</span>? Merchant ID will stop working instantly.</p>
              <div className="flex flex-col gap-3">
                 <button onClick={handleAdminWipe} className="w-full bg-rose-500 text-white py-4 rounded-2xl font-black uppercase text-xs">Purge Data</button>
      ...
      {showConfirmModal && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-[500] flex items-center justify-center p-6 animate-in zoom-in-95">
           <div className="bg-white w-full rounded-[3.5rem] p-10 shadow-2xl">
              ...
              <div className="bg-gray-50 p-6 rounded-3xl space-y-2 mb-8 text-[10px] font-black uppercase">
                 <div className="flex justify-between"><span>Expert:</span><span>{bookingMeta.resourceName || 'Assigned'}</span></div>
                 <div className="flex justify-between"><span>Time:</span><span>{bookingMeta.time}</span></div>
                 <div className="flex justify-between pt-2 border-t font-black"><span>Total:</span><span className="text-emerald-600">₹{cart.reduce((a, b) => a + Number(b.price), 0)}</span></div>
              </div>
              <button disabled={isProcessing} onClick={handleFinalConfirm} className="w-full bg-emerald-600 text-white py-5 rounded-[2.5rem] font-black uppercase text-xs">
      ...
      <header className="bg-emerald-600 text-white p-6 pb-12 rounded-b-[4rem] shadow-lg sticky top-0 z-50">
        <div className="flex justify-between items-center mb-6">
          <div onClick={() => setView('home')} className="cursor-pointer">
            ...
            <p className="text-[9px] font-bold opacity-70 uppercase tracking-widest">Enterprise Hub MH-08</p>
          </div>
        ...
        {view === 'home' && (
          <div className="relative animate-in slide-in-from-top-2">
            ...
            <input type="text" placeholder="Search verified services..." className="w-full bg-white/10 border border-white/20 rounded-2xl py-3.5 pl-12 pr-4 text-white placeholder-emerald-200 outline-none focus:bg-white/20" />
          </div>
        )}
      </header>
      ...
      <main className="flex-1 -mt-6 px-4 pb-32 overflow-y-auto z-10">
        ...
        {view === 'home' && (
          <div className="space-y-8 pt-2">
             ...
             <section className="animate-in slide-in-from-bottom-4">
               <h2 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-4 ml-1 italic">Verified Businesses</h2>
               <div className="space-y-4">
                 {stores.filter(s => s.isLive).map(store => (
                   <div key={store.id} onClick={() => { setSelectedStore(store); setView('store_detail'); setCart([]); setBookingMeta({ ...bookingMeta, resourceName: '' }); }} className="bg-white p-3 rounded-[2.5rem] flex gap-4 items-center shadow-sm border border-gray-100 hover:border-emerald-200 active:scale-[0.98] transition-all">
             ...
             <section className="bg-white p-6 rounded-[3rem] shadow-sm border border-gray-100">
               ...
               {selectedStore.category === 'travel' && (
                 <div className="grid grid-cols-2 gap-2 mb-6 animate-in fade-in">
                    <input placeholder="Source" onChange={e => setBookingMeta({...bookingMeta, source: e.target.value})} className="bg-gray-50 p-3 rounded-xl text-xs font-bold border border-gray-100 uppercase" />
                    <input placeholder="Destination" onChange={e => setBookingMeta({...bookingMeta, destination: e.target.value})} className="bg-gray-50 p-3 rounded-xl text-xs font-bold border border-gray-100 uppercase" />
                 </div>
               )}
             ...
             {cart.length > 0 && (
               <div className="bg-white p-6 rounded-[3rem] shadow-sm border border-gray-100 animate-in slide-in-from-bottom-4">
                 <div className="space-y-4">
                   ...
                   >
                      <option value="">{selectedStore.category === 'salon' ? 'Choose Barbers' : 'Choose Specialist'}</option>
                      {selectedStore.staff?.map((st, i) => <option key={i} value={st}>{st}</option>)}
                   </select>
                   <input placeholder="Phone Number" type="tel" value={bookingMeta.custPhone} onChange={e => setBookingMeta({...bookingMeta, custPhone: e.target.value})} className="w-full bg-gray-50 p-4 rounded-2xl font-bold text-sm outline-none border border-gray-100 uppercase shadow-inner" />
                   <button disabled={!bookingMeta.date || !bookingMeta.time} onClick={() => setShowConfirmModal(true)} className="w-full bg-emerald-600 text-white py-5 rounded-3xl font-black shadow-xl uppercase tracking-widest active:scale-95 transition-all">Review & Secure</button>
                 </div>
              </div>
            )}
           </div>
        ...
        {/* VIEW: MERCHANT HUB (Fixed Navigation) */}
        {view === 'merchant_hub' && (
              ...
              {!isSessionVerified ? (
                 <div className="animate-in zoom-in-95">
                    <div className="text-center mb-10">
                       ...
                       <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest text-center">Authorization Required</p>
                    </div>
                    <div className="bg-white p-8 rounded-[3.5rem] shadow-2xl space-y-4 border border-gray-100">
                       ...
                       <button onClick={handleVendorLogin} className="w-full bg-emerald-600 text-white py-5 rounded-[2rem] font-black shadow-xl uppercase tracking-widest active:scale-95 transition-all">Unlock Session</button>
                    </div>
                 </div>
              ) : (
                <div className="space-y-6">
                   ...
                   {/* LIVE LEDGER */}
                   <section className="bg-emerald-600 text-white p-7 rounded-[3rem] shadow-2xl space-y-4">
                      <h3 className="text-[10px] font-black uppercase tracking-widest flex items-center opacity-80 font-black italic"><Lucide.Calendar size={14} className="mr-2"/> Today's Queue</h3>
                      <div className="space-y-3 max-h-60 overflow-y-auto">
                        {merchantBookings.length > 0 ? merchantBookings.sort((a,b) => b.timestamp - a.timestamp).map((b, i) => (
                           <div key={i} className="bg-white/10 p-4 rounded-2xl border border-white/5 space-y-1">
                              ...
                              <p className="text-[9px] opacity-70 uppercase font-bold">{b.date} • {b.time}</p>
                        ...
                        )) : <p className="text-center py-10 text-[9px] opacity-60 font-bold uppercase tracking-widest">Zero appointments</p>}
                      </div>
                   </section>

                   <section className="bg-white p-7 rounded-[3rem] shadow-sm border border-gray-100 space-y-6">
                      <div className="flex justify-between items-center"><h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest flex items-center font-black uppercase"><Lucide.Image size={14} className="mr-2 text-emerald-600"/> Modify Storefront</h3>{editForm.image && <img src={editForm.image} className="w-10 h-10 rounded-xl object-cover" />}</div>
                      <input placeholder="Branding URL" value={editForm.image} onChange={e => setEditForm({...editForm, image: e.target.value})} className="w-full bg-gray-50 p-4 rounded-xl font-bold text-xs outline-none shadow-inner uppercase" />
                      <input placeholder="Business Name" value={editForm.name} onChange={e => setEditForm({...editForm, name: e.target.value})} className="w-full bg-gray-50 p-4 rounded-xl font-bold text-xs outline-none shadow-inner uppercase" />
                      <button onClick={async () => { await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'stores', profile.businessId), editForm); alert("Updated!"); }} className="w-full bg-emerald-600 text-white py-3 rounded-xl font-black uppercase text-[10px] shadow-lg shadow-emerald-100">Save Appearance</button>
                   </section>
                   ...
                   <section className="bg-white p-7 rounded-[3rem] shadow-sm border border-gray-100">
                      ...
                      <div className="space-y-3 pt-6 border-t border-gray-100 text-left">
                        ...
                        }} className="w-full bg-emerald-600 text-white py-3 rounded-xl font-black uppercase text-[10px]">Add to Menu</button>
                      </div>
                   </section>

                   <section className="bg-white p-7 rounded-[3rem] shadow-sm border border-gray-100">
                      <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-6 font-black uppercase">Staff (Capacity)</h3>
                      ...
                      <div className="flex gap-2"><input placeholder="Add Name" value={newStaff} onChange={e => setNewStaff(e.target.value)} className="flex-1 bg-gray-50 p-3 rounded-lg font-bold text-sm outline-none shadow-inner" /><button onClick={() => { if(!newStaff) return; updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'stores', profile.businessId), { staff: [...(myStore.staff || []), newStaff] }); setNewStaff(''); }} className="bg-emerald-600 text-white p-3 rounded-lg active:scale-95 shadow-lg"><Lucide.Plus size={18}/></button></div>
                   </section>
                   ...
                   <button onClick={() => updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'stores', profile.businessId), { isLive: !myStore?.isLive })} className={`w-full py-5 rounded-[2.5rem] font-black uppercase shadow-2xl active:scale-95 transition-all ${myStore?.isLive ? 'bg-rose-500 text-white shadow-rose-100' : 'bg-emerald-600 text-white shadow-emerald-100'}`}>
                      {myStore?.isLive ? 'Set Offline' : 'Set Business Live'}
                   </button>
                </div>
              )}
           </div>
        ...
        {/* VIEW: TRACK TOKEN (Reliable Search) */}
        {view === 'track_token' && (
           <div className="pt-4 space-y-6 px-4 animate-in slide-in-from-bottom-4">
                ...
                <div className="bg-emerald-600 text-white p-8 rounded-[3.5rem] shadow-2xl animate-in zoom-in-95 relative overflow-hidden">
                   ...
                   <div className="space-y-4 text-sm font-bold relative z-10">
                      <div className="grid grid-cols-2 gap-4 border-t border-white/10 pt-6 font-black uppercase">
                         ...
                         <div><p className="opacity-60 text-[8px] mb-1">Provider</p>{searchedBooking.resourceName || 'First Available'}</div>
                      </div>
                      <div className="bg-white/10 p-4 rounded-3xl">
                         <p className="opacity-60 text-[8px] uppercase mb-1 italic">Schedule Detail</p>
                         <p>{searchedBooking.date} • {searchedBooking.time}</p>
                      </div>
                   </div>
                   <button onClick={() => setSearchedBooking(null)} className="w-full mt-10 py-3 text-[10px] font-black uppercase bg-white/10 rounded-2xl">New Track</button>
                </div>
              ) : (
        ...
        {view === 'vendor_portal' && (
           <div className="pt-4 space-y-6 animate-in slide-in-from-bottom-4 px-4 pb-10 text-center">
              ...
              <div className="bg-white p-7 rounded-[3.5rem] shadow-xl space-y-4 border border-gray-100 text-left">
                 ...
                 <input placeholder="Area Location" value={regForm.addr} onChange={e => setRegForm({...regForm, addr: e.target.value})} className="w-full bg-gray-50 p-4 rounded-xl font-bold text-sm outline-none border border-transparent focus:border-emerald-200 uppercase" />
        ...
        {view === 'confirmation' && (
           <div className="text-center pt-10 animate-in zoom-in-90 duration-700 pb-10 px-4">
             ...
             <button onClick={() => setView('home')} className="w-full bg-emerald-700 text-white py-5 rounded-[2.5rem] font-black shadow-2xl active:scale-95 uppercase tracking-widest mt-12">Finish</button>
           </div>
        )}
      ...
      <nav className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-white/90 backdrop-blur-3xl border-t border-gray-100 px-8 py-5 pb-9 flex justify-between items-center z-50 rounded-t-[3.5rem] shadow-2xl">
        ...
        <button onClick={() => { setView('track_token'); setSearchedBooking(null); }} className={`flex flex-col items-center gap-1.5 transition-all ${view === 'track_token' ? 'text-emerald-600 scale-110' : 'text-gray-300'}`}><Lucide.Calendar size={22} /><span className="text-[9px] font-black uppercase tracking-widest">Track</span></button>
        <button onClick={() => setView(isSessionVerified ? 'dashboard' : 'merchant_login')} className={`flex flex-col items-center gap-1.5 transition-all ${['merchant_login', 'dashboard'].includes(view) ? 'text-emerald-600 scale-110' : 'text-gray-300'}`}><Lucide.UserCircle size={22} /><span className="text-[9px] font-black uppercase tracking-widest">Account</span></button>
      </nav>
