...
const appId = "chiplun-pro-v23-production"; 
const ADMIN_PIN = "112607";

export default function App() {
  const [user, setUser] = useState(null);
  const [profile, setProfile] = useState({ role: 'customer', status: 'none' });
  const [view, setView] = useState('home'); // home, onboarding, track, account, admin, detail
  ...
  // MERCHANT LEDGER: Filter bookings for current logged merchant
  ...
  const handleTrackToken = () => {
    ...
    else alert("Token not found. Verify the ID.");
  };
  ...
  const handleFinalConfirm = async () => {
    ...
    } catch (e) { alert("Network error. Try again."); setIsProcessing(false); }
  };
  ...
  if (loading) return <div className="h-screen flex items-center justify-center bg-emerald-600 text-white font-black animate-pulse uppercase tracking-widest text-xs">ChiplunConnect Pro V23</div>;
    ...
    <div className="max-w-md mx-auto bg-gray-50 min-h-screen flex flex-col shadow-2xl relative font-sans text-gray-900 overflow-x-hidden">
      ...
      {deleteTarget && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-md z-[600] flex items-center justify-center p-6 animate-in fade-in">
           <div className="bg-white w-full rounded-[3rem] p-10 shadow-2xl text-center border-4 border-rose-500">
              ...
              <h3 className="text-xl font-black text-gray-900 uppercase tracking-tighter">Destroy Business?</h3>
              <p className="text-gray-400 text-xs mt-3 mb-10 leading-relaxed">Permanently wipes <span className="text-rose-600 font-bold underline">"{deleteTarget.name}"</span>. Login ID will be deactivated forever.</p>
              <button onClick={handleAdminWipe} className="w-full bg-rose-500 text-white py-5 rounded-2xl font-black uppercase text-xs mb-3 shadow-xl">Wipe All Data</button>
      ...
      <header className="bg-emerald-600 text-white p-6 pb-12 rounded-b-[4rem] shadow-lg sticky top-0 z-50">
        <div className="flex justify-between items-center mb-6">
          ...
          <button onClick={() => setView('admin')} className="w-10 h-10 bg-white/10 rounded-2xl flex items-center justify-center border border-white/5 shadow-inner"><Lucide.Lock size={20}/></button>
        </div>
        {view === 'home' && (
             ...
             {cart.length > 0 && (
               <div className="bg-white p-6 rounded-[3rem] shadow-sm border border-gray-100 animate-in slide-in-from-bottom-4">
                 ...
                 <div className="space-y-4">
                   ...
                   <button disabled={!bookingMeta.date || !bookingMeta.time} onClick={() => setShowConfirmModal(true)} className="w-full bg-emerald-600 text-white py-5 rounded-3xl font-black shadow-xl uppercase active:scale-95 transition-all tracking-widest">Review & Book</button>
                 </div>
              </div>
            )}
           </div>
        ...
        {view === 'track' && (
           <div className="pt-4 space-y-6 px-4 animate-in slide-in-from-bottom-4 pb-20">
              ...
              {searchedBooking ? (
                <div className="bg-emerald-600 text-white p-8 rounded-[3.5rem] shadow-2xl animate-in zoom-in-95 relative overflow-hidden">
                   ...
                   <div className="space-y-4 text-sm font-bold relative z-10">
                      <div className="grid grid-cols-2 gap-4 border-t border-white/10 pt-6 font-black uppercase">
                         <div><p className="opacity-60 text-[8px] mb-1">Queue Pos</p>#{searchedBooking.queuePos || '1'}</div>
                         <div><p className="opacity-60 text-[8px] mb-1">Provider</p>{searchedBooking.resourceName || 'First Free'}</div>
                      </div>
                ...
                <p className="text-center py-20 text-gray-300 text-[10px] font-black uppercase tracking-widest animate-pulse italic">Enter unique token code to track status</p>
              )}
           </div>
        )}

        {/* VIEW: MERCHANT HUB (Fixed Nav & Ledger) */}
        {view === 'account' && (
              ...
              {!isSessionVerified ? (
                 <div className="animate-in zoom-in-95 pt-20">
                    <div className="text-center mb-10">
                       ...
                       <h2 className="text-3xl font-black uppercase tracking-tighter italic text-emerald-900 text-center">Merchant Hub</h2>
                    </div>
                ...
                <div className="space-y-6 animate-in slide-in-from-bottom-8">
                   ...
                   {/* MERCHANT LEDGER: Shows user name, token, and booked service */}
                   <section className="bg-emerald-600 text-white p-7 rounded-[3rem] shadow-xl space-y-4 border border-black/5">
                      <h3 className="text-[10px] font-black uppercase tracking-widest flex items-center opacity-80 font-black italic"><Lucide.Calendar size={14} className="mr-2"/> Live Ledger</h3>
                      <div className="space-y-3 max-h-72 overflow-y-auto pr-1">
                        {merchantBookings.length > 0 ? merchantBookings.sort((a,b) => b.timestamp - a.timestamp).map((b, i) => (
                           <div key={i} className="bg-white/10 p-4 rounded-2xl border border-white/5 space-y-1 animate-in fade-in">
                              ...
                              {/* BOOKED SERVICE DISPLAY */}
                              <div className="bg-emerald-700/50 px-3 py-1 rounded-xl inline-block mt-1">
                                 <p className="text-[9px] font-black text-emerald-50 uppercase tracking-tighter">Services: {b.services?.map(s => s.name).join(', ')}</p>
                              </div>
                        ...
                        )) : <p className="text-center py-10 text-[9px] opacity-60 font-bold uppercase tracking-widest italic font-black">Zero activity today</p>}
                      </div>
                   </section>
                   ...
                   <section className="bg-white p-7 rounded-[3rem] shadow-sm border border-gray-100 space-y-5 animate-in fade-in">
                      ...
                      <input placeholder="Image URL Link" value={editForm.image} onChange={e => setEditForm({...editForm, image: e.target.value})} className="w-full bg-gray-50 p-4 rounded-xl font-bold text-xs outline-none shadow-inner uppercase border" />
                      ...
                      <button onClick={async () => { await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'stores', profile.businessId), editForm); alert("Profile Synced!"); }} className="w-full bg-emerald-600 text-white py-3 rounded-xl font-black uppercase text-[10px] shadow-lg">Save Changes</button>
                   </section>
                   ...
                   <section className="bg-white p-7 rounded-[3rem] shadow-sm border border-gray-100">
                      ...
                      <div className="space-y-3 pt-6 border-t border-gray-100">
                        <input placeholder="Service Title" value={newSvc.name} onChange={e => setNewSvc({...newSvc, name: e.target.value})} className="w-full bg-gray-50 p-3 rounded-xl text-xs outline-none uppercase font-bold" />
                        ...
                        }} className="w-full bg-emerald-600 text-white py-3 rounded-xl font-black uppercase text-[10px] shadow-lg shadow-emerald-50">Publish to Menu</button>
                      </div>
                   </section>

                   <section className="bg-white p-7 rounded-[3rem] shadow-sm border border-gray-100">
                      <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-6 font-black uppercase tracking-widest">Add Staff (Capacity)</h3>
                      ...
                      <div className="flex gap-2"><input placeholder="Add Worker" value={newStaff} onChange={e => setNewStaff(e.target.value)} className="flex-1 bg-gray-50 p-3 rounded-lg font-bold text-sm outline-none border shadow-inner" /><button onClick={() => { if(!newStaff) return; updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'stores', profile.businessId), { staff: [...(myStore.staff || []), newStaff] }); setNewStaff(''); }} className="bg-emerald-600 text-white p-3 rounded-lg active:scale-95 shadow-lg"><Lucide.Plus size={18}/></button></div>
                   </section>
                </div>
              )}
           </div>
        ...
        {view === 'onboarding' && (
           <div className="pt-4 space-y-6 animate-in slide-in-from-bottom-4 px-4 pb-20">
              <div className="text-center mb-10">
                 ...
                 <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">MH-08 Verification Intake</p>
              </div>
        ...
        {view === 'admin_panel' && (
          <div className="pt-4 space-y-6 pb-20 px-4">
            ...
            <div className="space-y-4">
              ...
              <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1 mt-10">Pending Applications</h3>
      ...
      {/* BOTTOM NAVIGATION (Hard-Wired Strings Fix) */}
