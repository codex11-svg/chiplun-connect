...
// Safety Singleton check for React rendering
const app = getApps().length === 0 ? initializeApp(firebaseConfig) : getApps()[0];
const auth = getAuth(app);
const db = getFirestore(app);
const appId = "chiplun-pro-v50-master-ironclad";

// MASTER SECURITY IDENTITY
...
export default function App() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (u) => {
      if (!u) {
        try { await signInAnonymously(auth); } catch(e) { console.error("Identity Handshake Failed"); }
      }
      setUser(u);
    });
    return () => unsubscribe();
  ...
  useEffect(() => {
    ...
    const unsubs = [
      onSnapshot(profileRef, (s) => {
        if (s.exists()) {
          const data = s.data();
          // SECURITY RULE: If UID matches Master but role is outdated, force upgrade
      ...
      }, (err) => console.log("Profile Sync restricted")),
  ...
  const handleAdminAuth = async () => {
    ...
    } catch (e) { notify("Invalid Access Credentials", "error"); }
  ...
  const handleBookingSubmission = async () => {
    ...
    } catch (e) { notify("Database Failure", "error"); }
    setIsProcessing(false);
  };

  const adminApprove = async (req) => {
    const mid = prompt("Assign Merchant ID (CH-XXX):");
    const key = prompt("Assign Secret Key:");
    if (!mid || !key) return;
    try {
      ...
      <header className="bg-emerald-600 text-white p-6 pb-12 rounded-b-[3.5rem] shadow-lg sticky top-0 z-50">
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex gap-2">
            <button onClick={() => setView('admin')} className={`w-10 h-10 rounded-2xl flex items-center justify-center border transition-all ${user?.uid === MASTER_ADMIN_UID ? 'bg-amber-400 text-slate-900 border-amber-500 shadow-xl' : 'bg-white/10 border-white/10'}`}>
              <Shield size={18} />
            </button>
        ...
        {view === 'home' && (
          <div className="relative animate-in slide-in-from-top-4">
            ...
            <input type="text" placeholder="Search Chiplun Registry..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full bg-white/10 border border-white/20 rounded-2xl py-4 pl-12 pr-4 text-white placeholder-emerald-200 outline-none focus:bg-white/20 transition-all shadow-inner" />
          </div>
        )}
      </header>
          ...
          <div className="pt-6 space-y-6 animate-in slide-in-from-bottom-8">
             ...
             {user?.uid !== MASTER_ADMIN_UID ? (
               <div className="bg-slate-900 p-8 rounded-[3.5rem] shadow-2xl space-y-6 text-center border-t-8 border-emerald-500">
                  ...
                  <h3 className="text-white text-xl font-black uppercase italic tracking-tighter leading-none">Identity Verification</h3>
                  <div className="space-y-4">
                     <div className="relative">
                        <Mail className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500" size={16} />
                        <input type="email" placeholder="System Email" value={adminCreds.email} onChange={e => setAdminCreds({...adminCreds, email: e.target.value})} className="w-full bg-slate-800 border border-slate-700 p-5 pl-12 rounded-2xl text-white outline-none focus:border-emerald-500 transition-all font-bold text-xs" />
                     </div>
                     <div className="relative">
                        <Key className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500" size={16} />
                        <input type="password" placeholder="Passkey" value={adminCreds.pass} onChange={e => setAdminCreds({...adminCreds, pass: e.target.value})} className="w-full bg-slate-800 border border-slate-700 p-5 pl-12 rounded-2xl text-white outline-none focus:border-emerald-500 transition-all font-bold text-xs" />
                     </div>
               ...
               <div className="space-y-6 pb-20 px-1 animate-in fade-in">
                  ...
                  <div className="flex bg-slate-200 p-1 rounded-2xl shadow-inner">
                    ...
                    <button onClick={() => setAdminTab('merchants')} className={`flex-1 py-3 rounded-xl text-[9px] font-black uppercase transition-all ${adminTab === 'merchants' ? 'bg-white text-emerald-600 shadow-sm' : 'text-slate-500'}`}>Live ({stores.length})</button>
                 </div>
                 ...
                 {adminTab === 'requests' ? requests.map(r => (
                    ...
                    <div key={s.id} className="bg-white p-5 rounded-[2.5rem] border border-slate-100 shadow-sm flex justify-between items-center animate-in fade-in">
                       ...
                       <button onClick={() => { if(window.confirm("Purge Network Node?")) deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'stores', s.id)); }} className="p-2 bg-slate-50 rounded-lg text-rose-600 active:scale-90 transition-all"><Trash2 size={16}/></button>
                    </div>
                 ))}
               </div>
             )}
          ...
          <div className="pt-6 space-y-6 animate-in slide-in-from-bottom-8 px-1 pb-32">
             ...
             {mTab === 'ledger' && (
                <div className="space-y-4 animate-in fade-in">
                   <div className="grid grid-cols-2 gap-4">
                      <div className="bg-white p-6 rounded-[2rem] border border-slate-100 text-center"><p className="text-[8px] font-black text-slate-400 uppercase">Rev.</p><p className="text-2xl font-black text-emerald-600 italic leading-none">₹{merchantData.rev}</p></div>
                      <div className="bg-white p-6 rounded-[2rem] border border-slate-100 text-center"><p className="text-[8px] font-black text-slate-400 uppercase">Queue</p><p className="text-2xl font-black text-blue-600 italic leading-none">{merchantData.queue.length}</p></div>
                   </div>
           ...
           <div className="pt-6 space-y-6 animate-in slide-in-from-bottom-8 px-1">
              ...
              {mySpot && !mySpot.error ? (
                <div className="bg-white p-8 rounded-[3.5rem] shadow-2xl border-t-8 border-emerald-500 text-center space-y-6 animate-in zoom-in-95">
                   ...
                   <div><h3 className="text-3xl font-black tracking-tighter uppercase italic leading-none">{mySpot.displayId}</h3><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-2">Rank Confirmed</p></div>
              ...
              ) : trackId && <p className="text-center text-slate-400 font-black uppercase text-[10px] py-10 italic animate-pulse tracking-widest">Validating session...</p>}
           </div>
        )}
           ...
           <div className="pt-4 space-y-6 animate-in slide-in-from-right-4 px-1 pb-32">
             ...
             {activeCart && (
                <div className="bg-white p-6 rounded-[3rem] shadow-sm border border-slate-100 animate-in slide-in-from-bottom-6 space-y-5 mx-1">
                   <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-center italic leading-none">2. Token Details</h3>
                     ...
                     {(activeStore?.resources || []).map(r => {
                       ...
                       return (
                         <div key={r.id} onClick={() => !isFull && setBookForm({...bookForm, resId: r.id})} className={`p-4 rounded-2xl border-2 transition-all cursor-pointer flex justify-between items-center ${bookForm.resId === r.id ? 'border-emerald-600 bg-emerald-50' : 'border-slate-50 opacity-60'} ${isFull ? 'opacity-30' : ''}`}>
                           <div className="text-left"><p className="font-black text-[10px] uppercase italic leading-none">{r.name}</p>{r.time && <p className="text-[8px] font-black text-blue-500 mt-1 uppercase tracking-widest leading-none">Time: {r.time}</p>}</div>
      ...
      {showPayment && (
        <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-md z-[1000] flex items-center justify-center p-6 animate-in fade-in duration-300">
           <div className="bg-white w-full max-w-sm rounded-[3rem] p-8 shadow-2xl space-y-6 text-center">
              <h3 className="text-xl font-black uppercase tracking-tighter italic leading-none">Payment Logic</h3>
              <div className="space-y-3">
                 <button onClick={() => notify("Digital payments enabled in V51 Patch", "info")} className="w-full p-4 bg-slate-50 rounded-2xl border border-slate-100 flex items-center justify-between opacity-50"><span className="font-black text-[10px] uppercase">Online Gateway</span><span className="text-[8px] bg-rose-100 text-rose-600 px-2 py-1 rounded font-black tracking-widest">SOON</span></button>
                 <button onClick={() => setShowConfirm(true)} className="w-full p-6 bg-emerald-600 text-white rounded-[1.8rem] shadow-xl flex items-center justify-between active:scale-95 transition-all"><span className="font-black text-sm uppercase tracking-widest">Verify Cash</span><Banknote size={20} /></button>
              </div>
        ...
        <div className="fixed inset-0 bg-emerald-600 z-[1001] flex items-center justify-center p-6 animate-in zoom-in-95 duration-200 text-white text-center">
           <div className="space-y-8">
              ...
              <div className="space-y-2">
                 ...
                 <p className="text-sm font-bold uppercase opacity-70 tracking-widest">Authorization: ₹{activeCart?.price || 0}</p>
              </div>
