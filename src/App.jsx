...
import { 
  ...
  Loader2, Phone, Briefcase, BarChart3, TrendingUp, AlertCircle
} from 'lucide-react';
...
const appId = "chiplun-connect-pro"; // Main DB Identifier
...
const App = () => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (u) => {
      if (!u) {
        try {
          ...
          setErrorMsg("Failed to connect to server. Check Firebase Rules.");
        }
      }
      setUser(u);
    });
  ...
  useEffect(() => {
    ...
    // Global Bookings for Conflict Check
    ...
    // Personal Bookings listener
    const unsubMy = onSnapshot(collection(db, 'artifacts', appId, 'users', user.uid, 'bookings'), (snap) => {
    ...
    return () => { unsubProf(); unsubStores(); unsubBookings(); unsubMy(); unsubReqs(); };
  }, [user]);

  // --- 3. Resource Availability Logic ---
  const checkSlotAvailability = (date, time, staffName = null) => {
    if (!selectedStore || !date || !time) return true;
    
    // Filter bookings for this store, date, and time
    const currentBookings = allBookings.filter(b => 
      b.storeId === selectedStore.id && b.date === date && b.time === time
    );

    const totalStaffCapacity = selectedStore.staff?.length || 1;

    // Case A: User wants a specific worker
    if (staffName && staffName !== "") {
      const isWorkerBusy = currentBookings.some(b => b.staffName === staffName || b.staffName === "");
      return !isWorkerBusy;
    }

    // Case B: General slot capacity check
    return currentBookings.length < totalStaffCapacity;
  };
  ...
  const finalizeBooking = async () => {
    ...
    // Simulated bank gateway latency
    setTimeout(async () => {
      ...
      try {
        ...
        setIsProcessing(false);
        setErrorMsg("Booking failed. Check database permissions.");
      }
    }, 2000);
  };

  if (loading) return <div className="h-screen flex flex-col items-center justify-center bg-emerald-50 gap-4"><Loader2 className="animate-spin text-emerald-600" size={48}/><p className="text-emerald-900 font-bold uppercase tracking-widest">Chiplun Connect</p></div>;
    ...
    <div className="max-w-md mx-auto bg-gray-50 min-h-screen flex flex-col shadow-2xl relative font-sans overflow-hidden text-gray-900">
      ...
      {isProcessing && (
        <div className="fixed inset-0 bg-white/95 backdrop-blur-md z-[100] flex flex-col items-center justify-center animate-in fade-in">
           <div className="w-24 h-24 bg-emerald-600 rounded-[2.5rem] flex items-center justify-center mb-6 shadow-2xl shadow-emerald-100">
              <Loader2 className="animate-spin text-white" size={48} />
           </div>
           <h2 className="text-2xl font-black text-emerald-900 uppercase tracking-tighter">Securing Slot</h2>
           <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-2">Local Server Authentication</p>
        </div>
      )}
      ...
      <header className="bg-emerald-600 text-white p-6 pb-12 rounded-b-[4rem] shadow-lg sticky top-0 z-20">
        <div className="flex justify-between items-center mb-6">
          <div onClick={() => setView('home')} className="cursor-pointer active:scale-95 transition-transform">
            ...
            <div className="flex items-center text-emerald-100 text-[10px] font-black uppercase tracking-widest mt-1">
              <MapPin size={10} className="mr-1" /> Smart Local Booking
            </div>
          </div>
      ...
      <main className="flex-1 -mt-6 px-4 pb-32 overflow-y-auto z-10">
               ...
               <div className="space-y-4">
                   ...
                   <div key={store.id} onClick={() => { setSelectedStore(store); setView('store_detail'); setCart([]); }} className="bg-white p-3 rounded-[2.5rem] flex gap-4 items-center shadow-sm border border-gray-100 active:scale-98 transition-all cursor-pointer">
                     ...
                     <div className="flex-1">
                       ...
                       <div className="flex items-center gap-2 mt-2">
                         ...
                         <span className="text-[8px] bg-emerald-50 text-emerald-600 px-2 py-0.5 rounded font-black uppercase tracking-widest">{store.staff?.length || 0} Experts</span>
                       </div>
                     </div>
                 ...
                 {stores.filter(s => s.isLive).length === 0 && (
                   <div className="text-center py-20 bg-white rounded-[3rem] border-2 border-dashed border-gray-100">
                     ...
                     <p className="text-gray-300 text-[10px] font-black uppercase">No Active Businesses</p>
                   </div>
                 )}
               </div>
             </section>
               ...
               <div className="bg-white p-6 rounded-[3rem] shadow-sm animate-in slide-in-from-bottom-6">
                 ...
                 <div className="space-y-4 mb-8">
                   <div className="space-y-1.5">
                     <p className="text-[9px] font-bold text-gray-400 ml-1 uppercase">Select Expert</p>
                     <select onChange={e => setBookingMeta({...bookingMeta, staffName: e.target.value})} className="w-full bg-gray-50 p-4 rounded-2xl outline-none font-bold text-sm border-2 border-transparent focus:border-emerald-100 transition-all appearance-none">
                       <option value="">Any Professional (First Available)</option>
                       {selectedStore?.staff?.map((st, i) => <option key={i} value={st}>{st}</option>)}
                     </select>
                   </div>
                   <div className="space-y-1.5">
                     <p className="text-[9px] font-bold text-gray-400 ml-1 uppercase">Date</p>
                   ...
                   <div className="grid grid-cols-3 gap-2 pt-2">
                     {['10:00 AM', '11:00 AM', '12:00 PM', '01:00 PM', '04:00 PM', '06:00 PM'].map(t => {
                       const full = !checkSlotAvailability(bookingMeta.date, t, bookingMeta.staffName);
                       return (
                         <button key={t} disabled={full} onClick={() => setBookingMeta({...bookingMeta, time: t})} className={`py-3.5 rounded-xl text-[10px] font-black uppercase tracking-tighter transition-all ${bookingMeta.time === t ? 'bg-emerald-600 text-white shadow-xl scale-105' : full ? 'bg-gray-100 text-gray-300 line-through' : 'bg-gray-50 text-gray-600'}`}>
                           {t} {full && '• FULL'}
                         </button>
                       );
                     })}
                 ...
                 <div className="space-y-4 pt-6 border-t border-gray-100">
                    <p className="text-[9px] font-bold text-gray-400 ml-1 uppercase">Customer Profile</p>
                    <input placeholder="Enter Your Name" value={bookingMeta.custName} onChange={e => setBookingMeta({...bookingMeta, custName: e.target.value})} className="w-full bg-gray-50 p-4 rounded-2xl font-bold text-sm outline-none focus:ring-2 ring-emerald-500" />
           ...
           <div className="pt-4 space-y-6 animate-in zoom-in-95">
             ...
             <div className="bg-white p-6 rounded-[3rem] border border-gray-100 shadow-sm space-y-6">
                <div className="flex justify-between items-start">
                   ...
                   <div className="bg-emerald-50 text-emerald-600 px-4 py-1 rounded-2xl text-[10px] font-black uppercase">{bookingMeta.staffName || 'Auto-Expert'}</div>
                </div>
             ...
             <div className="space-y-4 pt-4 text-center">
                <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Choose Payment Method</p>
                <div className="grid grid-cols-3 gap-2">
                   {['upi', 'card', 'cash'].map(m => (
                     <button key={m} onClick={() => setBookingMeta({...bookingMeta, paymentMethod: m})} className={`py-5 rounded-2xl border-2 flex flex-col items-center gap-1 transition-all ${bookingMeta.paymentMethod === m ? 'border-emerald-600 bg-emerald-50 text-emerald-600 scale-105' : 'border-gray-50 bg-white text-gray-400'}`}>
             ...
             <button onClick={finalizeBooking} className="w-full bg-emerald-600 text-white py-5 rounded-3xl font-black shadow-xl mt-6">Confirm & Pay Now</button>
          </div>
        )}
        ...
        {view === 'dashboard' && profile.role === 'vendor' && (
          <div className="pt-4 space-y-6 animate-in slide-in-from-bottom-8">
            <div className="flex justify-between items-center"><h2 className="text-2xl font-black text-emerald-900 tracking-tighter">{profile.businessName}</h2><button onClick={() => setView('home')} className="bg-gray-100 p-2 rounded-xl text-gray-400 active:scale-90 transition-all"><XCircle size={18}/></button></div>
            ...
            <div className={`p-6 rounded-[3rem] border-2 flex justify-between items-center transition-all ${stores.find(s => s.ownerId === profile.businessId)?.isLive ? 'bg-emerald-50 border-emerald-200 shadow-md' : 'bg-gray-50 border-gray-100 opacity-80'}`}>
               <div><p className="text-[10px] font-black text-gray-400 uppercase">Visibility</p><p className={`font-black text-lg ${stores.find(s => s.ownerId === profile.businessId)?.isLive ? 'text-emerald-700' : 'text-gray-400'}`}>{stores.find(s => s.ownerId === profile.businessId)?.isLive ? 'PUBLIC' : 'OFFLINE'}</p></div>
            ...
            <section className="bg-white p-7 rounded-[3.5rem] shadow-sm border border-gray-100">
               <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-6 flex items-center"><Users size={14} className="mr-2"/> Staff / Team</h3>
               ...
               <div className="flex gap-2">
                 ...
                 <button onClick={() => { if(!newStaff) return; const s = stores.find(st => st.ownerId === profile.businessId); updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'stores', profile.businessId), { staff: [...(s.staff || []), newStaff] }); setNewStaff(''); }} className="bg-emerald-600 text-white p-4 rounded-2xl active:scale-95 shadow-lg"><Plus size={20}/></button>
               </div>
            </section>
            ...
            <section className="bg-white p-7 rounded-[3.5rem] shadow-sm border border-gray-100">
               ...
               <div className="space-y-3 pt-4 border-t border-gray-50">
                  <input placeholder="Service Name" value={newSvc.name} onChange={e => setNewSvc({...newSvc, name: e.target.value})} className="w-full bg-gray-50 p-4 rounded-2xl outline-none font-bold text-sm" />
          ...
          <div className="pt-4 space-y-6 animate-in slide-in-from-top-6">
            ...
            <section className="space-y-4">
              ...
              {requests.filter(r => r.status === 'pending').map(req => (
                <div key={req.id} className="bg-white p-6 rounded-[3rem] border border-gray-100 shadow-sm space-y-4">
              ...
              {requests.filter(r => r.status === 'pending').length === 0 && <p className="text-center py-20 text-gray-300 text-[10px] font-black uppercase tracking-widest">No Pending Tasks</p>}
            </section>
          </div>
        )}
        ...
        {view === 'admin_auth' && (
           <div className="pt-20 text-center animate-in zoom-in-95">
              ...
              <p className="text-[10px] text-gray-400 mb-10 uppercase tracking-widest">Enter Administrative Key</p>
              <input type="password" maxLength={4} value={adminPinInput} onChange={e => setAdminPinInput(e.target.value)} className="w-48 text-center text-5xl font-black border-b-4 outline-none py-4 text-emerald-600 border-gray-100 focus:border-emerald-500 bg-transparent mb-12 tracking-[0.4em]" />
              <button onClick={() => {if(adminPinInput===ADMIN_PIN) setView('admin_panel'); else setAdminPinInput('')}} className="w-full bg-emerald-600 text-white py-5 rounded-[2.5rem] font-black shadow-2xl active:scale-95 transition-all uppercase tracking-widest">Unlock Panel</button>
           </div>
        )}
        ...
        {view === 'vendor_portal' && profile.role !== 'vendor' && (
           <div className="pt-4 space-y-6 animate-in slide-in-from-bottom-6">
              <div className="text-center"><div className="w-24 h-24 bg-emerald-50 text-emerald-600 rounded-[2.5rem] flex items-center justify-center mx-auto mb-6 shadow-sm"><Store size={48}/></div><h2 className="text-3xl font-black tracking-tight">Merchant Portal</h2></div>
              ...
              {profile.status === 'none' ? (
                <div className="bg-white p-7 rounded-[3.5rem] space-y-4 shadow-sm mt-4 border border-gray-50">
                  ...
                  <input placeholder="Store Location (Chiplun)" value={regForm.addr} onChange={e => setRegForm({...regForm, addr: e.target.value})} className="w-full bg-gray-50 p-4 rounded-2xl font-bold text-sm outline-none focus:ring-2 ring-emerald-500" />
        ...
        {view === 'vendor_login' && (
          <div className="pt-4 space-y-6 animate-in slide-in-from-right-8">
             <div className="text-center mb-8"><div className="w-24 h-24 bg-emerald-50 text-emerald-600 rounded-[2.5rem] flex items-center justify-center mx-auto mb-6"><LogIn size={48}/></div><h2 className="text-3xl font-black text-gray-800 tracking-tighter uppercase">Merchant Login</h2></div>
        ...
        {view === 'confirmation' && (
           <div className="h-full flex flex-col items-center justify-center text-center pt-24 animate-in zoom-in-90 duration-700">
             ...
             <h2 className="text-4xl font-black text-emerald-900 mb-2 tracking-tighter uppercase">Confirmed</h2>
