...
const appId = "chiplun-pro-v50-iron-master";
...
export default function App() {
  ...
  const getInventoryCount = (sId, rId, cap) => {
  ...
  const handleBookingExecution = async () => {
    ...
    try {
      ...
      if (selStore.category === 'travel') {
         ...
         bookingTime = trip?.time || "Scheduled";
      }
      ...
      setPayModal(false);
      setView('track');
    } catch (e) { notify("Error Syncing", "error"); }
      ...
      <header className="bg-emerald-600 text-white p-6 pb-12 rounded-b-[3.5rem] shadow-lg sticky top-0 z-50">
        <div className="flex justify-between items-center mb-6">
          <div onClick={() => setView('home')} className="cursor-pointer active:scale-95 transition-all">
            ...
            <p className="text-[10px] font-bold opacity-80 uppercase tracking-widest mt-1 italic">Supreme V50 Master</p>
          </div>
          <div className="flex gap-2">
            ...
            <button onClick={() => setView(profile.role === 'vendor' ? 'merchant' : 'business')} className={`w-10 h-10 rounded-2xl flex items-center justify-center border transition-all ${view === 'merchant' || view === 'business' ? 'bg-white text-emerald-600 border-white' : 'bg-white/10'}`}>
      ...
      <main className="flex-1 -mt-6 px-4 pb-32 z-10">
        ...
        {view === 'home' && (
          <div className="space-y-8 pt-2">
            <div className="grid grid-cols-4 gap-3 animate-in fade-in">
               ...
               <button onClick={() => notify("Coming Soon", "info")} className="flex flex-col items-center gap-2 opacity-30"><div className="p-4 bg-white rounded-2xl border border-slate-100 shadow-sm"><Lucide.Plus size={20}/></div><span className="text-[9px] font-black uppercase text-slate-400">More</span></button>
            </div>
            <section className="space-y-4">
              ...
              <div className="space-y-4">
                {(marketplace || []).map(store => (
                  <div key={store.id} onClick={() => { setSelStore(store); setView('detail'); setSelService(null); setBookForm({ custName:'', date:'', time:'', phone:'', resId:'', seats:1 }); }} className="bg-white p-3 rounded-[2.5rem] flex gap-4 items-center shadow-sm border border-slate-100 active:scale-[0.98] transition-all">
        ...
        {view === 'track' && (
           <div className="pt-6 space-y-6 animate-in slide-in-from-bottom-8 px-1">
              ...
              <div className="bg-white p-6 rounded-[2.5rem] shadow-xl border border-slate-100 space-y-4">
                 ...
                 <button onClick={() => { const b = bookings.find(x => x.displayId === trackInput); if(b) setActiveReceipt(b); else notify("No active token found", "error"); }} className="w-full py-5 bg-emerald-600 text-white rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-lg active:scale-95">Check My Position</button>
              </div>
              ...
              {trackerStats && !trackerStats.error ? (
                <div className="bg-white p-8 rounded-[3.5rem] shadow-2xl border-t-8 border-emerald-500 text-center space-y-6 animate-in zoom-in-95">
                   ...
                   <button onClick={() => setView('home')} className="w-full py-4 bg-slate-900 text-white rounded-2xl font-black uppercase text-[10px] active:scale-95">Back explorer</button>
                </div>
              ) : activeReceipt && <p className="text-center text-rose-500 font-black uppercase text-[10px] tracking-widest py-10 italic">Token is finished or removed.</p>}
           </div>
        )}

        {/* VIEW: MERCHANT DASHBOARD (SAFE DATA) */}
        {view === 'merchant' && (
          <div className="pt-6 space-y-6 animate-in slide-in-from-bottom-8 px-1">
            {!mData ? (
               <div className="flex flex-col items-center justify-center py-20 gap-4">
                  <Lucide.Loader2 className="animate-spin text-emerald-600" size={40} />
                  <p className="font-black uppercase text-[10px] tracking-widest text-slate-400">Loading Business Data...</p>
                  <button onClick={() => { setView('home'); setProfile({role:'customer'}); }} className="text-rose-500 font-black uppercase text-[8px] tracking-widest">Logout</button>
               </div>
            ) : (
               <>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-slate-900 p-6 rounded-[2.5rem] text-white flex flex-col justify-between shadow-lg h-44">
                       <div><p className="text-[8px] font-black uppercase opacity-50 mb-1 tracking-widest leading-none">VISIBILITY</p><p className="text-lg font-black uppercase italic tracking-tighter leading-none">{mData.store.isLive ? 'Active' : 'Offline'}</p></div>
                  ...
                  {mTab === 'ledger' && (
                     <section className="space-y-4 pb-20 px-1 animate-in fade-in">
                        <div className="grid grid-cols-2 gap-4 px-1">
                          <div className="bg-white p-6 rounded-[2rem] border border-slate-100 text-center shadow-sm"><p className="text-[8px] font-black text-slate-400 mb-1">TOTAL EARNED</p><p className="text-2xl font-black text-emerald-600 italic leading-none">₹{mData.rev}</p></div>
                          <div className="bg-white p-6 rounded-[2rem] border border-slate-100 text-center shadow-sm"><p className="text-[8px] font-black text-slate-400 mb-1">WAITLIST</p><p className="text-2xl font-black text-blue-600 italic leading-none">{mData.queue.length}</p></div>
                        </div>
                        {mData.queue.map((b, i) => (
                          <div key={i} className="bg-white p-5 rounded-[2rem] border-l-8 border-emerald-500 shadow-sm space-y-4 animate-in slide-in-from-left-4">
                            <div className="flex justify-between items-start">
                              <div>
                                 ...
                                 <p className="text-[8px] text-blue-600 font-black uppercase mt-1 tracking-widest italic">{b.seats || 1} Person • Cash</p>
                              </div>
                            ...
                            <div className="flex gap-2">
                              ...
                              <button onClick={async () => { if(window.confirm("Cancel Booking? Seats will restore!")) await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'bookings', b.id)); }} className="p-3 bg-rose-50 text-rose-500 rounded-xl active:scale-90"><Lucide.X size={18}/></button>
                            </div>
                          </div>
                        ))}
                     </section>
                  ...
                  {mTab === 'assets' && (
                    <div className="bg-white p-8 rounded-[3rem] border border-slate-100 space-y-6 mx-1 animate-in fade-in">
                      ...
                      }} className="w-full py-4 border-2 border-dashed border-slate-200 rounded-2xl font-black uppercase text-[10px] text-slate-400 hover:border-emerald-300 transition-all">+ Add Item</button>
          ...
          <div className="pt-10 space-y-6 animate-in fade-in px-2">
             ...
             {!adminAuth ? (
               <div className="bg-white p-8 rounded-[3rem] shadow-2xl border border-rose-100 space-y-4 text-center">
                 ...
                 <button onClick={() => { if (adminPin === ADMIN_PIN) setAdminAuth(true); else notify("Verification Failed", "error"); }} className="w-full bg-rose-600 text-white py-5 rounded-2xl font-black shadow-xl uppercase active:scale-95 transition-all tracking-[0.2em]">Authorize Terminal</button>
               </div>
             ) : (
        ...
        {view === 'detail' && selStore && (
          <div className="pt-4 space-y-6 animate-in slide-in-from-right-4 px-1 pb-32">
            <button onClick={() => setView('home')} className="flex items-center text-emerald-600 font-black text-[10px] uppercase tracking-widest active:scale-95 px-2 leading-none"><Lucide.ArrowLeft size={16} className="mr-2"/> Back Discovery</button>
              ...
              <div className="bg-white p-6 rounded-[3rem] shadow-sm border border-slate-100 animate-in slide-in-from-bottom-6 space-y-5">
                  ...
                  <div className="space-y-2">
                    ...
                    {(selStore.resources || []).map(r => {
                      const { count, left } = getInventoryCount(selStore.id, r.id, r.capacity);
                  ...
                  {isCheatTimeDetected ? (
                     <div className="p-4 bg-rose-50 border border-rose-100 rounded-2xl flex items-center gap-3 text-rose-500">
                        ...
                        <span className="text-[10px] font-black uppercase tracking-tighter">Cannot select past time for today!</span>
                     </div>
                  ) : (
        ...
        <div className="fixed inset-0 bg-emerald-600 z-[1001] flex items-center justify-center p-6 animate-in zoom-in-95 duration-200 text-white text-center">
           <div className="space-y-8">
              ...
              <div className="space-y-2">
                 ...
                 <p className="text-sm font-bold uppercase opacity-70 tracking-widest leading-none">Confirming will generate Token ID</p>
              </div>
...
// Logic check outside scope for safety
